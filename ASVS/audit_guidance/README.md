...
There are four places guidance can live, each suited to different content. Projects can use any combination of them; the audit pipeline merges all available sources at run time.

### `.audit_guidance/` in the project's own repo

For projects that prefer to own and version their guidance directly. Drop an `.audit_guidance/` directory at the repo root, follow the same file conventions as the per-project subdirectories below, and the audit pipeline picks it up when it clones the target repo. This is the right home for guidance that's safe to publish and benefits from travelling with the code — same reviews, same git history, no drift between what the guidance says and what the code does.

### `audit_guidance/<project>/` here in tooling-agents

For projects that would rather have Tooling maintain the guidance on their behalf, or for guidance that the Tooling team is iterating on jointly with the project. The directory layout below (`audit_guidance/tooling-trusted-releases/`, etc.) is exactly this case. Same content rules, just hosted here instead of in the project repo. The audit pipeline checks both locations and uses whatever is available.

A project can use both sources simultaneously. A common pattern: the project owns its own `.audit_guidance/` for architecture-level context that changes as the code changes; Tooling maintains additional files under `audit_guidance/<project>/` for cross-project security patterns or while a project ramps up on the convention.
