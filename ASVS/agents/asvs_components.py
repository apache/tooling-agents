...
async def run(input_dict, tools):
    ...
    try:
        # Inputs arrive as a JSON object in inputText (the convention used by
        # asvs_guidance_upload / asvs_push_github), so a single inputText
        # field carries all parameters regardless of how many there are.
        #   inputText: {"repo": "owner/repo"}
        input_text = input_dict.get("inputText", "")
        if not input_text:
            return {"outputText": json.dumps({"error": "inputText is required (JSON with 'repo')"})}
        try:
