...
*Informational findings are recorded in this report but not opened as GitHub issues — see issues.md for the 1 actionable items.*
...
This audit was conducted against ASVS Level 3 (L3) across all in-scope domains, with the severity threshold set to none so that every finding — including informational and not-applicable determinations — is recorded. Because `superset-extensions-cli` is a local, developer-only scaffolding tool with no network listener, the overwhelming majority of network- and authentication-oriented L3 requirements do not instantiate and are delegated to the Flask-AppBuilder and deployment layers.

### Top 5 Risks

1. **Unbounded/unpinned first-party-named dependency** [Low] — A dependency referenced by a first-party-style name lacks index pinning and integrity hashing, leaving a dependency-confusion / supply-chain exposure (ASVS 15.2.4).
2. **No multi-stage canonicalization/decoding pipeline** [Info] — No layered decode/canonicalization path exists, so the requirement is largely not applicable in this dev-only CLI (ASVS 1.1.1).
3. **Regular expressions present without exploitable ReDoS path** [Info] — Static developer-authored regexes operate in a local-only context with no attacker-reachable input amplification (ASVS 1.3.12).
4. **Jinja2 templates treated as static repository files** [Info] — User input is passed as render data rather than template source, structurally barring server-side template injection (ASVS 1.3.7).
5. **No dynamic code execution primitives in the CLI** [Info] — Absence of eval/exec/compile/pickle/getattr-dispatch sinks removes the dynamic code execution attack surface (ASVS 1.3.2).

### Positive Controls Observed

- **Input validation:** Template inputs (`display_name`/`publisher`/`name`) pass strict allow-list validators before Jinja rendering, structurally barring JSON/JSX break-out even in the dev-only context.
- **Safe serialization:** Security-relevant manifest artifact is produced via Pydantic `model_dump_json` (proper encoder); all interpolated values are allow-list validated.
- **No shell injection:** All `subprocess.run` calls use argument-list form (no `shell=True`) with static command tokens; no user input reaches a command interpreter.
- **Static regex usage:** All regex patterns are static developer-authored literals; user input is the subject of `.match()`, never used to build patterns.
- **No dynamic execution:** No `eval`/`exec`/`compile`/`pickle`/`getattr`-dispatch primitives are used.
- **Schema validation:** CLI applies named allow-list validation patterns and Pydantic `ExtensionConfig` schema validation for extension metadata.
- **Fail-closed build pipeline:** Local build pipeline fails closed when prerequisite steps are skipped (bundle→build→validate, manifest-existence check).
- **Path-traversal resistance:** Internally-generated, regex-validated identifiers drive directory layout, neutralizing path metacharacters before path construction.
- **Constrained file operations:** CLI confines all file operations to the developer's working directory with no untrusted file ingestion path.
- **Single-parser discipline:** Single JSON and single TOML parser used consistently; no XML parser instantiated, eliminating XXE attack surface.
- **Modern client stack:** Scaffold templates emit a modern supported client stack (React 17 / TypeScript 5 / webpack 5) with no banned legacy browser technologies, and bundle/externalize dependencies locally rather than loading unversioned CDN assets.
- **Proper delegation of auth concerns:** Password, MFA, federated identity, and session lifecycle handling are correctly delegated to Flask-AppBuilder and the deployment layer rather than re-implemented in the dev-only CLI.
- **No hardcoded credentials:** No hardcoded credentials or default accounts in CLI, templates, or test fixtures.
...
#### FINDING-001: Unbounded/unpinned first-party-named dependency with no index or integrity pinning
...
| Files | superset-extensions-cli/pyproject.toml, superset-extensions-cli/src/superset_extensions_cli/templates/frontend/package.json.j2 |
| Source Reports | 15.2.4.md |
| Related | None |

**Description:**

The `apache-superset-core` runtime dependency in `pyproject.toml` is declared without version bounds, index pinning, or hash/lockfile integrity verification, and the CLI imports `superset_core.extensions.*` at startup. If the distribution name is unclaimed on the index a developer's resolver uses, a third party could publish a malicious package, yielding arbitrary code execution in the developer's environment at install/import time. This is the explicitly in-scope exception to the dev-only carve-out (CLI fetches remote dependency content without integrity verification, where tampering compromises the developer's workstation). Severity bounded to Low because exploitability depends on the first-party name being unclaimed (in practice ASF-controlled) and no concrete unclaimed-name proof exists; the npm scoped `@apache-superset/core` is protected by scope ownership.

**Remediation:**

Pin a floor and ceiling once a stable `apache-superset-core` exists and ship a hash-pinned lockfile (`--require-hashes`); recommend installing from the explicit expected index; reserve `apache-superset-core` and `@apache-superset/*` names on public registries to foreclose squatting.

---

### 3.5 Informational

#### FINDING-002: No multi-stage canonicalization/decoding pipeline present — requirement largely N/A
...
| ASVS Level(s) | L2 |
| CWE | N/A |
| ASVS Section(s) | 1.1.1 |
| Files | superset_extensions_cli/utils.py, superset_extensions_cli/cli.py |
| Source Reports | 1.1.1.md |
| Related | None |

**Description:**

Each input is parsed exactly once before validation; no double-decode or decode-after-validate pattern exists.

**Remediation:**

None required. Continue parsing each input exactly once before validation.

---

#### FINDING-003: Not applicable — CLI produces no HTTP responses, HTML documents, or XML documents
...
The CLI emits only on-disk scaffolding artifacts; it renders no HTTP/HTML/XML output. Requirement N/A.
...
#### FINDING-004: Not applicable — no dynamic URL construction from untrusted data in a network context
...
| Files | templates/frontend/webpack.config.js.j2 |
| Source Reports | 1.2.2.md |
| Related | None |

**Description:**

Only a relative webpack publicPath built from strictly validated kebab-case identifiers; no scheme-bearing URL or remote fetch from untrusted data. Requirement N/A.
...
#### FINDING-005: Not applicable — the CLI performs no database access or query construction
...
No SQL/NoSQL/ORM/stored-procedure interaction exists; persistence is local JSON/TOML files and a zip bundle. Requirement N/A.
...
#### FINDING-006: No untrusted format-string evaluation — requirement not applicable
...
| Files | superset-extensions-cli/src/superset_extensions_cli/cli.py, superset-extensions-cli/src/superset_extensions_cli/utils.py |
| Source Reports | 1.3.10.md |
| Related | None |

**Description:**

Format templates are hard-coded literals; user values are only f-string arguments. No format-template injection sink. Requirement N/A.
...
#### FINDING-007: Regular expressions present but no exploitable ReDoS path in local-only context
...
In-repo regexes are linear (no nested quantifiers) operating on short bounded strings; no remote/untrusted input channel exists. No demonstrable ReDoS scenario.

**Remediation:**

No action required; if patterns are reused server-side on untrusted input, audit for linear-time matching and apply length caps there.

---

#### FINDING-008: No dynamic code execution primitives present in the CLI
...
No eval/exec/compile/pickle/getattr-dispatch; subprocess uses fixed argument lists. No dynamic code-execution sink. Requirement satisfied.
...
#### FINDING-009: Input is validated against allow-list patterns; no network-reachable dangerous context
...
| Files | superset-extensions-cli/src/superset_extensions_cli/utils.py |
| Source Reports | 1.3.3.md |
| Related | None |

**Description:**

All name/identifier inputs pass allow-list validators; missing max-length cap is a dev-only hardening nicety with no attack scenario.

**Remediation:**

Optional hardening: add explicit max-length bounds to name validators.

---

#### FINDING-010: Jinja2 templates are static repository files; user input is passed as render data, not as template source
...
Templates are immutable package files loaded by fixed name; user values are render context only, never the template body. No SSTI sink. The autoescape observation is a documented dev-only design decision, not a vulnerability.

**Remediation:**

Optional hardening: selectively enable Jinja autoescape / use json.dumps for generated artifacts.

---

#### FINDING-011: No systems-language or unmanaged code present
...
| ASVS Level(s) | L2 |
| CWE | N/A |
| ASVS Section(s) | 1.4.1 |
| Files | superset-extensions-cli/src/superset_extensions_cli/cli.py |
| Source Reports | 1.4.1.md |
| Related | None |

**Description:**

ASVS 1.4.1 targets unmanaged-language memory safety. The CLI is pure memory-safe CPython with no FFI/C extensions; no unsafe memory operations exist. Requirement N/A.

**Remediation:**

None required. Maintain all-managed-Python implementation.

---

#### FINDING-012: Arbitrary-precision integers; no fixed-width integer arithmetic
...
CPython integers are arbitrary-precision and cannot overflow; no size/index arithmetic feeds allocations. Requirement N/A.
...
#### FINDING-013: Garbage-collected runtime; no manual memory management or raw pointers
...
CPython manages object lifetime; file/archive handles and observer threads are correctly released via context managers and stop/join. Requirement N/A.
...
|--------|---------|-----------------|------------------|
| Flask Backend Input Validation | Template inputs (display_name/publisher/name) pass strict allow-list validators before Jinja rendering, structurally barring JSON/JSX break-out even in the dev-only context. | Promoted from dropped finding ASVS-112-LOW-001 | — |
| Flask Backend Input Validation | Security-relevant manifest artifact is produced via Pydantic model_dump_json (proper encoder); all interpolated values are allow-list validated. | source: Dropped finding ASVS-123-LOW-001 | — |
| Flask Backend Input Validation | All subprocess.run calls use argument-list form (no shell=True) with static command tokens; no user input reaches a command interpreter. | Report 1.2.5 (no findings) | — |
| Flask Backend Input Validation | All regex patterns are static developer-authored literals; user input is the subject of .match(), never used to build patterns. | Report 1.2.9 (no findings) | — |
| Flask Backend Input Validation | Format templates are hard-coded literals with user values only as f-string arguments | No format-template injection sink present | superset-extensions-cli/src/superset_extensions_cli/cli.py, superset-extensions-cli/src/superset_extensions_cli/utils.py |
| Flask Backend Input Validation | No eval/exec/compile/pickle/getattr-dispatch primitives used | CLI implementation uses fixed argument lists for subprocess calls | superset-extensions-cli/src/superset_extensions_cli/cli.py |
| Flask Backend Input Validation | superset-extensions-cli is a local dev-only tool with no network listener; injection-oriented requirements presupposing an untrusted cross-boundary data path do not instantiate here | 1.3.8.md - dropped finding ASVS-138-INFO-001 | — |
| Flask Backend Input Validation | No XML parser instantiated; only JSON/TOML/Jinja2 used in this dev-only CLI | 1.5.1.md - XXE attack surface does not exist | — |
| Flask Backend Input Validation | Single JSON and single TOML parser used consistently | 1.5.3.md - No exploitable parser divergence found; single parser implementation per format | — |
| Flask Backend Input Validation | No URL/URI parsing or remote fetch in dev-only local CLI | 1.5.3.md - No network-exposed parsing surface; RFI/SSRF/interop threat model does not instantiate | — |
| Flask Backend Input Validation | CLI applies named allow-list validation patterns and pydantic schema validation for extension metadata | 2.1.1.md - dropped finding ASVS-211-INFO-001 | — |
| Business Logic Controls | Local build pipeline fails closed when prerequisite steps are skipped (bundle→build→validate, manifest-existence check) | Sequential step enforcement in build pipeline prevents skipping of required steps | — |
| Browser Security Headers | superset-extensions-cli is dev-only with no browser/HTTP surface; browser-security feature documentation belongs to the web application + deployment layer | Dev-only CLI tool with no HTTP response handling | — |
| Browser Security Headers | Session lifecycle and cookie handling delegated to Flask-AppBuilder and deployment layer | Dev-only CLI; session lifecycle/cookie handling delegated to Flask-AppBuilder and deployment layer | flask_appbuilder_security_controls.md, extensions_cli_dev_only.md |
| Browser Security Headers | Wildcard dev-server CORS is confined to webpack devServer (development mode) and never emitted into production extension build artifacts | Development-only CORS configuration in webpack devServer, not present in production builds | — |
| Browser Security Headers | Generated extension scaffolds bundle/externalize all dependencies locally rather than loading unversioned CDN assets; SRI is the main frontend build pipeline's concern, not the dev-only CLI. | Promoted from dropped finding ASVS-361-INFO-001 | — |
| Browser Security Headers | Scaffold templates emit a modern supported client stack (React 17 / TypeScript 5 / webpack 5) with no banned legacy browser technologies. | source: Dropped finding ASVS-371-INFO-001 | — |
| Http Protocol Security | superset-extensions-cli emits no HTTP responses; it is a local-only developer scaffolding tool with no network attack surface | Dropped finding ASVS-411-INFO-001 | — |
| Http Protocol Security | Method restriction enforced in Flask/FAB layer | Dev-only CLI dispatches Click subcommands, not HTTP methods; method restriction is enforced in the Flask/FAB layer | — |
| Http Protocol Security | CLI invokes npm via argument lists (not shell strings), avoiding shell-injection and HTTP-framing surfaces | Dropped finding ASVS-422-INFO-001 | — |
| File Upload Handling | CLI confines all file operations to the developer's working directory with no untrusted file ingestion path | Application design prevents untrusted file upload scenarios | — |
| File Upload Handling | Strong schema validation via pydantic ExtensionConfig and regex name validators for project metadata | File extension and naming validation enforced at schema level | — |
| File Upload Handling | Internally-generated, regex-validated identifiers drive directory layout, neutralizing path metacharacters before path construction | Path traversal protection through controlled identifier generation | — |
| Password Authentication | Password/credential handling is correctly delegated to Flask-AppBuilder and not re-implemented in the dev-only extensions CLI | Dropped finding ASVS-623-INFO-001 | — |
| Password Authentication | Credential handling kept out of the dev-only extensions CLI; breach-checking delegated to FAB | Dropped finding ASVS-624-INFO-001 | — |
| Password Authentication | Naming validators in the CLI are functional-correctness checks, not credential composition rules; password policy delegated to FAB | Dropped finding ASVS-625-INFO-001 | — |
| Password Authentication | Scaffold templates generate no authentication UI/password fields; masking is a FAB concern | Dropped finding ASVS-626-INFO-001 | — |
| Password Authentication | No anti-paste or password-manager-blocking behavior in CLI or generated templates | Dropped finding ASVS-627-INFO-001 | — |
| Password Authentication | Case/whitespace normalization is scoped to non-secret naming metadata, isolated from any credential path | Promoted from dropped finding ASVS-628-INFO-001 | — |
| Password Authentication | No maximum-length cap on any secret; CLI has no secret-handling surface. | Dropped finding ASVS-629-INFO-001 | — |
| Password Authentication | CLI exposes no network authentication surface; anti-automation delegated to FAB/ingress/WAF | Dropped finding ASVS-631-INFO-001 | — |
| Password Authentication | No hardcoded credentials or default accounts in CLI, templates, or test fixtures | source: Dropped finding ASVS-632-INFO-001 | — |
| Password Authentication | CLI implements no authentication; MFA enforcement delegated to FAB/deployment | Dropped finding ASVS-633-INFO-001 | — |
| Password Authentication | CLI has no authentication surface; pathway-consistency enforcement is a FAB concern. | Dropped finding ASVS-634-INFO-001 | — |
| Password Authentication | CLI has no authentication event lifecycle; notifications are a FAB concern. | Dropped finding ASVS-635-INFO-001 | — |
| Password Authentication | No email used as an authentication mechanism; CLI handles no email or authentication | Dropped finding ASVS-636-INFO-001 | — |
| Password Authentication | CLI manages only project metadata, no credential/identity lifecycle; notifications delegated to FAB. | Dropped finding ASVS-637-INFO-001 | — |
| Password Authentication | No user-enumeration vector; CLI handles no user identities and exposes no network endpoints | Dropped finding ASVS-638-INFO-001 | — |
| Password Authentication | CLI generates no secret material; initial-credential lifecycle delegated to FAB. | Dropped finding ASVS-641-INFO-001 | — |
| Password Authentication | CLI collects only non-sensitive project metadata; no surface for password hints or KBA | Dropped finding ASVS-642-INFO-001 | — |
| Password Authentication | CLI manages no credentials and exposes no reset path; reset flows delegated to FAB. | source: Dropped finding ASVS-643-INFO-001 | — |
| Password Authentication | CLI implements no MFA enrollment/recovery; identity-proofing delegated to FAB. | Dropped finding ASVS-644-INFO-001 | — |
| Password Authentication | CLI manages no expiring authentication mechanisms; renewal lifecycle delegated to FAB. | source: Dropped finding ASVS-645-INFO-001 | — |
| Password Authentication | CLI has no admin user-management or password-setting surface; reset behavior delegated to FAB. | Admin-initiated password reset delegated to Flask-AppBuilder RBAC/auth layer; audited component is dev-only superset-extensions-cli | — |
| Federated Authentication | Codebase contains no identity-provider integration | superset-extensions-cli is a dev-only local tool with no auth surface; IdP identity binding delegated to Flask-AppBuilder | extensions_cli_dev_only.md, flask_appbuilder_security_controls.md |
| Federated Authentication | Dev-only CLI with no assertion processing | extensions_cli_dev_only.md | — |
| Federated Authentication | JWT/SAML signature validation delegated to FAB AUTH_TYPE managers | flask_appbuilder_security_controls.md | — |
| Session Management | superset-extensions-cli holds no session or credential state; session lifecycle is delegated to Flask-AppBuilder | Dropped finding ASVS-711-INFO-001 in 7.1.1.md | — |
| Session Management | Dev-only CLI maintains no session | extensions_cli_dev_only.md referenced in 7.3.1.md | — |
| Session Management | Inactivity timeout delegated to Flask-AppBuilder | flask_appbuilder_security_controls.md referenced in 7.3.1.md | — |
| Session Management | Logout provided by Flask-AppBuilder in main app | flask_appbuilder_security_controls.md referenced in 7.4.4.md | — |
| Session Management | Dev-only CLI renders no authenticated pages | extensions_cli_dev_only.md referenced in 7.4.4.md | — |
| Session Management | Dev-only CLI has no session store/admin role | extensions_cli_dev_only.md referenced in 7.4.5.md | — |
| Session Management | Session administration delegated to FAB | flask_appbuilder_security_controls.md referenced in 7.4.5.md | — |
| Session Management | Dev-only CLI implements no federation | extensions_cli_dev_only.md referenced in 7.6.1.md | — |
| Session Management | Federated auth delegated to FAB AUTH_TYPE managers | flask_appbuilder_security_controls.md referenced in 7.6.1.md | — |
| Rbac Authorization | CLI performs no authorization logic, preserving the backend as the authoritative enforcement boundary | Dev-only local CLI with no cross-boundary operations; all authorization enforced by backend services | extensions_cli_dev_only.md, frontend_backend_enforcement_boundary.md |
| Rbac Authorization | Field-level authorization enforced by backend serialization layer | Backend implements field-level (BOPLA) controls through serialization layer, preventing unauthorized field access | extensions_cli_dev_only.md, frontend_backend_enforcement_boundary.md |
| Rbac Authorization | CLI commands operate only against developer-supplied local paths, crossing no privilege boundary | CLI operates exclusively on local file system with developer's own permissions, no multi-user authorization required | — |
| Jwt Token Security | CLI performs no token validation and contains no homegrown signature-checking code; no token-acceptance trust boundary exists. | Observed in ASVS 9.1.1 audit - application does not implement JWT validation logic | — |
| Jwt Token Security | No cryptographic algorithm negotiation exists in the CLI, so there is no algorithm-confusion or 'none'-downgrade surface. | Observed in ASVS 9.1.2 audit - application does not implement algorithm selection logic | — |
| Jwt Token Security | CLI does not fetch remote key material or honor untrusted key-source pointers; no untrusted-key-injection surface. | Observed in ASVS 9.1.3 audit - application does not implement remote key fetching | — |
| Cryptographic Operations | Extension .supx bundles are intentionally unsigned; trust is anchored at the trusted-Admin install step rather than at the local developer CLI | Architectural decision to rely on admin-level trust rather than cryptographic signing for extension bundles | — |
| Cryptographic Operations | Password storage delegated to Flask-AppBuilder | CLI does not store or handle passwords; password storage delegated to Flask-AppBuilder framework | flask_appbuilder_security_controls.md, extensions_cli_dev_only.md |
| Tls Configuration | CLI delegates all network transport to established tooling (npm) and the deployment layer; implements no ad-hoc TLS that could downgrade protocol versions. | 12.1.1.md - Dropped finding ASVS-1211-INFO-001 | — |
| Tls Configuration | No insecure or hardcoded cipher configuration exists; the CLI introduces no cipher policy at all. | 12.1.2.md - Promoted from dropped finding ASVS-1212-INFO-001 | — |
| Tls Configuration | Network transport for dependency installation is delegated to mature tooling (npm); plaintext dev server is confined to localhost developer scaffolding, not a production path. | 12.2.1.md - Promoted from dropped finding ASVS-1221-INFO-001 | — |
| Tls Configuration | Subprocess invocations use argument lists (no shell string interpolation), avoiding command injection in the local context. | 12.3.1.md - Promoted from dropped finding ASVS-1231-INFO-001 | — |
| Tls Configuration | No certificate-verification-disabling patterns (verify=False, CERT_NONE, _create_unverified_context, check_hostname=False) present in the CLI. | 12.3.2.md - Dropped finding ASVS-1232-INFO-001 | — |
| Data Protection | Build pipeline emits content-hashed filenames (cache-busting) for static build assets, preventing stale-cache serving of updated bundles. | Observed in build configuration (source: 14.2.5.md) | — |
| Data Protection | Manifest serialization uses a typed Pydantic model (exclude_none, by_alias) constraining output to declared schema fields — good field-projection discipline. | Observed in serialization layer (source: 14.2.6.md) | — |
| Dependency Management | Third-party remediation timeframes governed by the documented ASF vulnerability-handling process | Documented in third_party_dependency_policy.md | — |
| Dependency Management | All subprocess.run invocations use fixed list-form argv with no shell=True, eliminating shell injection | Promoted from dropped finding ASVS-1515-INFO-001 | — |
| Dependency Management | Test/runtime dependency separation (test deps under optional extra), src/ packaging boundary, and coverage exclusions keep dev/test functionality out of distributable package | Report 15.2.3 (no findings) | — |
| Dependency Management | Subprocess calls use list-form argv (no shell=True) | Report 15.2.5 (no findings) | — |
| Dependency Management | Jinja2 templates loaded only from packaged templates dir | Report 15.2.5 (no findings) | — |
| Dependency Management | Generated identifiers validated against reserved-word/format allow-lists | Report 15.2.5 (no findings) | — |
| Extension System Security | File/archive handles and observer threads are correctly released via context managers and stop/join | CPython manages object lifetime appropriately | superset-extensions-cli/src/superset_extensions_cli/cli.py |
...
| 1.1.1 | Verify that input is decoded or unescaped into a canonical form only once, it is only decoded when encoded data in that form is expected, and that this is done before processing the input further, for example it is not performed after input validation or sanitization. | **Pass** | See FINDING-002 |
...
| 1.2.1 | Verify that output encoding for an HTTP response, HTML document, or XML document is relevant for the context required, such as encoding the relevant characters for HTML elements, HTML attributes, HTML comments, CSS, or HTTP header fields, to avoid changing the message or document structure. | **N/A** | See FINDING-003 |
| 1.2.2 | Verify that when dynamically building URLs, untrusted data is encoded according to its context (e.g., URL encoding or base64url encoding for query or path parameters). Ensure that only safe URL protocols are permitted (e.g., disallow javascript: or data:). | **N/A** | See FINDING-004 |
...
| 1.2.4 | Verify that data selection or database queries (e.g., SQL, HQL, NoSQL, Cypher) use parameterized queries, ORMs, entity frameworks, or are otherwise protected from SQL Injection and other database injection attacks. This is also relevant when writing stored procedures. | **N/A** | See FINDING-005 |
...
| 1.3.3 | Verify that data being passed to a potentially dangerous context is sanitized beforehand to enforce safety measures, such as only allowing characters which are safe for this context and trimming input which is too long. | **Pass** | See FINDING-009 |
...
| 1.3.10 | Verify that format strings which might resolve in an unexpected or malicious way when used are sanitized before being processed. | **N/A** | See FINDING-006 |
...
| 1.3.12 | Verify that regular expressions are free from elements causing exponential backtracking, and ensure untrusted input is sanitized to mitigate ReDoS or Runaway Regex attacks. | **Pass** | See FINDING-007 |
| 1.4.1 | Verify that the application uses memory-safe string, safer memory copy and pointer arithmetic to detect or prevent stack, buffer, or heap overflows. | **N/A** | See FINDING-011 |
| 1.4.2 | Verify that sign, range, and input validation techniques are used to prevent integer overflows. | **N/A** | See FINDING-012 |
| 1.4.3 | Verify that dynamically allocated memory and resources are released, and that references or pointers to freed memory are removed or set to null to prevent dangling pointers and use-after-free vulnerabilities. | **N/A** | See FINDING-013 |
...
| 15.2.4 | Verify that third-party components and all of their transitive dependencies are included from the expected repository, whether internally owned or an external source, and that there is no risk of a dependency confusion attack. | **Partial** | See FINDING-001 |
...
- **Partial**: 1 requirements (0.3%)
- **N/A**: 324 requirements (93.9%)
...
|------------|----------|-------------------|------------------|---------------------|
| FINDING-001 | Low | 15.2.4 | — | superset-extensions-cli/pyproject.toml, superset-extensions-cli/src/superset_extensions_cli/templates/frontend/package.json.j2 |
| FINDING-002 | Informational | 1.1.1 | — | superset_extensions_cli/utils.py, superset_extensions_cli/cli.py |
| FINDING-003 | Informational | 1.2.1 | — | — |
| FINDING-004 | Informational | 1.2.2 | — | templates/frontend/webpack.config.js.j2 |
| FINDING-005 | Informational | 1.2.4 | — | — |
| FINDING-006 | Informational | 1.3.10 | — | superset-extensions-cli/src/superset_extensions_cli/cli.py, superset-extensions-cli/src/superset_extensions_cli/utils.py |
| FINDING-007 | Informational | 1.3.12 | — | superset-extensions-cli/src/superset_extensions_cli/cli.py, superset-extensions-cli/src/superset_extensions_cli/utils.py |
| FINDING-008 | Informational | 1.3.2 | — | superset-extensions-cli/src/superset_extensions_cli/cli.py |
| FINDING-009 | Informational | 1.3.3 | — | superset-extensions-cli/src/superset_extensions_cli/utils.py |
| FINDING-010 | Informational | 1.3.7 | — | superset-extensions-cli/src/superset_extensions_cli/cli.py |
| FINDING-011 | Informational | 1.4.1 | — | superset-extensions-cli/src/superset_extensions_cli/cli.py |
| FINDING-012 | Informational | 1.4.2 | — | superset-extensions-cli/src/superset_extensions_cli/cli.py |
| FINDING-013 | Informational | 1.4.3 | — | superset-extensions-cli/src/superset_extensions_cli/cli.py |

**Total Unique Findings**: 13 (0 Critical, 0 High, 0 Medium, 1 Low, 12 Info)

*1 of 13 are actionable. Informational findings are recorded here but not opened as GitHub issues; see issues.md for the 1 actionable items.*
