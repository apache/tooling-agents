# Security Audit Consolidated Report — apache/superset/superset-core

## Report Metadata

| Field | Value |
|-------|-------|
| Repository | apache/superset/superset-core |
| ASVS Level | L3 |
...
This audit was conducted against ASVS Level 3 (L3), the most rigorous verification tier, covering authentication, session management, cryptography, API boundaries, transport security, and logging across the audited domains. All 19 findings are informational, primarily documenting requirements that are not applicable because the audited `superset-core` code delegates security-sensitive behavior to Flask-AppBuilder and deployment infrastructure rather than re-implementing it.

### Top 5 Risks

1. **Timestamp fields typed as bare datetime without explicit timezone awareness** [Info] — `datetime` fields lack explicit timezone awareness, which could lead to ambiguous or inconsistent log/event timestamps if not normalized by consumers.
2. **stack_trace/error_message fields declared in Task.properties** [Info] — Sensitive-data foot-gun where `stack_trace`/`error_message` fields in `Task.properties` could capture detailed error content that downstream handlers must avoid persisting or exposing.
3. **Abstract stubs raise NotImplementedError without logging** [Info] — Abstract method stubs raise `NotImplementedError` without emitting log records, leaving error/log handling to out-of-scope concrete implementations.
4. **No log-emission code present in audited files** [Info] — The audited files contain no log-emission code, so logging-related verification depends entirely on the host application and deployment stack.
5. **Result-object lifecycle observation** [Info] — Requirement largely not applicable, with a minor observation noted on the lifecycle handling of result objects.

### Positive Controls Observed

- **Secure-by-default authentication posture:** MCP `@tool`/prompt decorators default to `protect=True`, ensuring authentication is on by default.
- **Fail-closed decorator placeholders:** Decorator stubs raise `NotImplementedError` rather than registering unvalidated routes, preventing endpoint exposure without the host's validation layer.
- **Consistent delegation to Flask-AppBuilder:** Session lifecycle, password policy, MFA/credential handling, CSRF protection, and concurrent-session management are all delegated to FAB rather than re-implemented, avoiding divergent or undocumented authentication pathways.
- **No XML parsing present:** Full-text review confirmed the absence of any XML parser (`lxml`, `xml.etree`, `xml.sax`, `xml.dom`, `xmltodict`), eliminating XXE exposure in the query execution engine.
- **JSON-only, type-safe serialization:** Metadata fields use JSON serialization and query results use structured dataclasses/pandas DataFrames, with no `pickle`/`yaml.load`/`eval`-based reconstruction in scope.
- **No JSONP support:** No callback-parameter handling or `text/javascript` response construction found in `api.py`.
- **Framework-delegated transport/header hardening:** The library carries no hardcoded transport/browser-header defaults, and Content-Type is set correctly via FAB/Flask `jsonify`, leaving header policy to the reverse proxy.
- **No ad-hoc session/token primitives:** No static API secrets used as session identifiers and no weak token-generation routines introduced; UUID fields serve as persistence identifiers only.
- **Centralized SQL dialect determination:** A single `get_sqlglot_dialect()` entry point ensures consistent dialect handling, with `StatementResult` retaining both `original_sql` and `executed_sql` for verification.
- **RBAC binding surface:** `class_permission_name`/`method_permission_name` allow trusted authors to bind tools to the host's canonical RBAC model with fine-grained authorization.
...
#### FINDING-001: No XML parsing present — requirement not applicable
...
| **CWE** | N/A |
| **ASVS sections** | 1.5.1 |
| **Files** | common/daos.py, common/models.py, queries/daos.py, queries/models.py, queries/query.py, queries/types.py |
| **Source Reports** | 1.5.1.md |
| **Related** | None |

**Description:**

A full text review of every file shows no XML parser instantiation, no lxml, xml.etree, xml.sax, xml.dom, xmltodict, or any external-entity-bearing parser. Serialized fields (params, extra, json_metadata, position_json, query_context) are JSON strings, not XML. Result payloads are pandas DataFrame objects. ASVS 1.5.1 targets XML-parsing systems; this code set parses no XML, so the requirement is Not Applicable. If a host implementation introduces XML parsing, it must be audited separately.

**Remediation:**

None required. If any host implementation introduces XML parsing of query metadata or results, audit it against 1.5.1 (disable external entities, DTDs).

---

#### FINDING-002: OAuth/OIDC token distribution (BFF pattern) not applicable - no token-handling logic in audited code
...
| **Files** | superset-core/src/superset_core/** |
| **Source Reports** | 10.1.1.md |
| **Related** | None |

**Description:**

ASVS 10.1.1 targets OAuth/OIDC token distribution (BFF pattern). Audited superset-core contains no OAuth token storage, forwarding, cookie/session writing, or client-side token exposure. Token-handling surfaces (embedded SDK, websocket sidecar) are in other packages not included in this audit. No source → no sink.

**Remediation:**

No action against superset-core. Audit token-distribution against the Flask backend (guest-token endpoint) and the embedded SDK.

---

#### FINDING-003: OAuth/OIDC client session binding not applicable - no authorization flow in audited code
...
ASVS 10.1.2 applies to OAuth/OIDC clients binding authz codes/ID tokens to a user-agent session via PKCE/state/nonce. Audited code contains no authorization-flow initiation, no state/nonce/code_verifier generation, and no redirect/callback handling. OAuth/OIDC protocol enforcement is delegated to Flask-AppBuilder's AUTH_TYPE managers, not in audit set.

**Remediation:**

No action against superset-core. Supply the Flask-AppBuilder integration and configured auth manager to audit OAuth/OIDC client behavior.

---

#### FINDING-004: Claim-based authorization decisions not applicable - no token claim parsing in audited code
...
| **ASVS Level(s)** | L2 |
| **CWE** | N/A |
| **ASVS sections** | 10.3.2, 10.3.3 |
| **Files** | superset-core/src/superset_core/** |
| **Source Reports** | 10.3.2.md, 10.3.3.md |
| **Related** | None |

**Description:**

ASVS 10.3.2 requires authorization decisions from token claims (sub, scope). Audited code contains no access-token parsing and no claim-driven authorization decision logic. DAO interfaces are data-access abstractions; permission enforcement is performed at the API layer, not in scope.

**Remediation:**

No action against superset-core. Verify claim-based authorization in the host implementation of the API/tool decorators and the WebSocket sidecar channel-scoping logic.

---

#### FINDING-005: Authentication strength constraints not applicable - no resource server in audited code
...
ASVS 10.3.4 applies to a resource server enforcing authentication-strength constraints (acr/amr/auth_time). Audited apache-superset-core contains no token-receiving endpoint, JWT parsing, claim inspection, or authorization decision logic. Guest-token and sidecar JWT handling live in other components not included.

**Remediation:**

No action against superset-core. Audit acr/amr/auth_time enforcement against the Flask backend's guest-token validation path and the websocket sidecar JWT validation.

---

#### FINDING-006: Sender-constrained tokens not applicable - no token-consuming resource server present
...
ASVS 10.3.5 (L3) requires sender-constrained tokens (mTLS/DPoP). Audited code has no token receipt/parsing, no TLS client-certificate handling (transport delegated to reverse proxy), no DPoP proof validation, no cnf/jkt handling. Token-validation surfaces live in other components.

**Remediation:**

No action against superset-core. Evaluate sender-constrained tokens against the token-issuing/consuming components, not the interface package.

---

#### FINDING-007: Self-contained token signature validation not applicable - no token validation in audited code
...
| **ASVS Level(s)** | L1, L2 |
| **CWE** | N/A |
| **ASVS sections** | 9.1.1, 10.3.1, 9.1.2, 9.1.3, 9.2.1, 9.2.2, 9.2.3 |
| **Files** | superset-core/src/superset_core/** |
| **Source Reports** | 9.1.1.md, 10.3.1.md, 9.1.2.md, 9.1.3.md, 9.2.1.md, 9.2.2.md, 9.2.3.md |
| **Related** | None |

**Description:**

ASVS 9.1.1 requires that self-contained tokens be validated via digital signature or MAC before their contents are trusted. The audited code set (superset-core) contains no token ingestion or validation at all. superset-core consists exclusively of abstract interfaces and NotImplementedError stubs replaced at runtime by host implementations. No code path accepts, decodes, or acts on a JWT/PASETO/signed-cookie payload. Token validation lives in superset/, superset-embedded-sdk/, and superset-websocket/, which are not in scope here.

**Remediation:**

None within this code set. Verify the sidecar passes an explicit algorithms allowlist (e.g. ['HS256']) to jwt.verify and that the Flask backend pins PyJWT's algorithms argument — in the appropriate source files, which must be supplied.

---

#### FINDING-008: Token issuer audience restriction not applicable - no token issuance in audited code
...
ASVS 9.2.4 applies only to components that issue self-contained tokens. The audited superset-core package contains no token-issuance code path: no signing key handling, no JWT construction, no aud claim assignment, and no audience-provisioning logic. The components that actually issue tokens — the Flask backend (guest tokens) and WebSocket sidecar (session tokens) — are not part of this audit set.

**Remediation:**

No action against superset-core. To audit 9.2.4 properly, supply the token-minting code in the Flask backend and the WebSocket sidecar's token issuer, then verify that tokens issued under a shared signing key carry an aud claim distinguishing guest-token from sidecar-session context to prevent cross-context replay.

---

#### FINDING-009: CORS Preflight Enforcement — not in scope (no CORS logic present)
...
| **CWE** | N/A |
| **ASVS sections** | 3.5.2, 3.5.3, 3.5.4, 3.5.5, 3.5.6, 3.5.7, 3.5.8, 5.1.1, 5.2.1, 5.2.2, 5.2.3, 5.2.4, 5.2.5, 5.2.6, 5.3.1, 5.3.2, 5.3.3, 5.4.1, 5.4.2, 5.4.3 |
| **Files** | superset-core/src/superset_core/rest_api/api.py |
| **Source Reports** | 3.5.2.md, 3.5.3.md, 3.5.4.md, 3.5.5.md, 3.5.6.md, 3.5.7.md, 3.5.8.md, 5.1.1.md, 5.2.1.md, 5.2.2.md, 5.2.3.md, 5.2.4.md, 5.2.5.md, 5.2.6.md, 5.3.1.md, 5.3.2.md, 5.3.3.md, 5.4.1.md, 5.4.2.md, 5.4.3.md |
| **Related** | None |

**Description:**

The audited file (superset-core/src/superset_core/rest_api/api.py) is a base REST API class that contains no file upload, download, storage, or processing functionality. All ASVS 5.x file upload security requirements (5.1.1 through 5.4.3) are not applicable to this code scope. The file has no request handlers accepting files, no multipart parsing, no file storage operations, no archive extraction, no image processing, no file serving endpoints, and no Content-Disposition header construction. To properly audit file upload security controls, the scope must be expanded to include actual upload-handling modules, storage logic, import/export endpoints, and image processing components within the Superset application.

**Remediation:**

Re-scope the audit to include: (1) Upload-handling modules with file acceptance endpoints; (2) CSV/Excel/image/database driver upload handlers; (3) Import-bundle and archive decompression modules; (4) Storage modules with per-user quota tracking; (5) Download/export endpoints that serve files; (6) Image processing components. For each component, verify the relevant ASVS controls: size limits, extension/content validation, decompression safeguards, per-user quotas, symlink rejection, pixel flood protection, execution prevention, path traversal protection, zip-slip prevention, Content-Disposition handling, filename encoding, and antivirus scanning.

---

#### FINDING-010: No logging inventory artifact present in audited files
...
| **Files** | rest_api/api.py, tasks/models.py |
| **Source Reports** | 16.1.1.md |
| **Related** | None |

**Description:**

ASVS 16.1.1 requires an inventory documenting what is logged at each layer. This is a documentation/process artifact, not a code control. The audited files neither implement logging nor are the place a logging inventory would reside. No exploit path exists in the audited code.

**Remediation:**

Maintain the logging inventory at the project/operations documentation layer enumerating per-layer events, format, destination, access controls, and retention. No code change to these files is warranted.

---

#### FINDING-011: No log-emission code present in audited files
...
| **ASVS Level(s)** | L2 |
| **CWE** | N/A |
| **ASVS sections** | 16.2.1, 16.2.3, 16.2.4, 16.4.1, 16.4.3 |
| **Files** | rest_api/api.py, tasks/models.py |
| **Source Reports** | 16.2.1.md, 16.2.3.md, 16.2.4.md, 16.4.1.md, 16.4.3.md |
| **Related** | None |

**Description:**

The audited files contain no log-emission code; RestApi only toggles browser login and Task/TaskSubscriber are abstract stubs. There is no log entry to evaluate for metadata completeness, formatting, encoding, or destination control.

**Remediation:**

Evaluate log-entry metadata completeness, destination allow-listing, structured formatting (e.g., JSON), encoding/neutralization against log injection, and secure transmission to separate analysis systems in host logging implementations where logging actually occurs.

---

#### FINDING-012: Timestamp fields typed as bare datetime without explicit timezone awareness
...
| **Files** | superset-core/src/superset_core/tasks/models.py |
| **Source Reports** | 16.2.2.md |
| **Related** | None |

**Description:**

Timestamp fields are typed as bare datetime in abstract interface stubs, with concrete timezone behavior determined by AuditMixinNullable in the host. No attacker capability and no direct C/I/A impact; purely a forensic-hygiene concern. If host implementations populate naive local-time values, timeline reconstruction could be ambiguous.

**Remediation:**

Ensure host implementations of AuditMixinNullable-derived fields store timezone-aware UTC datetimes and document this in the model contract. Time-source synchronization (NTP/chrony) remains a deployment responsibility.

---

#### FINDING-013: stack_trace/error_message fields declared in Task.properties (sensitive-data foot-gun)
...
| **ASVS Level(s)** | L2 |
| **CWE** | N/A |
| **ASVS sections** | 16.2.5, 16.5.1 |
| **Files** | superset-core/src/superset_core/tasks/models.py |
| **Source Reports** | 16.2.5.md, 16.5.1.md |
| **Related** | None |

**Description:**

Task.properties documents stack_trace, error_message, and exception_type fields. These are abstract declarations; this file performs no logging and no client serialization. No reachable attacker-controlled path exists within these files. Flagged as a foot-gun to be governed where the field is consumed by host implementations to prevent exposure of sensitive internal system data.

**Remediation:**

At host implementation/worker/error-handling layers consuming Task.properties, return only generic error messages to end users, confine stack traces to protected server-side logs, apply classification-based redaction, and document the fields as sensitive. In host implementations, apply a field allow-list (schema.dump() with explicit columns) omitting stack_trace/exception_type for non-admin/external callers.

---

#### FINDING-014: No authentication operations implemented in audited files
...
| **ASVS Level(s)** | L2, L3 |
| **CWE** | N/A |
| **ASVS sections** | 16.3.1, 16.3.2, 16.3.3, 16.4.2, 16.5.2, 16.5.4 |
| **Files** | superset-core/src/superset_core/rest_api/api.py, superset-core/src/superset_core/tasks/models.py |
| **Source Reports** | 16.3.1.md, 16.3.2.md, 16.3.3.md, 16.4.2.md, 16.5.2.md, 16.5.4.md |
| **Related** | None |

**Description:**

The audited files define no application entry point, WSGI app, or error-handler registration. The last-resort handler, if present, lives in the host application factory at the Flask/FAB layer, consistent with documented delegation of framework-level controls. The targeted pattern is absent.

**Remediation:**

Confirm failed authorization decisions are logged at the API enforcement boundary. For L3 compliance, ensure all authorization decisions and sensitive data access are logged.

---

#### FINDING-015: Abstract stubs raise NotImplementedError without logging; concrete error/log handling out of scope
...
NotImplementedError statements are abstract-interface placeholders replaced by host implementations; they are not runtime error paths in production. Requirement 16.3.4's intent is satisfied (or not) by concrete host implementations and the last-resort handler, outside scope. The error-capture fields (error_message/exception_type/stack_trace) support 16.3.4 at the layer where it applies.

**Remediation:**

Confirm host implementations log unexpected errors and security-control failures, confining stack_trace to server-side logs.

---

#### FINDING-016: Abstract methods fail closed via NotImplementedError (no fail-open)
...
The abstract interface methods all fail closed by raising NotImplementedError rather than returning a permissive default. There is no validation-bypass or transaction-processing-despite-error logic. This requirement is satisfied for the code present.

**Remediation:**

No change required; maintain fail-closed abstract method behavior.

---

#### FINDING-017: Requirement largely not applicable; minor observation on result-object lifecycle
...
| **Files** | superset-core/src/superset_core/queries/types.py, superset-core/src/superset_core/tasks/models.py |
| **Source Reports** | 11.7.2.md |
| **Related** | None |

**Description:**

ASVS 11.7.2 (Level 3) concerns minimizing the amount of decrypted/sensitive data exposed during processing and re-encrypting it as soon as feasible. The audited package is a set of type definitions and abstract interfaces; it performs no decryption, holds no secrets, and implements no data-handling logic whose minimization could be evaluated. The closest constructs are data-carrying dataclasses (StatementResult.data, QueryResult.statements, Task.payload). These are contracts, not implementations. The actual lifecycle is governed by the host implementations that replace these stubs. No minimization decision is made in scope, so there is no demonstrable defect to report. Recorded as informational: the host implementations that materialize QueryResult/Task.payload are where data-minimization and prompt-clearing of decrypted/sensitive content should be enforced and audited.

**Remediation:**

Ensure the host implementations that replace QueryResult/Task.payload stubs apply data-minimization and bounded-lifetime handling of any decrypted or sensitive data they materialize; capture this expectation in the contract docstrings of queries/types.py and tasks/models.py.

---

#### FINDING-018: OAuth Client Requirements Not Applicable - No Client Implementation Present
...
| **ASVS Level(s)** | L2, L3 |
| **CWE** | N/A |
| **ASVS sections** | 10.2.1, 10.2.2, 10.2.3, 10.5.1 |
| **Files** | superset-core package (all files) |
| **Source Reports** | 10.2.1.md, 10.2.2.md, 10.2.3.md, 10.5.1.md |
| **Related** | FINDING-019 |

**Description:**

ASVS sections 10.2.1 (CSRF protection via PKCE/state), 10.2.2 (mix-up attack defense via iss validation), and 10.2.3 (minimal scope requests) target OAuth client implementations. The audited superset-core package contains no OAuth client code: no authorization request construction, token endpoint calls, state handling, PKCE logic, authorization server interaction, iss handling, response parsing, or scope request logic. OAuth/OIDC integration is delegated to Flask-AppBuilder's AUTH_TYPE manager, which is outside the scope of this audit.

**Remediation:**

None required for this package. Verify PKCE/state handling, iss validation (if multiple IdPs are configured), and minimal-scope requests in the Flask-AppBuilder authentication manager configuration.

---

#### FINDING-019: OAuth Authorization Server Requirements Not Applicable - Code Acts as Client, Not Server
...
| **ASVS Level(s)** | L1, L3 |
| **CWE** | N/A |
| **ASVS sections** | 10.4.1, 10.4.2, 10.4.3, 10.4.16 |
| **Files** | superset-core package (all files) |
| **Source Reports** | 10.4.1.md, 10.4.2.md, 10.4.3.md, 10.4.16.md |
| **Related** | FINDING-018 |

**Description:**

ASVS sections 10.4.1 (redirect URI validation), 10.4.2 (authorization code single-use), 10.4.3 (short-lived authorization codes), and 10.4.16 (strong client authentication) apply to OAuth authorization servers. Superset acts as an OAuth client/relying party, not an authorization server. The audited superset-core package contains no authorization server functionality: no redirect URI validation, client registration store, authorization endpoint, authorization code issuance, token endpoint, code-to-token exchange, or client authentication mechanisms.

**Remediation:**

No action required. If a future audit covers OAuth client/OIDC RP behavior, supply the relevant Flask-AppBuilder integration/security-manager source code.
...
|--------|---------|-----------------|------------------|
| Flask Rest Api Boundary | Extension/host API registration surface is treated as a trusted, deployment-equivalent input source; decorator metadata is not adversary-controlled at runtime. | Promoted from dropped finding ASVS-211-INFO-001 | — |
| Flask Rest Api Boundary | Decorator placeholder fails closed (NotImplementedError) rather than registering an unvalidated route, ensuring no endpoint is exposed without the host's validation layer. | Promoted from dropped finding ASVS-221-INFO-001 | — |
| Flask Rest Api Boundary | Responses inherit FAB/Flask jsonify Content-Type (application/json) rather than hand-rolled responses; header hardening delegated to reverse proxy | Content-Type is set correctly by FAB/Flask jsonify (application/json); broader header policy (Content-Type/X-Content-Type-Options) is delegated to the reverse proxy per deployment_infrastructure_delegated.md (security headers) | — |
| Flask Rest Api Boundary | Application implements no ad-hoc HTTP→HTTPS redirect; redirect policy centralized at the proxy layer. | source: Dropped finding ASVS-412-INFO-001 | — |
| Flask Rest Api Boundary | Audited code introduces no header-derived trust decisions; proxy-header trust handled in deployment/WSGI config. | source: Dropped finding ASVS-413-INFO-001 | — |
| Flask Rest Api Boundary | Per-route HTTP method enumeration via FAB @expose(methods=...); no wildcard handlers. | Dropped finding ASVS-414-INFO-001 | — |
| Flask Rest Api Boundary | Response generation uses framework helper (self.response()), avoiding manual Content-Length framing mismatch. | Response length framing handled by Werkzeug via FAB self.response(); no manual Content-Length computation in scope. Transport/framing delegated to deployment stack per deployment_infrastructure_delegated.md. | — |
| Query Execution Engine | No XML parsing libraries in use | Full text review confirmed absence of lxml, xml.etree, xml.sax, xml.dom, xmltodict, or any XML parser instantiation | common/daos.py, common/models.py, queries/daos.py, queries/models.py, queries/query.py, queries/types.py |
| Query Execution Engine | JSON-only serialization for metadata fields | Serialized fields (params, extra, json_metadata, position_json, query_context) use JSON format, not XML | — |
| Query Execution Engine | Query results returned as structured dataclasses and pandas DataFrames | No pickle/yaml.load/marshal/jsonpickle/eval reconstruction appears in scope; results use type-safe structured objects | — |
| Query Execution Engine | Centralized SQL dialect determination | Single get_sqlglot_dialect() entry point centralizes dialect determination, ensuring consistent SQL-parsing dialect rather than ad-hoc per-call detection. StatementResult retains both original_sql and executed_sql for verification | — |
| Flask Appbuilder Rbac | MCP tool/prompt decorators default to protect=True (authentication on by default) | Promoted from dropped finding ASVS-821-INFO-001 | — |
| Flask Appbuilder Rbac | RBAC resource/action binding (class_permission_name/method_permission_name) is exposed for trusted authors to bind tools to the host's canonical RBAC model | Promoted from dropped finding ASVS-821-INFO-001 | — |
| Jwt Authentication Tokens | Token expiration for guest tokens and sidecar sessions is enforced in the out-of-scope backend verifier and sidecar JWT library defaults | Mentioned as implemented outside the audited code scope | — |
| Session Management | Session lifecycle delegated to Flask-AppBuilder; no parallel/diverging timeout implementation | superset-core does not re-implement session timeout logic; delegates to FAB | — |
| Session Management | No ad-hoc concurrent-session mechanism that could diverge from FAB's policy | Concurrent session handling delegated to Flask-AppBuilder | — |
| Session Management | WebSocket JWT session mechanism segregated from Flask cookie sessions | Intentional separation prevents cross-mechanism session confusion | — |
| Session Management | MCP @tool decorator defaults protect=True (secure-by-default) | RestApi centralizes browser-login enablement rather than scattering custom auth paths | — |
| Session Management | No static API secrets used as session identifiers | Dynamic session token generation throughout package | — |
| Session Management | No weak or predictable token-generation routine introduced | UUID identifier fields are persistence identifiers, not session tokens | — |
| Session Management | RestApi cleanly extends FAB's BaseApi | Inherits FAB session handling rather than circumventing it | — |
| Session Management | Query/task timeout primitives correctly scoped to resource lifecycles | Timeouts do not entangle with session validity | — |
| Session Management | No code attempts to extend or weaken session lifetime contrary to FAB's policy | Audit timestamps are domain-object lifecycle, not session age | — |
| Session Management | Defers to FAB's stateful session model | No self-contained-token mechanism requiring separate revocation list | — |
| Session Management | User model carries explicit active flag in interface contract | Clear state for host implementations to base session-termination behavior | — |
| Session Management | No parallel credential-management surface that could diverge from FAB | Password/MFA handling delegated to Flask-AppBuilder | — |
| Session Management | All session/auth UI concerns deferred to host framework | No re-implementation of logout or session management UI | — |
| Session Management | Session store and admin session-management owned by FAB | No counterfeit or no-op session-termination function; cleanly delegated upstream | — |
| Session Management | User model exposes only metadata type hints and no concrete attribute-mutation surface | Authentication-sensitive changes left to upstream framework | — |
| Session Management | Session enumeration/self-service revocation owned by FAB | No misleading partial implementation; session-management surface cleanly delegated | — |
| Session Management | @tool decorator supports RBAC permission metadata for host-layer enforcement | Defaults protect=True and enables fine-grained authorization | — |
| Session Management | No partial or insecure federation implementation | Protocol-level federation enforcement correctly left to dedicated upstream auth managers | — |
| Password Authentication | superset-core introduces no parallel/unprotected authentication path; auth and anti-automation delegated to FAB and deployment infrastructure | Dropped finding ASVS-611-INFO-001 | — |
| Password Authentication | Password policy delegated to Flask-AppBuilder | flask_appbuilder_security_controls.md (ASVS V2 out of scope); no password validator in superset-core | — |
| Password Authentication | MCP tool/prompt decorator stubs default to protect=True (authenticated-by-default posture) | Dropped finding ASVS-613-INFO-001 | — |
| Password Authentication | Password-strength/banned-word validation delegated to Flask-AppBuilder | flask_appbuilder_security_controls.md (ASVS V2 out of scope); no password validation pipeline in superset-core | — |
| Password Authentication | Authentication-pathway logic is uniformly delegated to a single upstream framework (FAB) rather than re-implemented per module, avoiding undocumented pathways. | source: Dropped finding ASVS-634-INFO-001 | — |
| Password Authentication | Abstract `User` interface exposes a minimal field set with no knowledge-based-authentication/secret-question fields, preventing extensions from being steered toward implementing secret questions. | source: Dropped finding ASVS-642-INFO-001 | — |
| Password Authentication | UserDAO interface exposes no method that sets or returns a user's password, so the SDK provides no primitive an admin path could misuse to learn a user's password | Promoted from dropped finding ASVS-646-INFO-001 | — |
| Mfa And Cryptographic Auth | MFA/authenticator one-time-use enforcement delegated to Flask-AppBuilder / configured IdP; superset-core is interface-only with no authenticator surface | Dropped finding ASVS-651-INFO-001 | — |
| Mfa And Cryptographic Auth | Lookup-secret/credential storage and hashing delegated to FAB; abstract User model exposes no credential fields | Credential/secret storage delegated to Flask-AppBuilder per flask_appbuilder_security_controls.md (ASVS V2 out of scope); no secret-at-rest storage in superset-core | — |
| Mfa And Cryptographic Auth | Authenticator seed/secret generation delegated to Flask-AppBuilder/IdP | Per flask_appbuilder_security_controls.md - ASVS V2 out of scope, no RNG/secret generation in superset-core | — |
| Mfa And Cryptographic Auth | Authenticator code generation delegated to Flask-AppBuilder/IdP | per flask_appbuilder_security_controls.md (ASVS V2 out of scope); no code construction in superset-core | — |
| Mfa And Cryptographic Auth | Authenticator factor lifecycle/revocation delegated to Flask-AppBuilder/IdP | per flask_appbuilder_security_controls.md (ASVS V2 out of scope); no factor-management surface in superset-core | — |
| Mfa And Cryptographic Auth | Authentication factor handling delegated to Flask-AppBuilder/IdP | No biometric/factor logic in superset-core; authentication delegated per flask_appbuilder_security_controls.md | — |
| Browser Security Headers | Library carries no hardcoded transport/browser-header defaults, leaving deployment layer full authority to harden per environment. | source: Dropped finding ASVS-311-INFO-001 | — |
| Browser Security Headers | Cookie emission delegated to FAB/Flask session layer and deployment proxy | flask_appbuilder_security_controls.md and deployment_infrastructure_delegated.md; no cookie sink in superset-core | — |
| Browser Security Headers | CORS/Origin allowlisting enforced at host Flask app and reverse proxy | No CORS handling in superset-core; enforcement delegated per deployment_infrastructure_delegated.md and frontend_backend_enforcement_boundary.md | — |
| Csrf And Cors Protection | CSRF protection delegated to Flask-AppBuilder @protect() rather than re-implemented, with allow_browser_login centralized in one auditable base class | Dropped finding ASVS-351-INFO-001 | — |
| Csrf And Cors Protection | No JSONP support present in codebase | No callback-parameter handling or text/javascript response construction found in api.py | superset-core/src/superset_core/rest_api/api.py |
| Websocket Security | WSS termination delegated to TLS-terminating reverse proxy; sidecar speaks plain ws:// behind it by documented design. | source: Dropped finding ASVS-441-INFO-001 | — |
| Websocket Security | JWT validation is the primary WebSocket access control; Origin allow-listing is delegated to the reverse proxy as secondary hardening. | source: Dropped finding ASVS-442-INFO-001 | — |
| Tls And Transport Security | Package hardcodes no TLS protocol version and does not undermine the deployment-layer TLS floor. | No hardcoded TLS version constraints found in codebase | — |
| Tls And Transport Security | No weak cipher string is hardcoded; package does not override deployment-layer cipher policy | No cipher suite configuration present in package code | — |
| Tls And Transport Security | Package accepts no certificate-derived identities, so cannot trust an unvalidated client certificate. | No mTLS client certificate handling logic present | — |
| Tls And Transport Security | Package does not disable or weaken revocation-checking behavior | No OCSP or revocation checking configuration present | — |
| Tls And Transport Security | No SNI-exposing TLS behavior configured in the package; handled at the TLS terminator. | No ECH or SNI configuration present; delegated to infrastructure | — |
| Tls And Transport Security | No abstract method forces or implies unencrypted transport; transport selection left entirely to host implementation | Package does not enforce transport protocol selection | — |
| Tls And Transport Security | Package contains no native TLS implementation, avoiding a parallel divergent TLS stack, consistent with documented delegation. | No custom TLS stack implementation found | — |
| Tls And Transport Security | QueryOptions separates query intent from transport concerns, leaving connection-security configuration to the driver-owning layer | Transport security delegated to underlying driver layer | — |
| Tls And Transport Security | No verify=False / disabled-validation anti-pattern present anywhere in the package | No certificate validation bypass patterns detected | — |
| Tls And Transport Security | No insecure internal HTTP fallback is hardcoded; the library opens no internal HTTP connections. | No HTTP fallback mechanisms present | — |
| Tls And Transport Security | Library introduces no permissive 'trust any CA' knob, so cannot weaken a deployment-configured internal trust posture. | No trust store override or permissive CA trust configuration present | — |
| Tls And Transport Security | WebSocket sidecar uses JWT session tokens validated at connection time plus network segmentation as a documented compensating intra-service posture | JWT-based authentication for internal WebSocket communications with network segmentation | — |
| Secrets And Key Management | No hardcoded secrets anywhere in the package; secret-bearing models are abstract type contracts with no values | Dropped finding ASVS-1331-INFO-001 | — |
| Service Communication Security | Task framework exposes timeout + on_abort cleanup hooks giving authors a documented resource-release path | Stub exposes timeout/on_abort contract | — |
| Service Communication Security | Bounded task execution via @task(timeout=...) plus overridable per-invocation TaskOptions(timeout=...) provides a documented resource-control affordance | Report 13.2.6 positive observation | — |
| Deployment Hardening | Build packaging scopes discovery to src/, so repository metadata is not shipped in wheel/sdist | Promoted from dropped finding ASVS-1341-INFO-001 | — |
| Deployment Hardening | NotImplementedError guards use static, state-free messages that leak no runtime/environment data | Promoted from dropped finding ASVS-1342-INFO-001 | — |
| Deployment Hardening | MCP tool()/prompt() decorators default protect=True (auth-by-default, opt-out rather than opt-in) | Promoted from dropped finding ASVS-1345-INFO-001 | — |
| Dependency And Component Management | Third-party dependency remediation timing governed by the ASF coordinated vulnerability process (documented response policy) | Documented in third_party_dependency_policy.md and SECURITY.md | third_party_dependency_policy.md, SECURITY.md |
| Dependency And Component Management | MCP tools/prompts are authenticated-by-default (protect=True); disabling auth is an explicit trusted-author opt-in | Default secure configuration prevents exposure of extraneous functionality without explicit opt-in | — |
| Dependency And Component Management | Dangerous operations (SQL execution, template rendering, code registration) are encapsulated behind abstract contracts; concrete risky logic lives host-side where isolation/RBAC controls apply | Architectural separation and encapsulation of risky components with host-side security controls | — |
| Security Logging And Monitoring | Abstract methods fail closed by raising NotImplementedError | No validation-bypass or transaction-processing-despite-error logic present | superset-core/src/superset_core/tasks/models.py |
| Cryptographic Implementation | superset-core interface package embeds no keys, secrets, or hardcoded cryptographic constants; cryptographic concerns are kept out of the public extension API via dependency injection | Dropped finding ASVS-1111-INFO-001 | — |
| Cryptographic Implementation | No custom/ad-hoc cryptographic routines are implemented in superset-core; cryptography is delegated to the host application's industry-validated libraries. | source: Dropped finding ASVS-1121-INFO-001 | — |
| Cryptographic Implementation | The public User interface deliberately exposes no password/credential attribute; password storage and verification stay within Flask-AppBuilder. | Password storage/hashing is delegated to Flask-AppBuilder per flask_appbuilder_security_controls.md; the abstract User model exposes no password field. | — |
| Cryptographic Implementation | Transport-layer key exchange (TLS) is delegated to the reverse-proxy/deployment infrastructure; no key-exchange logic exists in application code. | No key-exchange logic in this interface package; transport-layer key exchange (TLS) delegated to deployment infrastructure per deployment_infrastructure_delegated.md (ASVS V9) | — |
| Oauth Oidc Client | OIDC sub-claim-to-user mapping delegated to Flask-AppBuilder AUTH_TYPE manager | superset-core contains no claim-mapping code; ASVS 10.5.2 compliance handled by Flask-AppBuilder | — |
| Oauth Oidc Client | OIDC authorization-server metadata issuer validation delegated to Flask-AppBuilder | superset-core does not implement authorization server metadata validation; ASVS 10.5.3 compliance handled by Flask-AppBuilder | — |
| Oauth Oidc Client | OIDC ID Token audience validation delegated to Flask-AppBuilder | superset-core does not implement ID Token aud claim validation; ASVS 10.5.4 compliance handled by Flask-AppBuilder | — |
| Oauth Oidc Client | OIDC back-channel logout token validation delegated to Flask-AppBuilder auth layer | superset-core does not implement back-channel logout; ASVS 10.5.5 compliance handled by Flask-AppBuilder | — |
| Oauth Oidc Client | OAuth/OIDC authorization-server consent prompting delegated to upstream IdP | Superset acts as OAuth/OIDC relying party only; ASVS 10.7.1 consent enforcement is responsibility of external authorization server | — |
| Oauth Oidc Client | Consent-information display rendered by upstream authorization server | Superset does not implement a consent screen; ASVS 10.7.2 compliance handled by external IdP | — |
| Oauth Oidc Client | Consent review/modify/revoke handled by external IdP and Flask-AppBuilder integration | superset-core maintains no OAuth consent records; ASVS 10.7.3 compliance delegated to external systems | — |
...
| 1.1.2 | Verify that the application performs output encoding and escaping either as a final step before being used by the interpreter for which it is intended or by the interpreter itself. | **N/A** |  |
...
| 1.5.1 | Verify that the application configures XML parsers to use a restrictive configuration and that unsafe features such as resolving external entities are disabled to prevent XML eXternal Entity (XXE) attacks. | **N/A** | See FINDING-001 |
...
| 3.5.2 | Verify that, if the application relies on the CORS preflight mechanism to prevent disallowed cross-origin use of sensitive functionality, it is not possible to call the functionality with a request which does not trigger a CORS-preflight request. This may require checking the values of the 'Origin' and 'Content-Type' request header fields or using an extra header field that is not a CORS-safelisted header-field. | **N/A** | See FINDING-009 |
| 3.5.3 | Verify that HTTP requests to sensitive functionality use appropriate HTTP methods such as POST, PUT, PATCH, or DELETE, and not methods defined by the HTTP specification as "safe" such as HEAD, OPTIONS, or GET. Alternatively, strict validation of the Sec-Fetch-* request header fields can be used to ensure that the request did not originate from an inappropriate cross-origin call, a navigation request, or a resource load (such as an image source) where this is not expected. | **N/A** | See FINDING-009 |
| 3.5.4 | Verify that separate applications are hosted on different hostnames to leverage the restrictions provided by same-origin policy, including how documents or scripts loaded by one origin can interact with resources from another origin and hostname-based restrictions on cookies. | **N/A** | See FINDING-009 |
| 3.5.5 | Verify that messages received by the postMessage interface are discarded if the origin of the message is not trusted, or if the syntax of the message is invalid. | **N/A** | See FINDING-009 |
| 3.5.6 | Verify that JSONP functionality is not enabled anywhere across the application to avoid Cross-Site Script Inclusion (XSSI) attacks. | **N/A** | See FINDING-009 |
| 3.5.7 | Verify that data requiring authorization is not included in script resource responses, like JavaScript files, to prevent Cross-Site Script Inclusion (XSSI) attacks. | **N/A** | See FINDING-009 |
| 3.5.8 | Verify that authenticated resources (such as images, videos, scripts, and other documents) can be loaded or embedded on behalf of the user only when intended. This can be accomplished by strict validation of the Sec-Fetch-* HTTP request header fields to ensure that the request did not originate from an inappropriate cross-origin call, or by setting a restrictive Cross-Origin-Resource-Policy HTTP response header field to instruct the browser to block returned content. | **N/A** | See FINDING-009 |
...
| **V5: File Handling** | | | |
| 5.1.1 | Verify that the documentation defines the permitted file types, expected file extensions, and maximum size (including unpacked size) for each upload feature. Additionally, ensure that the documentation specifies how files are made safe for end-users to download and process, such as how the application behaves when a malicious file is detected. | **N/A** | See FINDING-009 |
| 5.2.1 | Verify that the application will only accept files of a size which it can process without causing a loss of performance or a denial of service attack. | **N/A** | See FINDING-009 |
| 5.2.2 | Verify that when the application accepts a file, either on its own or within an archive such as a zip file, it checks if the file extension matches an expected file extension and validates that the contents correspond to the type represented by the extension. This includes, but is not limited to, checking the initial 'magic bytes', performing image re-writing, and using specialized libraries for file content validation. For L1, this can focus just on files which are used to make specific business or security decisions. For L2 and up, this must apply to all files being accepted. | **N/A** | See FINDING-009 |
| 5.2.3 | Verify that the application checks compressed files (e.g., zip, gz, docx, odt) against maximum allowed uncompressed size and against maximum number of files before uncompressing the file. | **N/A** | See FINDING-009 |
| 5.2.4 | Verify that a file size quota and maximum number of files per user are enforced to ensure that a single user cannot fill up the storage with too many files, or excessively large files. | **N/A** | See FINDING-009 |
| 5.2.5 | Verify that the application does not allow uploading compressed files containing symlinks unless this is specifically required (in which case it will be necessary to enforce an allowlist of the files that can be symlinked to). | **N/A** | See FINDING-009 |
| 5.2.6 | Verify that the application rejects uploaded images with a pixel size larger than the maximum allowed, to prevent pixel flood attacks. | **N/A** | See FINDING-009 |
| 5.3.1 | Verify that files uploaded or generated by untrusted input and stored in a public folder, are not executed as server-side program code when accessed directly with an HTTP request. | **N/A** | See FINDING-009 |
| 5.3.2 | Verify that when the application creates file paths for file operations, instead of user-submitted filenames, it uses internally generated or trusted data, or if user-submitted filenames or file metadata must be used, strict validation and sanitization must be applied. This is to protect against path traversal, local or remote file inclusion (LFI, RFI), and server-side request forgery (SSRF) attacks. | **N/A** | See FINDING-009 |
| 5.3.3 | Verify that server-side file processing, such as file decompression, ignores user-provided path information to prevent vulnerabilities such as zip slip. | **N/A** | See FINDING-009 |
| 5.4.1 | Verify that the application validates or ignores user-submitted filenames, including in a JSON, JSONP, or URL parameter and specifies a filename in the Content-Disposition header field in the response. | **N/A** | See FINDING-009 |
| 5.4.2 | Verify that file names served (e.g., in HTTP response header fields or email attachments) are encoded or sanitized (e.g., following RFC 6266) to preserve document structure and prevent injection attacks. | **N/A** | See FINDING-009 |
| 5.4.3 | Verify that files obtained from untrusted sources are scanned by antivirus scanners to prevent serving of known malicious content. | **N/A** | See FINDING-009 |
| **V6: Authentication** | | | |
...
| 9.1.1 | Verify that self-contained tokens are validated using their digital signature or MAC to protect against tampering before accepting the token's contents. | **N/A** | See FINDING-007 |
| 9.1.2 | Verify that only algorithms on an allowlist can be used to create and verify self-contained tokens, for a given context. The allowlist must include the permitted algorithms, ideally only either symmetric or asymmetric algorithms, and must not include the 'None' algorithm. If both symmetric and asymmetric must be supported, additional controls will be needed to prevent key confusion. | **N/A** | See FINDING-007 |
| 9.1.3 | Verify that key material that is used to validate self-contained tokens is from trusted pre-configured sources for the token issuer, preventing attackers from specifying untrusted sources and keys. For JWTs and other JWS structures, headers such as 'jku', 'x5u', and 'jwk' must be validated against an allowlist of trusted sources. | **N/A** | See FINDING-007 |
| 9.2.1 | Verify that, if a validity time span is present in the token data, the token and its content are accepted only if the verification time is within this validity time span. For example, for JWTs, the claims 'nbf' and 'exp' must be verified. | **N/A** | See FINDING-007 |
| 9.2.2 | Verify that the service receiving a token validates the token to be the correct type and is meant for the intended purpose before accepting the token's contents. For example, only access tokens can be accepted for authorization decisions and only ID Tokens can be used for proving user authentication. | **N/A** | See FINDING-007 |
| 9.2.3 | Verify that the service only accepts tokens which are intended for use with that service (audience). For JWTs, this can be achieved by validating the 'aud' claim against an allowlist defined in the service. | **N/A** | See FINDING-007 |
| 9.2.4 | Verify that, if a token issuer uses the same private key for issuing tokens to different audiences, the issued tokens contain an audience restriction that uniquely identifies the intended audiences. This will prevent a token from being reused with an unintended audience. If the audience identifier is dynamically provisioned, the token issuer must validate these audiences in order to make sure that they do not result in audience impersonation. | **N/A** | See FINDING-008 |
| **V10: OAuth and OIDC** | | | |
| 10.1.1 | Verify that tokens are only sent to components that strictly need them. For example, when using a backend-for-frontend pattern for browser-based JavaScript applications, access and refresh tokens shall only be accessible for the backend. | **N/A** | See FINDING-002 |
| 10.1.2 | Verify that the client only accepts values from the authorization server (such as the authorization code or ID Token) if these values result from an authorization flow that was initiated by the same user agent session and transaction. This requires that client-generated secrets, such as the proof key for code exchange (PKCE) 'code_verifier', 'state' or OIDC 'nonce', are not guessable, are specific to the transaction, and are securely bound to both the client and the user agent session in which the transaction was started. | **N/A** | See FINDING-003 |
| 10.2.1 | Verify that, if the code flow is used, the OAuth client has protection against browser-based request forgery attacks, commonly known as cross-site request forgery (CSRF), which trigger token requests, either by using proof key for code exchange (PKCE) functionality or checking the 'state' parameter that was sent in the authorization request. | **N/A** | See FINDING-018 |
| 10.2.2 | Verify that, if the OAuth client can interact with more than one authorization server, it has a defense against mix-up attacks. For example, it could require that the authorization server return the 'iss' parameter value and validate it in the authorization response and the token response. | **N/A** | See FINDING-018 |
| 10.2.3 | Verify that the OAuth client only requests the required scopes (or other authorization parameters) in requests to the authorization server. | **N/A** | See FINDING-018 |
| 10.3.1 | Verify that the resource server only accepts access tokens that are intended for use with that service (audience). The audience may be included in a structured access token (such as the 'aud' claim in JWT), or it can be checked using the token introspection endpoint. | **N/A** | See FINDING-007 |
| 10.3.2 | Verify that the resource server enforces authorization decisions based on claims from the access token that define delegated authorization. If claims such as 'sub', 'scope', and 'authorization_details' are present, they must be part of the decision. | **N/A** | See FINDING-004 |
| 10.3.3 | Verify that if an access control decision requires identifying a unique user from an access token (JWT or related token introspection response), the resource server identifies the user from claims that cannot be reassigned to other users. Typically, it means using a combination of 'iss' and 'sub' claims. | **N/A** | See FINDING-004 |
| 10.3.4 | Verify that, if the resource server requires specific authentication strength, methods, or recentness, it verifies that the presented access token satisfies these constraints. For example, if present, using the OIDC 'acr', 'amr' and 'auth_time' claims respectively. | **N/A** | See FINDING-005 |
| 10.3.5 | Verify that the resource server prevents the use of stolen access tokens or replay of access tokens (from unauthorized parties) by requiring sender-constrained access tokens, either Mutual TLS for OAuth 2 or OAuth 2 Demonstration of Proof of Possession (DPoP). | **N/A** | See FINDING-006 |
| 10.4.1 | Verify that the authorization server validates redirect URIs based on a client-specific allowlist of pre-registered URIs using exact string comparison. | **N/A** | See FINDING-019 |
| 10.4.2 | Verify that, if the authorization server returns the authorization code in the authorization response, it can be used only once for a token request. For the second valid request with an authorization code that has already been used to issue an access token, the authorization server must reject a token request and revoke any issued tokens related to the authorization code. | **N/A** | See FINDING-019 |
| 10.4.3 | Verify that the authorization code is short-lived. The maximum lifetime can be up to 10 minutes for L1 and L2 applications and up to 1 minute for L3 applications. | **N/A** | See FINDING-019 |
...
| 11.2.3 | Verify that all cryptographic primitives utilize a minimum of 128-bits of security based on the algorithm, key size, and configuration. For example, a 256-bit ECC key provides roughly 128 bits of security where RSA requires a 3072-bit key to achieve 128 bits of security. | **N/A** |  |
| 11.2.4 | Verify that all cryptographic operations are constant-time, with no 'short-circuit' operations in comparisons, calculations, or returns, to avoid leaking information. | **N/A** |  |
...
| 11.3.3 | Verify that encrypted data is protected against unauthorized modification preferably by using an approved authenticated encryption method or by combining an approved encryption method with an approved MAC algorithm. | **N/A** |  |
| 11.3.4 | Verify that nonces, initialization vectors, and other single-use numbers are not used for more than one encryption key and data-element pair. The method of generation must be appropriate for the algorithm being used. | **N/A** |  |
| 11.3.5 | Verify that any combination of an encryption algorithm and a MAC algorithm is operating in encrypt-then-MAC mode. | **N/A** |  |
...
| 11.4.3 | Verify that hash functions used in digital signatures, as part of data authentication or data integrity are collision resistant and have appropriate bit-lengths. If collision resistance is required, the output length must be at least 256 bits. If only resistance to second pre-image attacks is required, the output length must be at least 128 bits. | **N/A** |  |
...
| 11.5.1 | Verify that all random numbers and strings which are intended to be non-guessable must be generated using a cryptographically secure pseudo-random number generator (CSPRNG) and have at least 128 bits of entropy. Note that UUIDs do not respect this condition. | **N/A** |  |
| 11.5.2 | Verify that the random number generation mechanism in use is designed to work securely, even under heavy demand. | **N/A** |  |
...
| 11.7.2 | Verify that data minimization ensures the minimal amount of data is exposed during processing, and ensure that data is encrypted immediately after use or as soon as feasible. | **N/A** | See FINDING-017 |
...
| 13.4.7 | Verify that the web tier is configured to only serve files with specific file extensions to prevent unintentional information, configuration, and source code leakage. | **Pass** |  |
| **V14: Data Protection** | | | |
...
| 16.1.1 | Verify that an inventory exists documenting the logging performed at each layer of the application's technology stack, what events are being logged, log formats, where that logging is stored, how it is used, how access to it is controlled, and for how long logs are kept. | **N/A** | See FINDING-010 |
| 16.2.1 | Verify that each log entry includes necessary metadata (such as when, where, who, what) that would allow for a detailed investigation of the timeline when an event happens. | **N/A** | See FINDING-011 |
...
| 16.2.3 | Verify that the application only stores or broadcasts logs to the files and services that are documented in the log inventory. | **N/A** | See FINDING-011 |
| 16.2.4 | Verify that logs can be read and correlated by the log processor that is in use, preferably by using a common logging format. | **N/A** | See FINDING-011 |
| 16.2.5 | Verify that when logging sensitive data, the application enforces logging based on the data's protection level. For example, it may not be allowed to log certain data, such as credentials or payment details. Other data, such as session tokens, may only be logged by being hashed or masked, either in full or partially. | **N/A** | See FINDING-013 |
| 16.3.1 | Verify that all authentication operations are logged, including successful and unsuccessful attempts. Additional metadata, such as the type of authentication or factors used, should also be collected. | **N/A** | See FINDING-014 |
| 16.3.2 | Verify that failed authorization attempts are logged. For L3, this must include logging all authorization decisions, including logging when sensitive data is accessed (without logging the sensitive data itself). | **N/A** | See FINDING-014 |
| 16.3.3 | Verify that the application logs the security events that are defined in the documentation and also logs attempts to bypass the security controls, such as input validation, business logic, and anti-automation. | **N/A** | See FINDING-014 |
| 16.3.4 | Verify that the application logs unexpected errors and security control failures such as backend TLS failures. | **N/A** | See FINDING-015 |
| 16.4.1 | Verify that all logging components appropriately encode data to prevent log injection. | **N/A** | See FINDING-011 |
| 16.4.2 | Verify that logs are protected from unauthorized access and cannot be modified. | **N/A** | See FINDING-014 |
| 16.4.3 | Verify that logs are securely transmitted to a logically separate system for analysis, detection, alerting, and escalation. The aim is to ensure that if the application is breached, the logs are not compromised. | **N/A** | See FINDING-011 |
| 16.5.1 | Verify that a generic message is returned to the consumer when an unexpected or security-sensitive error occurs, ensuring no exposure of sensitive internal system data such as stack traces, queries, secret keys, and tokens. | **N/A** | See FINDING-013 |
| 16.5.2 | Verify that the application continues to operate securely when external resource access fails, for example, by using patterns such as circuit breakers or graceful degradation. | **N/A** | See FINDING-014 |
| 16.5.3 | Verify that the application fails gracefully and securely, including when an exception occurs, preventing fail-open conditions such as processing a transaction despite errors resulting from validation logic. | **Pass** | See FINDING-016 |
| 16.5.4 | Verify that a "last resort" error handler is defined which will catch all unhandled exceptions. This is both to avoid losing error details that must go to log files and to ensure that an error does not take down the entire application process, leading to a loss of availability. | **N/A** | See FINDING-014 |
| **V17: WebRTC** | | | |
...
**Summary Statistics:**
- **Pass**: 27 requirements (7.8%)
- **Partial**: 0 requirements (0.0%)
- **N/A**: 318 requirements (92.2%)
...
|------------|----------|-------------------|------------------|---------------------|
| FINDING-001 | Informational | 1.5.1 | — | common/daos.py, common/models.py, queries/daos.py, queries/models.py, queries/query.py, queries/types.py |
| FINDING-002 | Informational | 10.1.1 | — | superset-core/src/superset_core/** |
| FINDING-003 | Informational | 10.1.2 | — | superset-core/src/superset_core/** |
| FINDING-004 | Informational | 10.3.2, 10.3.3 | — | superset-core/src/superset_core/** |
| FINDING-005 | Informational | 10.3.4 | — | superset-core/src/superset_core/ |
| FINDING-006 | Informational | 10.3.5 | — | superset-core/src/superset_core/ |
| FINDING-007 | Informational | 9.1.1, 10.3.1, 9.1.2, 9.1.3, 9.2.1, 9.2.2, 9.2.3 | — | superset-core/src/superset_core/** |
| FINDING-008 | Informational | 9.2.4 | — | superset-core/src/superset_core/** |
| FINDING-009 | Informational | 3.5.2, 3.5.3, 3.5.4, 3.5.5, 3.5.6, 3.5.7, 3.5.8, 5.1.1, 5.2.1, 5.2.2, 5.2.3, 5.2.4, 5.2.5, 5.2.6, 5.3.1, 5.3.2, 5.3.3, 5.4.1, 5.4.2, 5.4.3 | — | superset-core/src/superset_core/rest_api/api.py |
| FINDING-010 | Informational | 16.1.1 | — | rest_api/api.py, tasks/models.py |
| FINDING-011 | Informational | 16.2.1, 16.2.3, 16.2.4, 16.4.1, 16.4.3 | — | rest_api/api.py, tasks/models.py |
| FINDING-012 | Informational | 16.2.2 | — | superset-core/src/superset_core/tasks/models.py |
| FINDING-013 | Informational | 16.2.5, 16.5.1 | — | superset-core/src/superset_core/tasks/models.py |
| FINDING-014 | Informational | 16.3.1, 16.3.2, 16.3.3, 16.4.2, 16.5.2, 16.5.4 | — | superset-core/src/superset_core/rest_api/api.py, superset-core/src/superset_core/tasks/models.py |
| FINDING-015 | Informational | 16.3.4 | — | superset-core/src/superset_core/tasks/models.py |
| FINDING-016 | Informational | 16.5.3 | — | superset-core/src/superset_core/tasks/models.py |
| FINDING-017 | Informational | 11.7.2 | — | superset-core/src/superset_core/queries/types.py, superset-core/src/superset_core/tasks/models.py |
| FINDING-018 | Informational | 10.2.1, 10.2.2, 10.2.3, 10.5.1 | FINDING-019 | superset-core package (all files) |
| FINDING-019 | Informational | 10.4.1, 10.4.2, 10.4.3, 10.4.16 | FINDING-018 | superset-core package (all files) |

**Total Unique Findings**: 19 (0 Critical, 0 High, 0 Medium, 0 Low, 19 Info)

*0 of 19 are actionable. Informational findings are recorded here but not opened as GitHub issues; see issues.md for the 0 actionable items.*
