...
This audit was conducted against ASVS Level 3, the highest assurance tier, covering the full scope of the WebSocket sidecar service up to L3 controls. The assessment evaluated authentication, session delegation, cryptographic key management, logging and monitoring, transport security, and dependency posture, with each finding mapped to its corresponding ASVS requirement.

### Top 5 Risks

1. **JWT signing secret accepted with no minimum-strength validation** [Medium] — The service permits an empty default and a weak placeholder example secret, enabling potential authentication bypass when operators fail to override defaults.
2. **WebSocket sidecar accepts empty/placeholder jwtSecret with no fail-closed validation** [Medium] — Startup does not fail closed on a missing or placeholder secret, allowing an insecurely configured service to run.
3. **Token type / purpose validation missing in websocket JWT verification** [Medium] — Verification does not confirm the token's intended type or purpose, weakening defense against token reuse across contexts.
4. **Audience (aud) validation not enforced in websocket JWT verification** [Medium] — Absent audience checks allow tokens issued for other audiences to be accepted by the sidecar.
5. **Log injection via unencoded attacker-controlled values on unauthenticated paths** [Medium] — Unencoded attacker-influenced values written to logs allow log forging and downstream log-processing manipulation.

### Positive Controls Observed

- **Input validation & encoding:** Input is decoded into canonical form only once before processing; output encoding is performed as the final step before interpreter use, including for JavaScript/JSON content.
- **Injection defenses:** Parameterized queries / ORM protection against SQL injection; regular expressions verified free of exponential backtracking (ReDoS); no use of `eval()` or dynamic code execution; safe deserialization practices enforced.
- **Server-side validation:** Positive (allowlist) input validation enforced at the trusted service layer; combined/related data items validated for logical consistency; consistent parsing across parsers for the same data type.
- **Session lifecycle delegation:** Token expiry/revocation and session termination intentionally owned by the issuing Superset backend / Flask-AppBuilder; the sidecar performs one-time signature-verified JWT channel binding at connect, with channel scoping preventing cross-user leakage on stale connections.
- **HTTP/API security:** Content-Type with proper charset set on all bodied responses; HTTP→HTTPS redirect scoped to user-facing endpoints; intermediary headers (X-Real-IP, X-Forwarded-*, X-User-ID) cannot be overridden by end-users; only supported HTTP methods allowed; request-smuggling and Content-Length conflicts prevented.
- **WebSocket handshake:** Origin header validated against an allowlist during the handshake; dedicated WebSocket session tokens comply with session management requirements and are validated through an authenticated HTTPS session during transition.
- **Cryptographic key management:** JWT shared secret correctly scoped to exactly two entities (Flask issuer and sidecar verifier); approved HS256 algorithm selected by default.
- **Frontend/transport posture:** Production sidecar serves no browser assets, eliminating external-script SRI attack surface; HSTS/TLS/preload correctly delegated to the TLS-terminating reverse proxy.
- **Service configuration:** Operator-configurable Redis username (REDIS_USERNAME) and logical DB separation support least-privilege ACL accounts; timeout, ping, GC, and bounded stream-read controls present, with connection-cap/saturation behavior delegated to the deployment/WAF layer.
...
#### FINDING-001: Token type / purpose validation missing in websocket JWT verification
...
| **File(s)** | superset-websocket/src/index.ts:255-269 |
| **Source Report(s)** | 9.2.2.md |
| **Related Finding(s)** | - |

**Description:**

Location: `superset-websocket/src/index.ts`, function `readChannelId`, lines 255–269. Data flow: `session`/`async-token` cookie → `jwt.verify()` (signature + exp only) → `channel` claim used to scope a Redis subscription → missing control: no claim distinguishing *this* token's intended type/purpose (e.g., a `type`/`typ`/`use`/`scope` claim) is checked. Attacker capability required: An actor able to obtain any JWT minted by the Flask backend under the same shared secret for a different purpose (e.g., a session, CSRF, or guest/embedding token) that also contains — or can be made to contain — a value under the configured `channel` key. This is an authenticated/cross-feature scenario, not remote-unauthenticated. Impact on success: A token issued for another purpose is accepted as a WebSocket authentication credential, allowing the holder to open a streaming connection scoped to whatever `channel` value the token carries — a token-cross-usage / authentication-purpose-confusion bypass. Impact is bounded to opening a stream for a channel value the token already encodes, so it is a control bypass without broad C/I/A escalation. Proof of concept: If the Flask backend signs any other JWT with the shared `jwtSecret` whose payload includes `{ "channel": "<target-uuid>" }` (directly or by reusing a claim name that collides with `jwtChannelIdKey`), present it as `Cookie: async-token=<that-token>` to the upgrade endpoint. Because only signature and `exp` are checked, the upgrade succeeds and the sidecar begins streaming the target channel. No purpose/type claim blocks it.

**Remediation:**

Require and validate an explicit purpose/type claim that the issuer sets exclusively for WebSocket auth tokens; pass `audience` to `jwt.verify()` and reject any token where `jwtPayload['type'] !== 'ws_async'`. Coordinate with the Flask issuer to stamp a dedicated `type`/`use` claim (and audience) so other token types signed with the same secret are not interchangeable here.

---

#### FINDING-002: Audience (aud) validation not enforced in websocket JWT verification
...
| **ASVS Level(s)** | L2 |
| **CWE** | CWE-346 |
| **ASVS Section(s)** | 9.2.3 |
| **File(s)** | superset-websocket/src/index.ts:260-263, superset-websocket/src/config.ts |
| **Source Report(s)** | 9.2.3.md |
| **Related Finding(s)** | - |

**Description:**

Location: `superset-websocket/src/index.ts`, function `readChannelId`, lines 260–263. Data flow: cookie JWT → `jwt.verify()` invoked without an `audience` option → claims trusted → missing control: the `aud` claim is never checked against an allowlist of identifiers belonging to this service. Attacker capability required: An actor holding any JWT validly signed with the shared `jwtSecret` but intended for a different audience/service (any other consumer that shares or could share this secret). Authenticated/cross-service capability. Impact on success: The sidecar accepts tokens minted for other services as valid WebSocket credentials, defeating audience-restriction. Combined with the missing type check (ASVS-922-MED-001), this widens the set of acceptable tokens beyond those issued specifically for the sidecar. Impact is a control bypass (token reuse across services) bounded by the `channel` value carried in the token. Proof of concept: Take any non-expired token signed with the shared secret whose payload contains the configured `channel` key (regardless of its `aud`), e.g. a token whose `aud` is `"superset-api"`. Send it as `Cookie: async-token=<token>`. Since `jwt.verify` receives no `audience` constraint, validation passes and the upgrade proceeds — even though the token was never minted for the websocket service.

**Remediation:**

Add an audience claim on the issuer (Flask) and enforce it at the verifier by passing `audience: opts.jwtAudience` to `jwt.verify()`. Expose `jwtAudience` as a configuration knob (`config.ts` / env override) so deployments can set the expected audience, and have Flask set a matching `aud` when issuing sidecar tokens.

---

#### FINDING-003: JWT signing secret accepted with no minimum-strength validation; empty default and weak placeholder example enable authentication bypass
...
| **ASVS Level(s)** | L2 |
| **CWE** | CWE-326 |
| **ASVS Section(s)** | 11.2.1, 11.2.3, 11.5.1 |
| **File(s)** | superset-websocket/src/config.ts, superset-websocket/config.example.json |
| **Source Report(s)** | 11.2.1.md, 11.2.3.md, 11.5.1.md |
| **Related Finding(s)** | - |

**Description:**

buildConfig() performs no validation that jwtSecret is non-empty or meets minimum strength requirements. The compiled-in default is '' (empty string, zero bits of entropy) and the shipped example config uses 'CHANGE-ME' (~72 bits). The application starts normally with these weak values, creating a silent fail-open condition. With HS256 signing directly keyed on this string, an empty or trivially guessable key makes JWT signatures forgeable by any attacker who can reach the WebSocket upgrade path. Data flow: missing/blank JWT_SECRET env var and absent/placeholder config file → _merge keeps '' or 'CHANGE-ME' → config.jwtSecret → consumed unchecked by JWT verifier → forgeable HMAC → authentication bypass at WebSocket layer → cross-user access to streamed async query results. This violates multiple ASVS requirements: no validation of key material strength (11.2.1), sub-128-bit entropy (11.2.3), and predictable/non-CSPRNG-generated secrets (11.5.1). The failure is silent and undocumented, qualifying as a fail-open default per the in-scope SECURITY.md policy.

**Remediation:**

Add fail-closed validation in buildConfig() that: (1) rejects empty jwtSecret; (2) enforces minimum length of 32 bytes (256 bits); (3) denylists known placeholders like 'CHANGE-ME'; (4) refuses to start the application if validation fails. Replace the 'CHANGE-ME' placeholder in config.example.json with instructions to generate a high-entropy secret using a CSPRNG (e.g., 'openssl rand -base64 48') and inject via JWT_SECRET environment variable. Document the secret generation and rotation procedure in deployment guidance, ensuring synchronization between Flask issuer and sidecar verifier.

---

#### FINDING-004: WebSocket sidecar accepts empty/placeholder jwtSecret with no fail-closed validation
...
| **ASVS Level(s)** | L2, L3 |
| **CWE** | CWE-798 |
| **ASVS Section(s)** | 13.1.4, 13.2.3 |
| **File(s)** | superset-websocket/src/config.ts, superset-websocket/config.example.json |
| **Source Report(s)** | 13.1.4.md, 13.2.3.md |
| **Related Finding(s)** | - |

**Description:**

`jwtSecret` (the critical secret authenticating the Flask backend↔sidecar trust) defaults to `''` and the example ships `CHANGE-ME`, and `buildConfig()` performs no validation that the secret is non-empty or of adequate length/entropy. Given an empty/known signing key, a remote unauthenticated attacker who can reach the sidecar could forge a valid HS256 JWT and open an authenticated channel, yielding cross-user query-result access. This represents both a failure to document and enforce secret rotation requirements (13.1.4) and a default credential vulnerability (13.2.3). JWT signature/auth validation defects are explicitly in scope per websocket_internal_sidecar.md, and an undocumented/unenforced fail-open default is a codebase issue per the fail-open default policy (SECURITY.md).

**Remediation:**

Add fail-closed validation in `buildConfig()` rejecting empty, placeholder (`CHANGE-ME`), or under-length `jwtSecret` before the service accepts connections. Document the set of critical secrets and a deployment-managed rotation schedule.

---

#### FINDING-005: Log injection via unencoded attacker-controlled values (unauthenticated paths)
...
| **ASVS Level(s)** | L2 |
| **CWE** | CWE-117 |
| **ASVS Section(s)** | 16.2.4, 16.4.1 |
| **File(s)** | superset-websocket/src/index.ts |
| **Source Report(s)** | 16.2.4.md, 16.4.1.md |
| **Related Finding(s)** | - |

**Description:**

Multiple log statements interpolate attacker-controlled, unencoded strings directly into log messages. `lastId` is taken from `url.searchParams.get('last_id')` and `rawUrl`/`method` come straight from the inbound HTTP request line — both attacker-controllable through the reverse proxy. They are concatenated into a log message without encoding. Embedded CR/LF lets an attacker forge or split log lines (log injection / log forging), degrading the ability of the log processor to parse and correlate entries. The `httpRequest` (`method`, `rawUrl`) and `getLastId` (`lastId`) sinks are reachable remote-unauthenticated (logged before any JWT check), undermining the forensic integrity Section 16.4 protects.

**Remediation:**

Encode/sanitize all user-derived values before logging (strip or escape CR/LF and control characters) and prefer structured logging where field values are serialized safely; a central sanitizing wrapper in `./logger` is preferable to per-call escaping.

---

#### FINDING-006: Successful authentication logged only at debug level with no metadata
...
| **ASVS Level(s)** | L2 |
| **CWE** | CWE-778 |
| **ASVS Section(s)** | 16.3.1 |
| **File(s)** | superset-websocket/src/index.ts |
| **Source Report(s)** | 16.3.1.md |
| **Related Finding(s)** | FINDING-007 |

**Description:**

The sidecar's authentication operation is JWT validation on the upgrade handshake. Failed validation is logged at `error` level, but successful authentication is logged only at `debug` level and carries no authentication metadata (no source IP, user identity, JWT algorithm/type, or channel-binding outcome). In production `debug` is normally suppressed, so successful WebSocket authentications effectively go unrecorded, defeating correlation of who connected, from where, to which channel.

**Remediation:**

Promote successful-auth logging to `info` and attach connection metadata (source IP via proxy-forwarded header, channel, algorithm). Ensure failure logging also records the source IP.

---

#### FINDING-007: Redis client constructed without an 'error' event handler
...
The Redis client is constructed without an 'error' event handler. ioredis emits 'error' events for connection failures, authentication failures, and TLS/handshake failures (the 'backend TLS failures' called out by 16.3.4). With no listener attached, these security-relevant backend failures are not logged through the application logger, and an unhandled emitter 'error' can destabilize the process. Note: this is about LOGGING backend connection/TLS failures, not about TLS enforcement (which is delegated); the defect is the missing error-handler/log path.

**Remediation:**

Attach redis.on('error', (e) => logger.error('redis_connection_error', { message: e.message }));

### 3.4 Low

#### FINDING-008: Forwarded event payloads (including `user_id` and `result_url`) are logged without redaction
...
| **Files** | superset-websocket/src/index.ts, superset-websocket/src/logger.ts |
| **Source Reports** | 14.2.4.md, 16.2.5.md |
| **Related** | — |

**Description:**

The `results`/`data` objects deserialize to `EventValue`, which carries `user_id`, `result_url`, `job_id`, `channel_id`, and `errors`. The Winston logger applies no field redaction. Data flow: Redis stream record (per-user query result metadata, incl. `user_id`, `result_url`) → `processStreamResults` `logger.debug(...)` → Winston Console transport, and Winston File transport when `logToFile=true` → missing control: no redaction/masking of sensitive fields before logging. Attacker capability required: None directly exploitable remotely. Exposure requires (a) `logLevel=debug` (default is `info`, so the full-record `debug` line does not fire by default) and/or (b) `logToFile=true` plus access to the log file/observability pipeline. Impact: information disclosure — correlatable `user_id` and internal `result_url` references plus job/channel identifiers persisted in logs. Why Low (not higher): the fullest exposure is at `debug` level, which is not the default; the leaked fields are metadata/identifiers rather than raw query result rows; and reaching the logs requires log-store access rather than a remote request.

**Remediation:**

Redact or project sensitive fields before logging, and avoid logging entire payloads at any level. Add a Winston format that strips known sensitive keys (`user_id`, `result_url`) centrally in `createLogger` so any future log call inherits redaction.

---

#### FINDING-009: Security-event logs missing required metadata (who/where/what)
...
A failed JWT validation (an authentication/security-control failure event) is logged as the bare error object only. There is no associated `who` (no subject/claims that did parse), `where` (no source IP / `X-Forwarded-For`, no requested path), or `what` (no indication that this was an upgrade-authentication failure on a specific channel). The reconnect/pong path logs a socket ID with no connection context. Impact is on incident responders: authentication-failure events cannot be correlated to a source during investigation, weakening detection of probing/brute-force against the upgrade endpoint.

**Remediation:**

Standardize an event field (`event`), `sourceIp`, `forwardedFor`, `path`, and an outcome/reason on every security-relevant log line; log `err.message` only, never the token.

---

#### FINDING-010: WebSocket error logging bypasses centralized logger via console.error
...
WebSocket-level errors are written directly via `console.error` instead of the centralized `logger`. These records skip the logger's formatting/level/file routing, producing entries in a different shape and possibly a different sink than `logFilename`, hampering single-pipeline correlation.

**Remediation:**

Route through the same logger: `ws.on('error', (e) => logger.error('ws_socket_error', { reason: (e as Error).message }));`

---

#### FINDING-011: No process-level last-resort exception handler; wsConnection re-parses JWT without try/catch
...
| **ASVS Level(s)** | L2, L3 |
| **CWE** | CWE-755 |
| **ASVS sections** | 16.3.4, 16.5.4 |
| **Files** | superset-websocket/src/index.ts |
| **Source Reports** | 16.3.4.md, 16.5.4.md |
| **Related** | — |

**Description:**

There is no process-level `uncaughtException`/`unhandledRejection` handler, and `wsConnection` re-parses the JWT (`readChannelId`) without a try/catch. An exception in an unguarded event-loop callback could terminate the process, dropping all live WebSocket connections. A defect outside the per-operation try/catch blocks would terminate the process with no application-level log entry recording the security-relevant failure, reducing forensic visibility and preventing controlled fail-secure logging. No concrete attacker-reachable trigger is demonstrated (the primary auth path is already guarded), so impact is bounded to availability resilience.

**Remediation:**

Add `process.on('uncaughtException')` / `process.on('unhandledRejection')` last-resort handlers that log full error detail and fail securely, and wrap the duplicate `readChannelId` call in `wsConnection` in a try/catch that terminates the socket on failure.

---

#### FINDING-012: Redis global-stream read loop retries with no backoff or circuit breaker
...
When Redis is down, `xread` rejects immediately (the `BLOCK` timeout only applies once connected), so the `while(true)` loop spins as fast as the event loop allows, consuming CPU and flooding logs. The service degrades but does not fail-open or serve stale data; there is no exponential backoff or circuit-breaker pattern. No direct attacker control; impact limited to CPU/log pressure during a dependency outage.

**Remediation:**

Add capped exponential backoff to the retry loop (reset on success), converting the tight-spin-on-outage into proper graceful degradation.

### 3.5 Informational

#### FINDING-013: Event payload metadata can persist in unrotated log file with no retention policy
...
Data flow: Redis stream payload (contains `result_url`, `job_id`, `user_id`) → `logger.debug(...)` → Winston `File` transport → `app.log` with no rotation/size cap/retention policy. Attacker capability required: None remotely; requires the operator to enable `logToFile=true` and set `logLevel=debug` (both non-default). Event metadata persists in the unrotated file indefinitely. Impact: query-result metadata accumulates in a log file with no defined retention or rotation. No direct C/I/A compromise; impact is information persistence beyond intended lifetime. Why Informational: requires non-default configuration, the data is metadata rather than credentials/PII content, and there is no demonstrated attacker path to read the file.

**Remediation:**

Avoid logging full event payloads even at debug level; log only the event `id` and `channel_id`. When file logging is enabled, configure rotation/retention (e.g., `winston-daily-rotate-file` with `maxFiles`/`maxSize`) so log retention is bounded.
...
|--------|---------|-----------------|------------------|
| Flask Backend Input Validation | Input is decoded into canonical form only once before processing | ASVS 1.1.1 passed - proper input decoding architecture observed | — |
| Flask Backend Input Validation | Output encoding performed as final step before interpreter use | ASVS 1.1.2 passed - proper output encoding architecture observed | — |
| Flask Backend Input Validation | Output encoding used when building JavaScript/JSON content | ASVS 1.2.3 passed - proper JavaScript/JSON encoding observed | — |
| Flask Backend Input Validation | Database queries use parameterized queries or ORM protection | ASVS 1.2.4 passed - SQL injection protection mechanisms in place | — |
| Flask Backend Input Validation | Regular expressions free from exponential backtracking vulnerabilities | ASVS 1.3.12 passed - ReDoS protections verified | — |
| Flask Backend Input Validation | Application avoids use of eval() or dynamic code execution | ASVS 1.3.2 passed - no unsafe dynamic code execution detected | — |
| Flask Backend Input Validation | Data sanitization is properly implemented for dangerous contexts | ASVS 1.3.3 passed - application appropriately sanitizes data before use in potentially dangerous contexts | — |
| Flask Backend Input Validation | Safe deserialization practices are implemented | ASVS 1.5.2 passed - deserialization of untrusted data enforces safe input handling | — |
| Flask Backend Input Validation | Consistent parsing across different parsers | ASVS 1.5.3 passed - parsers used for the same data type perform parsing consistently | — |
| Flask Backend Input Validation | Input validation enforces business and functional expectations | ASVS 2.2.1 passed - input is validated using positive validation or structure comparison | — |
| Flask Backend Input Validation | Server-side input validation is enforced at trusted service layer | ASVS 2.2.2 passed - validation is performed server-side and not reliant on client-side controls | — |
| Flask Backend Input Validation | Combined data items are validated for logical consistency | ASVS 2.2.3 passed - application ensures combinations of related data items are reasonable | — |
| Flask Appbuilder Rbac Authorization | Token-lifetime / in-session revocation policy for the WebSocket sidecar is delegated to the issuing Superset backend; the sidecar performs one-time signature-verified JWT channel binding at connect, consistent with documented deployment posture. | Observed in ASVS 8.3.2 review (dropped finding ASVS-832-INFO-001) | — |
| Flask Session Management | WebSocket sidecar validates a backend-issued JWT once at upgrade; token expiry/revocation policy and session termination are intentionally owned by the issuing Superset backend / Flask-AppBuilder, with channel scoping preventing cross-user leakage on stale connections. | Dropped finding ASVS-741-INFO-001 - architectural decision to delegate session lifecycle to backend | — |
| Frontend Browser Security | Production WebSocket sidecar serves no browser assets (only /health text and WebSocket frames), eliminating external-script SRI attack surface | CDN SRI defects exist only in the non-deployed client-ws-app developer utility | — |
| Frontend Browser Security | HSTS/TLS/preload correctly delegated to the TLS-terminating reverse proxy; the sidecar listens on plain HTTP/WS by design and does not undermine proxy-level HSTS | Dropped finding (none) - delegated per deployment_infrastructure_delegated.md and websocket_internal_sidecar.md | — |
| Business Logic Validation | Connection-rate / resource-exhaustion (anti-automation) controls for the WebSocket sidecar are delegated to the ingress/WAF and reverse proxy per deployment posture | Documented in deployment_infrastructure_delegated.md; ASVS V11 | — |
| Http Api Security | Content-Type headers with proper charset are set on all HTTP responses with message bodies | 4.1.1 - Pass | — |
| Http Api Security | HTTP to HTTPS redirect behavior is properly configured for user-facing endpoints only | 4.1.2 - Pass | — |
| Http Api Security | Intermediary HTTP headers (X-Real-IP, X-Forwarded-*, X-User-ID) cannot be overridden by end-users | 4.1.3 - Pass | — |
| Http Api Security | Only explicitly supported HTTP methods are allowed; unused methods are blocked | 4.1.4 - Pass | — |
| Http Api Security | HTTP message boundaries are properly determined to prevent request smuggling | 4.2.1 - Pass | — |
| Http Api Security | Content-Length headers do not conflict with actual content length in HTTP messages | 4.2.2 - Pass | — |
| Http Api Security | WebSocket Origin header is validated against allowlist during initial handshake | 4.4.2 - Pass | — |
| Http Api Security | Dedicated tokens for WebSocket session management comply with session management security requirements | 4.4.3 - Pass | — |
| Http Api Security | WebSocket session tokens are obtained or validated through authenticated HTTPS session during transition | 4.4.4 - Pass | — |
| Cryptography Key Management | JWT shared secret correctly scoped to two entities (Flask issuer and sidecar verifier) | jwtSecret is a shared secret used by exactly two entities, satisfying the no-oversharing rule in ASVS 11.1.1 | superset-websocket/src/config.ts |
| Cryptography Key Management | JWT algorithm selection uses approved HS256 by default | Default and example jwtAlgorithms configuration uses the approved HS256; algorithm selection is operator configuration within the operator trust boundary | — |
| Service Configuration Secrets | Connection-cap / resource-exhaustion bounding is delegated to the deployment/WAF/network layer; the sidecar exposes timeout and batch-read bounds. | Promoted from dropped finding ASVS-1312-LOW-001 | — |
| Service Configuration Secrets | Timeout and lifecycle bounds are present in config; retry/availability resilience is treated as a deployment-layer resource concern. | Promoted from dropped finding ASVS-1313-LOW-001 | — |
| Service Configuration Secrets | Redis username is operator-configurable (REDIS_USERNAME) enabling a scoped, prefix-restricted ACL account; least-privilege scoping is an operator/deployment responsibility. | Promoted from dropped finding ASVS-1322-LOW-001 | — |
| Service Configuration Secrets | Connection caps / saturation behavior are bounded at the deployment/WAF layer; the sidecar provides timeout, ping, GC, and bounded stream-read controls. | Promoted from dropped finding ASVS-1326-LOW-001 | — |
| Service Configuration Secrets | Distinct Redis username knob and logical DB separation support operator-defined least-privilege ACL accounts | Promoted from dropped finding ASVS-1332-LOW-001 | — |
| Dependency Vulnerability Management | Connection-level resource exhaustion / DoS is delegated to ingress/WAF and out of scope per SECURITY.md (resource exhaustion not bypassing a documented control) | ASVS 15.1.3 - Dropped finding ASVS-1513-MED-001 | — |
| Dependency Vulnerability Management | Availability/resource-exhaustion concerns (connection caps, payload bounds, error-loop backoff) are DoS-class items delegated to ingress/WAF and out of scope per SECURITY.md | ASVS 15.2.2 - Dropped finding ASVS-1522-MED-001 | — |
| Dependency Vulnerability Management | Channel-scoped delivery isolates forwarded results to the addressed channel; stream producer (backend) is within the trusted boundary, so field-projection is hygiene not a confidentiality finding | ASVS 15.3.1 - Dropped finding ASVS-1531-LOW-001 | — |
| Dependency Vulnerability Management | Dynamic-key collection keys derive only from signed JWT/UUID/operator config/trusted backend, all within the trusted boundary, eliminating attacker-controlled prototype-pollution input | ASVS 15.3.6 - Dropped finding ASVS-1536-INFO-001 | — |
| Dependency Vulnerability Management | Robustness/availability defects (busy-spin on Redis outage) are resource-exhaustion class, out of scope per SECURITY.md | ASVS 15.4.3 - Dropped finding ASVS-1543-LOW-001 | — |
| Dependency Vulnerability Management | Connection ceilings / fair-resource allocation are DoS-class controls delegated to ingress/WAF and out of scope per SECURITY.md | ASVS 15.4.4 - Dropped finding ASVS-1544-MED-001 | — |
| Logging Monitoring Errors | Log-store access control, file permissions, append-only storage, and encrypted forwarding are deployment-infrastructure responsibilities; the sidecar contains no code that undermines those protections. | 16.4.2 N/A determination (deployment_infrastructure_delegated.md) | — |
| Logging Monitoring Errors | Secure transmission of logs to an isolated SIEM is handled by the deployment's log-aggregation layer; the sidecar exposes log-destination knobs and ships nothing over an attacker-reachable plaintext channel of its own. | 16.4.3 N/A determination (deployment_infrastructure_delegated.md) | — |
| Redis Event Streaming | Resource-exhaustion/DoS concerns that do not bypass a documented control are out of scope; rate limiting and anti-automation are delegated to the ingress/WAF layer. | Dropped finding ASVS-143-LOW-001 | — |
...
| 4.4.2 | Verify that, during the initial HTTP WebSocket handshake, the Origin header field is checked against a list of origins allowed for the application. | **Pass** |  |
...
| 9.2.2 | Verify that the service receiving a token validates the token to be the correct type and is meant for the intended purpose before accepting the token's contents. For example, only access tokens can be accepted for authorization decisions and only ID Tokens can be used for proving user authentication. | **Fail** | See FINDING-001 |
| 9.2.3 | Verify that the service only accepts tokens which are intended for use with that service (audience). For JWTs, this can be achieved by validating the 'aud' claim against an allowlist defined in the service. | **Fail** | See FINDING-002 |
...
| 11.2.1 | Verify that industry-validated implementations (including libraries and hardware-accelerated implementations) are used for cryptographic operations. | **Fail** | See FINDING-003 |
...
| 11.2.3 | Verify that all cryptographic primitives utilize a minimum of 128-bits of security based on the algorithm, key size, and configuration. For example, a 256-bit ECC key provides roughly 128 bits of security where RSA requires a 3072-bit key to achieve 128 bits of security. | **Fail** | See FINDING-003 |
...
| 11.5.1 | Verify that all random numbers and strings which are intended to be non-guessable must be generated using a cryptographically secure pseudo-random number generator (CSPRNG) and have at least 128 bits of entropy. Note that UUIDs do not respect this condition. | **Fail** | See FINDING-003 |
...
| 13.1.4 | Verify that the application's documentation defines the secrets that are critical for the security of the application and a schedule for rotating them, based on the organization's threat model and business requirements. | **Fail** | See FINDING-004 |
...
| 13.2.3 | Verify that if a credential has to be used for service authentication, the credential being used by the consumer is not a default credential (e.g., root/root or admin/admin). | **Fail** | See FINDING-004 |
...
| 13.3.3 | Verify that all cryptographic operations are performed using an isolated security module (such as a vault or hardware security module) to securely manage and protect key material from exposure outside of the security module. | **Pass** |  |
...
| 14.2.4 | Verify that controls around sensitive data related to encryption, integrity verification, retention, how the data is to be logged, access controls around sensitive data in logs, privacy and privacy-enhancing technologies, are implemented as defined in the documentation for the specific data's protection level. | **Partial** | See FINDING-008 |
...
| 14.2.7 | Verify that sensitive information is subject to data retention classification, ensuring that outdated or unnecessary data is deleted automatically, on a defined schedule, or as the situation requires. | **Partial** | See FINDING-013 |
...
| 15.3.6 | Verify that JavaScript code is written in a way that prevents prototype pollution, for example, by using Set() or Map() instead of object literals. | **Pass** |  |
...
| 16.2.1 | Verify that each log entry includes necessary metadata (such as when, where, who, what) that would allow for a detailed investigation of the timeline when an event happens. | **Partial** | See FINDING-009 |
...
| 16.2.4 | Verify that logs can be read and correlated by the log processor that is in use, preferably by using a common logging format. | **Partial** | See FINDING-005, FINDING-010 |
| 16.2.5 | Verify that when logging sensitive data, the application enforces logging based on the data's protection level. For example, it may not be allowed to log certain data, such as credentials or payment details. Other data, such as session tokens, may only be logged by being hashed or masked, either in full or partially. | **Partial** | See FINDING-008 |
| 16.3.1 | Verify that all authentication operations are logged, including successful and unsuccessful attempts. Additional metadata, such as the type of authentication or factors used, should also be collected. | **Partial** | See FINDING-006 |
...
| 16.3.4 | Verify that the application logs unexpected errors and security control failures such as backend TLS failures. | **Partial** | See FINDING-007, FINDING-011 |
| 16.4.1 | Verify that all logging components appropriately encode data to prevent log injection. | **Partial** | See FINDING-005 |
...
| 16.5.2 | Verify that the application continues to operate securely when external resource access fails, for example, by using patterns such as circuit breakers or graceful degradation. | **Partial** | See FINDING-012 |
...
**Summary Statistics:**
- **Pass**: 79 requirements (22.9%)
- **Partial**: 10 requirements (2.9%)
- **N/A**: 249 requirements (72.2%)
- **Fail**: 7 requirements (2.0%)
...
|------------|----------|-------------------|------------------|---------------------|
| FINDING-001 | Medium | 9.2.2 | — | superset-websocket/src/index.ts |
| FINDING-002 | Medium | 9.2.3 | — | superset-websocket/src/index.ts, superset-websocket/src/config.ts |
| FINDING-003 | Medium | 11.2.1, 11.2.3, 11.5.1 | — | superset-websocket/src/config.ts, superset-websocket/config.example.json |
| FINDING-004 | Medium | 13.1.4, 13.2.3 | — | superset-websocket/src/config.ts, superset-websocket/config.example.json |
| FINDING-005 | Medium | 16.2.4, 16.4.1 | — | superset-websocket/src/index.ts |
| FINDING-006 | Medium | 16.3.1 | FINDING-007 | superset-websocket/src/index.ts |
| FINDING-007 | Medium | 16.3.4 | FINDING-006 | superset-websocket/src/index.ts |
| FINDING-008 | Low | 14.2.4, 16.2.5 | — | superset-websocket/src/index.ts, superset-websocket/src/logger.ts |
| FINDING-009 | Low | 16.2.1 | — | superset-websocket/src/index.ts |
| FINDING-010 | Low | 16.2.4 | — | superset-websocket/src/index.ts |
| FINDING-011 | Low | 16.3.4, 16.5.4 | — | superset-websocket/src/index.ts |
| FINDING-012 | Low | 16.5.2 | — | superset-websocket/src/index.ts |
| FINDING-013 | Informational | 14.2.7 | — | superset-websocket/src/index.ts, superset-websocket/src/logger.ts |

**Total Unique Findings**: 13 (0 Critical, 0 High, 7 Medium, 5 Low, 1 Info)

*12 of 13 are actionable. Informational findings are recorded here but not opened as GitHub issues; see issues.md for the 12 actionable items.*
