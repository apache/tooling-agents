...
This audit was conducted against ASVS Level 3 (L3) requirements, with scope spanning levels up to L3 across the in-scope security domains. All findings are reported regardless of severity, and ASVS control mappings are provided per finding to support remediation traceability.

### Top 5 Risks

1. **Missing Session Regeneration on Authentication** [High] — The application does not issue a fresh session identifier upon successful authentication, leaving users exposed to session fixation attacks (ASVS 7.2.4).
2. **Incomplete Session Invalidation on Termination** [High] — Sessions are not fully invalidated server-side on logout, allowing previously issued tokens to remain usable after termination (ASVS 7.4.1).
3. **Sessions Not Terminated on Account Disable or Deletion** [High] — Active sessions persist after an account is disabled or deleted, permitting continued access by deprovisioned users (ASVS 7.4.2).
4. **Missing Inactivity Timeout Enforcement** [Medium] — No idle-session timeout is enforced, extending the window of opportunity for hijacked or abandoned sessions (ASVS 7.3.1).
5. **Missing Absolute Maximum Session Lifetime** [Medium] — Sessions lack an absolute maximum lifetime, allowing them to remain valid indefinitely regardless of activity (ASVS 7.3.2).

### Positive Controls Observed

- **Input validation trust boundary** — Backend is documented as the authoritative validation boundary; client-side validation is advisory frontend-only UX (input_validation_and_injection).
- **Consistent URL sanitization** — All rendered external-link sinks (GenericLink, AnchorLink) uniformly apply `@braintree/sanitize-url` before reaching the DOM, neutralizing `javascript:`/`data:` payloads (input_validation_and_injection).
- **Safe text rendering** — Application uses `createTextNode`/`textContent` safe rendering functions to prevent unintended HTML/JavaScript execution (ASVS 3.2.2, browser_security_controls).
- **DOM clobbering prevention** — Explicit variable declarations, strict type checking, and namespace isolation are employed (ASVS 3.2.3, browser_security_controls).
- **CORS preflight enforcement** — Preflight mechanism properly prevents disallowed cross-origin requests to sensitive functionality (ASVS 3.5.2, browser_security_controls).
- **Appropriate HTTP methods** — Sensitive functionality uses POST/PUT/PATCH/DELETE rather than safe methods (ASVS 3.5.3, browser_security_controls).
- **No JSONP / XSSI exposure** — JSONP is not enabled and authorization-protected data is not included in script resource responses (ASVS 3.5.6, 3.5.7, browser_security_controls).
- **Secure client-side technologies** — Application only uses supported and secure client-side technologies (ASVS 3.7.1, browser_security_controls).
- **External redirect notification** — Users are notified and given a cancel option when redirected outside application control (ASVS 3.7.3, browser_security_controls).
- **Graceful degradation** — Application behaves as documented when the browser lacks expected security features (ASVS 3.7.5, browser_security_controls).
- **Delegated request-size limits** — URI and request-header size limits are enforced at the receiving backend/reverse proxy (http_api_security).
- **Delegated WebSocket encryption** — WSS transport encryption is delegated to the TLS-terminating reverse proxy via operator-controlled config (http_api_security).
- **Password policy** — Minimum 8-character length, top-3000 common-password checks, no composition rules, masking via `type=password`, paste/password-manager support, no truncation, and 64+ character support (ASVS 6.2.1, 6.2.4–6.2.9, authentication_mechanisms).
- **Authentication hardening** — Consistently secured authentication pathways, no email-based factor, no user enumeration via challenges, and no password hints/KBA (ASVS 6.3.4, 6.3.6, 6.3.8, 6.4.2, authentication_mechanisms).
- **Session management foundations** — Documented timeout/lifetime policies, backend-trusted token verification, dynamic tokens, CSPRNG with ≥128-bit entropy, accessible logout, and consent-based session creation (ASVS 7.1.1, 7.2.1–7.2.3, 7.4.4, 7.6.2, session_management).
- **Data protection** — Sensitive data sent only in body/headers, no sharing with untrusted third parties, documented protection controls, retention with automatic deletion, client storage cleared after termination, and no sensitive data in browser storage (ASVS 14.2.1, 14.2.3, 14.2.4, 14.2.7, 14.3.1, 14.3.3, data_protection_and_privacy).
- **Prototype pollution prevention** — Dynamically-keyed collections from backend data use `Map()` instead of object literals; plugin keys originate only from admin-controlled endpoints (third_party_dependencies).
...
#### FINDING-001: Missing Session Regeneration on Authentication
...
The application does not generate a new session token upon user authentication or re-authentication, and does not terminate the current session token. This violates ASVS 7.2.4 requirements and creates a risk of session fixation attacks where an attacker can pre-set a session identifier and hijack the user's authenticated session.

**Remediation:**

Implement session regeneration logic that generates a new session token immediately after successful authentication and re-authentication. Invalidate the previous session token to prevent reuse. Ensure this applies to all authentication paths including login, password reset, and account recovery flows.

---

#### FINDING-002: Incomplete Session Invalidation on Termination
...
| **Related Finding(s)** | FINDING-003, FINDING-004, FINDING-005, FINDING-006 |

**Description:**

The application does not properly invalidate sessions when termination is triggered (logout or expiration), allowing continued use of terminated session tokens. This violates ASVS 7.4.1 requirements and creates a risk where expired or logged-out sessions can still be used to access protected resources.

**Remediation:**

Ensure that all session termination events (logout, timeout, expiration) properly invalidate session data at the backend. For reference tokens or stateful sessions, remove or mark the session as invalid in the session store. For self-contained tokens (JWT), implement token revocation mechanisms such as a denylist or short expiration times combined with refresh token rotation. Verify that terminated sessions cannot be reused for any application functionality.

---

#### FINDING-003: Sessions Not Terminated on Account Disable or Deletion
...
| **Related Finding(s)** | FINDING-002, FINDING-004, FINDING-005, FINDING-006 |

**Description:**

The application does not terminate all active sessions when a user account is disabled or deleted. This violates ASVS 7.4.2 requirements and creates a security risk where disabled or deleted accounts can continue to access the application through existing active sessions until they naturally expire.

**Remediation:**

Implement automatic session termination logic that is triggered when user accounts are disabled or deleted. This should invalidate all active sessions associated with the account immediately. Integrate this with user management workflows including employee offboarding, account suspension, and account deletion processes. Maintain a mapping of user accounts to active sessions to enable bulk termination.

### 3.3 Medium

#### FINDING-004: Missing Inactivity Timeout Enforcement
...
| **ASVS Level(s)** | L2, L3 |
| **CWE** | CWE-613 |
| **ASVS Section(s)** | 7.3.1 |
| **File(s)** | - |
| **Source Report(s)** | 7.3.1.md |
| **Related Finding(s)** | FINDING-002, FINDING-003, FINDING-005, FINDING-006 |

**Description:**

The application does not enforce an inactivity timeout that would require re-authentication after a period of user inactivity. This violates ASVS 7.3.1 requirements and allows sessions to remain active indefinitely without user interaction, increasing the window of opportunity for session hijacking or unauthorized access from unattended devices.

**Remediation:**

Implement an inactivity timeout mechanism based on documented risk analysis. Track the last activity timestamp for each session and invalidate sessions that exceed the defined inactivity period. Configure appropriate timeout values based on application sensitivity (e.g., 15-30 minutes for sensitive applications). Provide clear user notification before timeout occurs.

---

#### FINDING-005: Missing Absolute Maximum Session Lifetime
...
| **ASVS Level(s)** | L2, L3 |
| **CWE** | CWE-613 |
| **ASVS Section(s)** | 7.3.2 |
| **File(s)** | - |
| **Source Report(s)** | 7.3.2.md |
| **Related Finding(s)** | FINDING-002, FINDING-003, FINDING-004, FINDING-006 |

**Description:**

The application does not enforce an absolute maximum session lifetime that would require re-authentication regardless of activity. This violates ASVS 7.3.2 requirements and allows sessions to persist indefinitely as long as they remain active, which may not align with security policies requiring periodic re-authentication.

**Remediation:**

Implement an absolute maximum session lifetime that enforces re-authentication after a defined period regardless of activity. Track session creation timestamp and invalidate sessions that exceed the maximum lifetime (e.g., 8-24 hours depending on risk profile). Document the chosen lifetime based on risk analysis and compliance requirements.

---

#### FINDING-006: Missing Option to Terminate Other Sessions After Authentication Factor Change
...
| **ASVS Level(s)** | L2, L3 |
| **CWE** | CWE-613 |
| **ASVS Section(s)** | 7.4.3 |
| **File(s)** | - |
| **Source Report(s)** | 7.4.3.md |
| **Related Finding(s)** | FINDING-002, FINDING-003, FINDING-004, FINDING-005 |

**Description:**

The application does not provide users with the option to terminate all other active sessions after successfully changing or removing authentication factors (password, MFA settings, etc.). This violates ASVS 7.4.3 requirements and prevents users from invalidating potentially compromised sessions after securing their account.

**Remediation:**

Implement functionality that offers users the option to terminate all other active sessions after changing authentication factors including password changes, password resets, MFA configuration updates, and recovery method modifications. This can be presented as a checkbox during the change process or as a prompt after successful changes. Consider making this automatic for high-risk changes like password reset via recovery.

### 3.4 Low

#### FINDING-007: `SafeMarkdown` disables react-markdown link sanitization (`transformLinkUri={null}`), relying solely on optional rehype-sanitize
...
| **File(s)** | superset-frontend/packages/superset-ui-core/src/components/SafeMarkdown/SafeMarkdown.tsx |
| **Source Report(s)** | 1.2.2.md |
| **Related** | FINDING-008 |

**Description:**

markdown source (e.g., dashboard markdown component content) → react-markdown link/HTML pipeline → DOM. transformLinkUri={null} removes react-markdown's built-in javascript:/data: URL stripping for [text](uri) links. Protocol filtering is then provided ONLY by rehypeSanitize, which is conditionally pushed (htmlSanitization === true). If htmlSanitization is false (and the EscapeMarkdownHtml flag is off), rehype-raw parses raw HTML with no sanitizer and transformLinkUri={null} removes link-protocol filtering, enabling javascript:-scheme links / raw HTML XSS in the viewer's browser. In the default configuration (htmlSanitization = true), rehype-sanitize's default schema clobbers dangerous href protocols, so there is no exploit path; exploitation requires a caller to opt out of sanitization.

**Remediation:**

Do not disable the built-in URI transform unconditionally. Retain react-markdown's default transformLinkUri (or supply an explicit allowlisting transform via @braintree/sanitize-url) so link-protocol safety does not depend on the optional rehypeSanitize plugin.

---

#### FINDING-008: `getOverrideHtmlSchema` mutates the shared `rehype-sanitize` default schema in place
...
| **File(s)** | superset-frontend/packages/superset-ui-core/src/components/SafeMarkdown/SafeMarkdown.tsx |
| **Source Report(s)** | 1.3.1.md |
| **Related** | FINDING-007 |

**Description:**

htmlSchemaOverrides prop (component/config-controlled) → mergeWith(defaultSchema, …) → mutates the module-level defaultSchema object imported from rehype-sanitize → all subsequent SafeMarkdown renders. lodash.mergeWith mutates its first argument; because defaultSchema is a shared module singleton, any SafeMarkdown instance that passes overrides permanently widens the allowlist used by every other SafeMarkdown instance. Not directly attacker-reachable — htmlSchemaOverrides is supplied by application/chart code, not end-user markdown. Impact is a latent sanitization-integrity bug (cross-component weakening of the markdown sanitization allowlist), increasing stored-XSS surface.

**Remediation:**

Treat the base schema as immutable; deep-clone before merging (cloneDeep(originalSchema) before mergeWith).

---

#### FINDING-009: postMessage origin validation defaults to permissive (accepts any origin) when allowed_domains is unset
...
| **File(s)** | superset-frontend/src/embedded/originValidation.ts, superset-frontend/src/embedded/index.tsx |
| **Source Report(s)** | 3.5.1.md |
| **Related** | - |

**Description:**

event.origin (attacker-controllable parent frame origin) flows into validateMessageEvent(event, bootstrapData.embedded?.allowed_domains) -> isMessageOriginAllowed. When allowed_domains is empty/undefined, the origin check is skipped (returns true) and the handshake proceeds, wiring up Switchboard methods (guestToken, setThemeConfig, setThemeMode, observeDataMask). Impact is limited: the embedded page still requires a valid host-issued guest token before start() renders any data, so an attacker without a guest token cannot cause dashboard data to load; realistic impact is confined to accepting benign UI-control messages from an unintended origin. This is a permissive-default foot-gun rather than a demonstrated data-exposure path.

**Remediation:**

Treat an empty/undefined allowlist as fail-closed for production embeds, or require operators to explicitly opt into the unrestricted mode behind a named configuration flag so the secure posture is the default.

---

#### FINDING-010: Automatic redirect relies on client-side trust list without server-side allowlist validation
...
| **File(s)** | superset-frontend/src/pages/RedirectWarning/index.tsx, superset-frontend/src/pages/RedirectWarning/utils.ts |
| **Source Report(s)** | 3.7.2.md |
| **Related** | - |

**Description:**

Data flow: `?url=` query param → `getTargetUrl()` → `useEffect` → automatic `window.location.href` assignment when the URL was previously stored in the client-side `superset_trusted_urls` list. The trust decision is entirely client-side (`localStorage`); there is no server-maintained or configured destination-host allowlist governing automatic redirects. The automatic path only fires for a destination the victim previously explicitly trusted, and the scheme is gated to `http:`/`https:`. Abuse requires same-origin XSS (a more severe compromise) or social engineering. This is a defense-in-depth/UX boundary gap, not a demonstrated open-redirect with meaningful impact in the default deployment.

**Remediation:**

Pair the client-side trust mechanism with a server-side or configured allowlist (e.g., `REDIRECT_ALLOWED_HOSTS`) so that automatic redirects (the `useEffect` path) only proceed when the destination host is on the allowlist, independent of client-side `localStorage`.

---

#### FINDING-011: Client-side log timestamps are taken from the unsynchronized browser clock
...
| **File(s)** | superset-frontend/src/middleware/loggerMiddleware.ts |
| **Source Report(s)** | 16.2.2.md |
| **Related** | - |

**Description:**

Browser Date.now() (client device clock) → ts field → shipped to /superset/log/. Date.getTime() returns epoch milliseconds, which is implicitly UTC, so the timezone portion of 16.2.2 is satisfied. The synchronization portion is not: the value originates from a client clock the server does not control and may be skewed or altered. An authenticated user can set their device clock arbitrarily, causing ts on their emitted events to be wrong. Impact: Low. Timeline confusion or misordering for client-originated events if the server persists ts verbatim. Mitigated entirely if the backend records its own server-side receipt timestamp and treats client ts as advisory. Time synchronization across logging components is documented as a deployment-infrastructure responsibility, which caps this at Low.

**Remediation:**

Ensure the backend /superset/log/ handler stamps an authoritative server-side UTC received_at and uses it (not the client ts) as the canonical event time for ordering/correlation, retaining client ts only as supplementary data. No client-code change strictly required.

---

#### FINDING-012: Logger middleware forwards full page URL and arbitrary event payload without client-side redaction
...
window.location.href / caller-supplied eventData → batched event → /superset/log/. The client performs no masking of URL query parameters or spread fields before transmission. Sensitive-data redaction is documented as being applied by server-side log-filtering configuration. The risk materializes only if (a) a URL or eventData field carries sensitive content and (b) the backend log filter is not configured to redact it. Impact: Low. URL filter/permalink/query-id content in Superset is documented as non-secret; the guest_token is sent as an authentication credential for the beacon, not persisted as event content. Net residual risk is the reliance on backend filtering for any future caller that places sensitive values into eventData.

**Remediation:**

Treat the client logger as untrusted input on the backend and apply allow-list field projection plus credential/token redaction in the /superset/log/ handler. Optionally strip query strings from path client-side before logging when the event does not require them. Ensure the backend never persists the guest_token form field into the event store.

### 3.5 Informational

#### FINDING-013: `validateMapboxStylesUrl` uses unanchored `startsWith` prefix matching
...
| **File(s)** | superset-ui-core/src/validator/validateMapboxStylesUrl.ts |
| **Source Report(s)** | 1.3.6.md |
| **Related** | - |

**Description:**

chart-control input → advisory client-side validator → (if used) tile/style URL fetched by the map renderer in the browser. The prefix 'https://tile.osm' will also accept 'https://tile.osmattacker.com/...' because matching is not anchored to a domain boundary. However, this validator is documented as advisory frontend-only UX validation with backend as the authoritative boundary, the resulting request is a client-side tile fetch (browser-originated) not a server-side request so it is not SSRF, and no concrete attacker-reachable impact is demonstrated. Recorded as a code defect only.

**Remediation:**

Match against parsed URL host with a suffix/exact check (new URL(trimmed_v) and compare hostname === host or endsWith('.'+host)).

---

#### FINDING-014: Export download size check logs a warning but does not enforce the limit (no-op control)
...
| **File(s)** | superset-frontend/src/utils/export.ts |
| **Source Report(s)** | 5.2.1.md |
| **Related** | - |

**Description:**

Data flow: server-generated export response (Content-Length) → size comparison against MAX_BLOB_SIZE → result is only logged, then the full body is buffered into a Blob in memory regardless. Type C gap (control called, result ignored) — the MAX_BLOB_SIZE constant gives the appearance of a guard but never short-circuits the in-memory buffering. The attacker capability required is an authenticated user exporting their own authorized resources; impact is limited to the requesting user's own browser memory (self-inflicted), not a server-side DoS or any C/I/A impact on another principal. It is a download path, not an upload-acceptance path. Real code defect with no meaningful attack scenario, recorded as Informational.

**Remediation:**

Either enforce the limit (abort the download and surface an error toast when Content-Length exceeds the cap) or remove the misleading constant/warning so the code does not imply a control that doesn't exist. For true large-export safety, prefer a streamed/anchor-to-URL download rather than buffering into a Blob.

---

#### FINDING-015: Client-emitted log events omit user identity ("who") and rely on backend session association
...
| **File(s)** | superset-frontend/src/middleware/loggerMiddleware.ts |
| **Source Report(s)** | 16.2.1.md |
| **Related** | - |

**Description:**

Redux LOG_EVENT action → middleware enriches with source, source_id, impression_id, ts, event_name, event_id → batched to /superset/log/. The payload carries when (ts), where (source, source_id) and what (event_name), but no who. Attacker capability required: None — this is a completeness observation, not an exploit. The "who" dimension is supplied server-side by associating the authenticated session/guest token with the log entry. Impact: No direct impact. If the backend failed to attach the authenticated principal, timeline reconstruction would lose the actor dimension — but that enrichment is a backend responsibility and is not visible as a defect in this code.

**Remediation:**

No client change required; the backend /superset/log/ handler should continue to derive the user identity from the authenticated session rather than trusting any client-supplied identity. Confirm that the backend stamps user_id/username on every persisted event.
...
|--------|---------|-----------------|------------------|
| Input Validation And Injection | Backend is documented as the authoritative boundary for validation | Validator is documented as advisory frontend-only UX validation | — |
| Input Validation And Injection | All rendered external-link sinks (GenericLink, AnchorLink) uniformly apply @braintree/sanitize-url before reaching the DOM, neutralizing javascript:/data: payloads regardless of upstream URL-classifier divergence | Consistent URL sanitization across parsers (dropped finding ASVS-153-LOW-001) | — |
| Input Validation And Injection | Application enforces input validation at trusted service layer | Backend validation is authoritative; client-side validation is advisory only | — |
| Browser Security Controls | Safe text rendering using createTextNode or textContent | ASVS 3.2.2 passed - application uses safe rendering functions to prevent unintended execution of HTML/JavaScript | — |
| Browser Security Controls | DOM clobbering prevention measures implemented | ASVS 3.2.3 passed - application employs explicit variable declarations, strict type checking, and namespace isolation | — |
| Browser Security Controls | CORS preflight mechanism properly prevents disallowed cross-origin requests to sensitive functionality | 3.5.2 | — |
| Browser Security Controls | Sensitive functionality uses appropriate HTTP methods (POST, PUT, PATCH, DELETE) rather than safe methods | 3.5.3 | — |
| Browser Security Controls | JSONP functionality is not enabled, preventing Cross-Site Script Inclusion (XSSI) attacks | 3.5.6 | — |
| Browser Security Controls | Data requiring authorization is not included in script resource responses | 3.5.7 | — |
| Browser Security Controls | Application only uses supported and secure client-side technologies | 3.7.1 | — |
| Browser Security Controls | User notification shown when redirected to URLs outside application control with option to cancel | 3.7.3 | — |
| Browser Security Controls | Application behaves as documented when browser lacks expected security features | 3.7.5 | — |
| Http Api Security | URI and request-header size limits are enforced at the receiving backend/reverse proxy as a delegated deployment control; client request-building is not required to length-guard. | Dropped finding ASVS-425-LOW-001 | — |
| Http Api Security | WebSocket transport encryption (WSS) is delegated to the TLS-terminating reverse proxy; the WebSocket URL scheme is set via the operator-controlled GLOBAL_ASYNC_QUERIES_WEBSOCKET_URL config knob. | source: Dropped finding ASVS-441-LOW-001 | — |
| Authentication Mechanisms | Password minimum length requirement of at least 8 characters is enforced | ASVS 6.2.1 passed | — |
| Authentication Mechanisms | Passwords are checked against top 3000 common passwords during registration and password change | ASVS 6.2.4 passed | — |
| Authentication Mechanisms | No character composition rules enforced - all password characters permitted | ASVS 6.2.5 passed | — |
| Authentication Mechanisms | Password input fields use type=password for masking | ASVS 6.2.6 passed | — |
| Authentication Mechanisms | Paste functionality and password managers are permitted | ASVS 6.2.7 passed | — |
| Authentication Mechanisms | Passwords verified exactly as received without truncation or transformation | ASVS 6.2.8 passed | — |
| Authentication Mechanisms | Passwords of at least 64 characters are permitted | ASVS 6.2.9 passed | — |
| Authentication Mechanisms | Authentication pathways are consistently secured | ASVS 6.3.4 passed | — |
| Authentication Mechanisms | Email is not used as a single-factor or multi-factor authentication mechanism | ASVS 6.3.6 passed | — |
| Authentication Mechanisms | Authentication challenges do not leak information about valid users | ASVS 6.3.8 passed | — |
| Authentication Mechanisms | Password hints or knowledge-based authentication (secret questions) are not present in the application | ASVS section 6.4.2 marked as Pass | — |
| Session Management | Session management documentation exists covering timeout policies and maximum session lifetimes with appropriate justification | ASVS 7.1.1 marked as Pass | — |
| Session Management | Session token verification performed using trusted backend service | ASVS 7.2.1 marked as Pass | — |
| Session Management | Dynamic session tokens used instead of static API secrets | ASVS 7.2.2 marked as Pass | — |
| Session Management | Reference tokens generated with cryptographically secure PRNG with at least 128 bits of entropy | ASVS 7.2.3 marked as Pass | — |
| Session Management | Logout functionality is easily accessible on authenticated pages | ASVS 7.4.4 marked as Pass | — |
| Session Management | Session creation requires user consent or explicit action | ASVS 7.6.2 marked as Pass | — |
| Data Protection And Privacy | Sensitive data is sent only in HTTP message body or headers, not in URLs or query strings | 14.2.1 - Pass status indicates proper handling of sensitive data transmission | — |
| Data Protection And Privacy | Application does not send defined sensitive data to untrusted third parties | 14.2.3 - Pass status indicates no unauthorized data sharing with external trackers | — |
| Data Protection And Privacy | Controls around sensitive data (encryption, integrity, retention, logging, access controls) are properly implemented | 14.2.4 - Pass status indicates documented protection controls are in place | — |
| Data Protection And Privacy | Data retention policies are implemented for sensitive information with automatic deletion | 14.2.7 - Pass status indicates proper data lifecycle management | — |
| Data Protection And Privacy | Authenticated data is cleared from client storage after session termination | 14.3.1 - Pass status indicates proper client-side data cleanup mechanisms | — |
| Data Protection And Privacy | Browser storage does not contain sensitive data (except session tokens) | 14.3.3 - Pass status indicates appropriate use of client-side storage | — |
| Third Party Dependencies | Dynamically-keyed collections sourced from backend data use Map() instead of object literals to prevent prototype pollution; plugin keys originate only from admin-controlled endpoints inside the trust boundary. | ExtensionsLoader.extensionIndex implementation | — |
...
| 1.2.2 | Verify that when dynamically building URLs, untrusted data is encoded according to its context (e.g., URL encoding or base64url encoding for query or path parameters). Ensure that only safe URL protocols are permitted (e.g., disallow javascript: or data:). | **Partial** | See FINDING-007 |
...
| 1.2.10 | Verify that the application is protected against CSV and Formula Injection. The application must follow the escaping rules defined in RFC 4180 sections 2.6 and 2.7 when exporting CSV content. Additionally, when exporting to CSV or other spreadsheet formats (such as XLS, XLSX, or ODF), special characters (including '=', '+', '-', '@', '\t' (tab), and '\0' (null character)) must be escaped with a single quote if they appear as the first character in a field value. | **Pass** |  |
| 1.3.1 | Verify that all untrusted HTML input from WYSIWYG editors or similar is sanitized using a well-known and secure HTML sanitization library or framework feature. | **Partial** | See FINDING-008 |
...
| 1.3.5 | Verify that the application sanitizes or disables user-supplied scriptable or expression template language content, such as Markdown, CSS or XSL stylesheets, BBCode, or similar. | **Partial** |  |
| 1.3.6 | Verify that the application protects against Server-side Request Forgery (SSRF) attacks, by validating untrusted data against an allowlist of protocols, domains, paths and ports and sanitizing potentially dangerous characters before using the data to call another service. | **Pass** | See FINDING-013 |
...
| 3.5.1 | Verify that, if the application does not rely on the CORS preflight mechanism to prevent disallowed cross-origin requests to use sensitive functionality, these requests are validated to ensure they originate from the application itself. This may be done by using and validating anti-forgery tokens or requiring extra HTTP header fields that are not CORS-safelisted request-header fields. This is to defend against browser-based request forgery attacks, commonly known as cross-site request forgery (CSRF). | **Fail** | See FINDING-009 |
...
| 3.5.5 | Verify that messages received by the postMessage interface are discarded if the origin of the message is not trusted, or if the syntax of the message is invalid. | **Fail** |  |
...
| 5.2.1 | Verify that the application will only accept files of a size which it can process without causing a loss of performance or a denial of service attack. | **Pass** | See FINDING-014 |
...
| 7.2.4 | Verify that the application generates a new session token on user authentication, including re-authentication, and terminates the current session token. | **Fail** | See FINDING-001 |
| 7.3.1 | Verify that there is an inactivity timeout such that re-authentication is enforced according to risk analysis and documented security decisions. | **Fail** | See FINDING-004 |
| 7.3.2 | Verify that there is an absolute maximum session lifetime such that re-authentication is enforced according to risk analysis and documented security decisions. | **Fail** | See FINDING-005 |
| 7.4.1 | Verify that when session termination is triggered (such as logout or expiration), the application disallows any further use of the session. For reference tokens or stateful sessions, this means invalidating the session data at the application backend. Applications using self-contained tokens will need a solution such as maintaining a list of terminated tokens, disallowing tokens produced before a per-user date and time or rotating a per-user signing key. | **Fail** | See FINDING-002 |
| 7.4.2 | Verify that the application terminates all active sessions when a user account is disabled or deleted (such as an employee leaving the company). | **Fail** | See FINDING-003 |
| 7.4.3 | Verify that the application gives the option to terminate all other active sessions after a successful change or removal of any authentication factor (including password change via reset or recovery and, if present, an MFA settings update). | **Fail** | See FINDING-006 |
...
| 16.2.5 | Verify that when logging sensitive data, the application enforces logging based on the data's protection level. For example, it may not be allowed to log certain data, such as credentials or payment details. Other data, such as session tokens, may only be logged by being hashed or masked, either in full or partially. | **Partial** | See FINDING-012 |
...
**Summary Statistics:**
- **Pass**: 76 requirements (22.0%)
- **Partial**: 6 requirements (1.7%)
- **N/A**: 255 requirements (73.9%)
- **Fail**: 8 requirements (2.3%)
...
|------------|----------|-------------------|------------------|---------------------|
| FINDING-001 | High | 7.2.4 | — | — |
| FINDING-002 | High | 7.4.1 | FINDING-003, FINDING-004, FINDING-005, FINDING-006 | — |
| FINDING-003 | High | 7.4.2 | FINDING-002, FINDING-004, FINDING-005, FINDING-006 | — |
| FINDING-004 | Medium | 7.3.1 | FINDING-002, FINDING-003, FINDING-005, FINDING-006 | — |
| FINDING-005 | Medium | 7.3.2 | FINDING-002, FINDING-003, FINDING-004, FINDING-006 | — |
| FINDING-006 | Medium | 7.4.3 | FINDING-002, FINDING-003, FINDING-004, FINDING-005 | — |
| FINDING-007 | Low | 1.2.2 | FINDING-008 | superset-frontend/packages/superset-ui-core/src/components/SafeMarkdown/SafeMarkdown.tsx |
| FINDING-008 | Low | 1.3.1 | FINDING-007 | superset-frontend/packages/superset-ui-core/src/components/SafeMarkdown/SafeMarkdown.tsx |
| FINDING-009 | Low | 3.5.1 | — | superset-frontend/src/embedded/originValidation.ts, superset-frontend/src/embedded/index.tsx |
| FINDING-010 | Low | 3.7.2 | — | superset-frontend/src/pages/RedirectWarning/index.tsx, superset-frontend/src/pages/RedirectWarning/utils.ts |
| FINDING-011 | Low | 16.2.2 | — | superset-frontend/src/middleware/loggerMiddleware.ts |
| FINDING-012 | Low | 16.2.5 | — | superset-frontend/src/middleware/loggerMiddleware.ts |
| FINDING-013 | Informational | 1.3.6 | — | superset-ui-core/src/validator/validateMapboxStylesUrl.ts |
| FINDING-014 | Informational | 5.2.1 | — | superset-frontend/src/utils/export.ts |
| FINDING-015 | Informational | 16.2.1 | — | superset-frontend/src/middleware/loggerMiddleware.ts |

**Total Unique Findings**: 15 (0 Critical, 3 High, 3 Medium, 6 Low, 3 Info)

*12 of 15 are actionable. Informational findings are recorded here but not opened as GitHub issues; see issues.md for the 12 actionable items.*
