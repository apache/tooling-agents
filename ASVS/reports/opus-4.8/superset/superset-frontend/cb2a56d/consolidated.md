...
*Informational findings are recorded in this report but not opened as GitHub issues — see issues.md for the 3 actionable items.*
...
This audit was conducted against ASVS Level 3 (L3), with the severity threshold set to include all findings regardless of impact. Coverage spanned 19 security domains across the superset-frontend codebase, with individual findings mapped to their relevant ASVS verification levels (L1 through L3).

### Top 5 Risks

1. **Unsanitized data interpolated into Datamaps popup HTML in WorldMap chart** [Medium] — User-controlled data is injected into Datamaps popup HTML without sanitization, creating a DOM-based XSS exposure in the WorldMap chart (ASVS 1.3.1).
2. **postMessage origin validation fails open when allowedDomains is empty/undefined** [Medium] — Origin validation logic accepts messages from any source when the allowedDomains list is empty or undefined, weakening cross-origin message trust boundaries (ASVS 3.5.1, 3.5.5).
3. **SafeMarkdown disables react-markdown's built-in link-URI sanitization** [Low] — Enabling the EscapeMarkdownHtml flag inadvertently disables react-markdown's built-in link-URI sanitization, reducing defense against unsafe link schemes (ASVS 1.3.5).
4. **Missing Timeout on Theme Recovery Fetch** [Informational] — The theme recovery fetch lacks a timeout, which could leave the request hanging indefinitely under adverse network conditions (ASVS 13.1.3).
5. **Templated link rendering lacks URL-scheme sanitization** [Informational] — Templated link rendering does not sanitize URL schemes, allowing potentially unsafe scheme values to be rendered (ASVS 15.2.5).

### Positive Controls Observed

- **nvd3 chart tooltip sanitization** — nvd3 chart utilities run every tooltip string through `dompurify.sanitize`, including `wrapTooltip` (`legacy-preset-chart-nvd3/src/utils.ts`).
- **Server-side input validation enforcement** — Client-side input validators (e.g., `validateMaxValue`) are UX-only; authoritative validation is re-enforced server-side, consistent with the documented backend-as-enforcement boundary.
- **Password change functionality** — Implemented and accessible to users (ASVS 6.2.2 Pass).
- **Unrestricted password composition** — Any character types permitted without restrictive character-class rules (ASVS 6.2.5 Pass).
- **Masked password entry** — Password input fields use `type=password` (ASVS 6.2.6 Pass).
- **Paste and password manager support** — Permitted in password fields (ASVS 6.2.7 Pass).
- **Verbatim password verification** — Passwords verified exactly as received, without truncation or case transformation (ASVS 6.2.8 Pass).
- **Long password support** — Passwords of at least 64 characters permitted (ASVS 6.2.9 Pass).
- **No username enumeration** — Authentication responses do not reveal valid usernames via timing, error messages, or response codes (ASVS 6.3.8 Pass).
- **No password hints / KBA** — Password hints and secret questions are not present (ASVS 6.4.2 Pass).
- **Trusted backend session verification** — Session token verification performed via trusted backend service (ASVS 7.2.1 Pass).
- **Dynamically generated session tokens** — Application uses dynamically generated tokens, not static API secrets (ASVS 7.2.2 Pass).
- **Accessible logout** — Logout functionality easily accessible on authenticated pages (ASVS 7.4.4 Pass).
- **Consent-based session creation** — Session creation requires user consent or explicit action (ASVS 7.6.2 Pass).
- **DOM clobbering protection** — Properly implemented (ASVS 3.2.3 Pass).
- **Safe URL-scheme handling** — URL-construction helpers refuse to treat `javascript:`/`data:` schemes as absolute URLs via `SAFE_ABSOLUTE_URL_RE` in `pathUtils.ts`.
- **Centralized client-storage key registry** — `LocalStorageKeys` enables consistent classification of persisted items (ASVS 14.1.1).
- **HTTPS-restricted, revalidated client cache** — Cache API restricted to HTTPS contexts and revalidated via ETag/If-None-Match (ASVS 14.2.2, 14.3.2).
- **Persisted state minimization** — Allow-list field projection and age/size eviction of persisted SQL Lab state (ASVS 14.2.4).
- **No secrets in browser storage** — No tokens/credentials/secrets persisted to browser storage (ASVS 14.3.3).
- **Cache minimization** — Strong allow-list minimization and HTTPS-only caching enforced (ASVS 14.3.2, 14.3.3).
- **One-time API key handling** — One-time API key display with explicit state erasure on modal close; list view holds only `key_prefix`; credential clearing on GSheets public-mode switch.
- **Server-side log identity attachment** — User identity on log records attached server-side from the authenticated session, not from client payload (ASVS 16.2.1).
- **Guest token in request body** — Guest token transmitted in FormData body, not the query string, avoiding capture by reverse-proxy access logs (ASVS 16.2.5).
- **Safe error normalization** — `getClientErrorObject` maps server HTML error pages and status codes to generic user-safe messages, never echoing backend internals (ASVS 16.5.1).
- **Trust-model scoped sandboxedEval** — deck.gl `sandboxedEval` JS hooks execute trusted author-supplied config; client-only resource exhaustion is out of scope per the trust model (ASVS 15.2.2).
- **Build-time dev artifact exclusion** — Test/story exclusion, data-test stripping, dev-only DevTools gating, and tree-shaking keep dev/POC functionality out of production bundles (ASVS 15.2.3).
- **Optional version disclosure fields** — `version_string`, `version_sha`, and `build_number` are optional in the bootstrap type contract, allowing hardened backends to omit them.
...
#### FINDING-001: Unsanitized data interpolated into Datamaps popup HTML in WorldMap chart
...
| **CWE** | CWE-79 |
| **ASVS sections** | 1.3.1 |
| **Files** | superset-frontend/plugins/legacy-plugin-chart-world-map/src/WorldMap.ts |
| **Source Reports** | 1.3.1.md |
| **Related** | FINDING-003, FINDING-005 |

**Description:**

Location: superset-frontend/plugins/legacy-plugin-chart-world-map/src/WorldMap.ts, inside the new Datamap({...}) configuration — geographyConfig.popupTemplate and bubblesConfig.popupTemplate. Data flow: Query result row → mapData[country].name (d.name) → string interpolation into an HTML template → Datamaps renders the returned string as innerHTML for the hover tooltip → no sanitization applied. Unlike the sibling nvd3 chart utilities (legacy-preset-chart-nvd3/src/utils.ts), which run every tooltip string through dompurify.sanitize(...) (including a belt-and-braces wrapTooltip), the WorldMap popup templates interpolate the name field (an entity/dimension value sourced from the dataset) directly into markup that Datamaps injects into the DOM. This is the encoding boundary the frontend owns (per the project's frontend-is-the-boundary guidance for XSS), so backend parameterization does not mitigate it. Attacker capability required: Authenticated user able to influence a dimension value rendered as the entity of a World Map chart (e.g., a dataset/column whose values contain markup such as &lt;img src=x onerror=alert(document.cookie)&gt;). The payload then executes in the browser of any user who hovers the chart. Impact on success: Stored cross-site scripting in the context of other Superset users viewing the dashboard/chart — session/credential theft, CSRF-token exfiltration, action-on-behalf.

**Remediation:**

Sanitize the interpolated content before returning it, mirroring the nvd3 utilities: import { sanitizeHtml } from '@superset-ui/core'; popupTemplate: (geo, d) => d && sanitizeHtml(`<div class="hoverinfo"><strong>${d.name}</strong><br>${formatter(d.m1)}</div>`). Apply the same to bubblesConfig.popupTemplate. Sanitizing the whole template string (rather than only d.name) protects future template edits as well.

---

#### FINDING-002: postMessage origin validation fails open when allowedDomains is empty/undefined
...
| **ASVS Level(s)** | L1, L2 |
| **CWE** | CWE-346 |
| **ASVS sections** | 3.5.1, 3.5.5 |
| **Files** | superset-frontend/src/embedded/originValidation.ts |
| **Source Reports** | 3.5.1.md, 3.5.5.md |
| **Related** | |

**Description:**

The embedded Superset frame's postMessage origin validation short-circuits to allow-all when the allowedDomains configuration is empty or undefined. Data flow: event.origin (attacker-controlled via any embedding page) -> validateMessageEvent -> isMessageOriginAllowed -> returns true when allowedDomains is empty/undefined -> __embedded_comms__ handshake is accepted from any origin. Attacker capability: embed the Superset dashboard/chart in an iframe from a malicious page and post a structurally valid handshake message. Impact: establishes the embedded-comms control channel from an untrusted origin. While the backend still enforces guest-token authentication and RLS/field-level access controls before returning data, this represents a defense-in-depth control bypass. The fail-open default provides zero origin protection when operators deploy embeds without configuring allowedDomains. Rated Medium because exploitation requires operator misconfiguration (not populating the allow-list) and the data path remains backend-gated.

**Remediation:**

Make secure behavior the default: deny postMessage communication when no allowedDomains list is configured (fail-closed). Require an explicit, clearly-named configuration flag (e.g., 'allowUnrestrictedOrigins: true') to enable the legacy 'any origin' behavior, rather than inferring it from an empty configuration. Document the security implications of the unrestricted mode prominently.

### 3.4 Low

#### FINDING-003: SafeMarkdown disables react-markdown's built-in link-URI sanitization when EscapeMarkdownHtml flag is enabled
...
| **Related Finding(s)** | FINDING-001, FINDING-005 |

**Description:**

The SafeMarkdown component in `superset-frontend/packages/superset-ui-core/src/components/SafeMarkdown/SafeMarkdown.tsx` disables react-markdown's built-in link-URI sanitization by setting `transformLinkUri={null}`. Data flow: Markdown source → react-markdown converts markdown link syntax `[x](javascript:...)` to an `<a href>` node → with `transformLinkUri={null}`, react-markdown does not sanitize the href → the href is only neutralized if rehype-sanitize runs (which it does only when `escapeHtml === false`). In the default configuration (EscapeMarkdownHtml flag off), rehype-sanitize runs with defaultSchema, whose protocols.href allowlist strips `javascript:`/`data:` hrefs, making the path safe. However, when the EscapeMarkdownHtml feature flag is enabled, `rehypePlugins` is empty (no rehype-raw, no rehype-sanitize), so a markdown link with a `javascript:` URI is rendered as a clickable link with the dangerous href intact. This creates a foot-gun scenario where enabling an ostensibly hardening flag removes a sanitization layer. Attacker capability required: Ability to supply markdown source rendered through SafeMarkdown (markdown dashboard component / Handlebars output), in a deployment with EscapeMarkdownHtml enabled. Requires authenticated content-editing capability and a non-default flag. Impact on success: Reflected/stored DOM XSS via `javascript:` link execution on user click (user-interaction required).

**Remediation:**

Do not disable link sanitization. Provide an explicit `transformLinkUri` that allowlists safe schemes (e.g., wrap @braintree/sanitize-url's `sanitizeUrl`), independent of the escapeHtml/htmlSanitization branches: `import { sanitizeUrl } from '@braintree/sanitize-url'; <ReactMarkdown ... transformLinkUri={uri => sanitizeUrl(uri)}>`. This ensures link URIs are sanitized in all flag combinations rather than relying on rehype-sanitize being present.

### 3.5 Informational

#### FINDING-004: Missing Timeout on Theme Recovery Fetch
...
| **File(s)** | superset-frontend/src/theme/ThemeController.ts |
| **Source Report(s)** | 13.1.3.md |
| **Related** | None |

**Description:**

Theme-recovery fallback uses raw fetch() without timeout/AbortController, inconsistent with the deliberate timeout pattern used elsewhere (e.g., language-pack fetch with 5s timeout). If the theme API stalls, recovery promise remains pending indefinitely. No attacker capability (requires server hang), no C/I/A impact (UX degradation in fallback path only).

**Remediation:**

Add AbortController + setTimeout (mirror preamble.ts 5s pattern) around the fetch in fetchSystemDefaultTheme, clearing the timeout in a finally block.

---

#### FINDING-005: Templated link rendering lacks URL-scheme sanitization
...
| **File(s)** | superset-frontend/src/visualizations/TimeTable/components/LeftCell/LeftCell.tsx |
| **Source Report(s)** | 15.2.5.md |
| **Related** | FINDING-001, FINDING-003 |

**Description:**

Chart-author-configured URL template is rendered via `Mustache.render` directly into `<a href>` without passing through `@braintree/sanitize-url` (`sanitizeUrl`), which the codebase applies elsewhere (e.g., `navigationUtils.ts`). A template resolving to `javascript:`/`data:` would be placed directly into the anchor `href`. Attacker capability required: chart-edit rights. Impact: potential script execution on click.
...
const fullUrl = useMemo(
  () => (url ? sanitizeUrl(Mustache.render(url, { metric: row })) : undefined),
  [url, row],
);
```
...
|--------|---------|-----------------|------------------|
| Input Validation And Sanitization | nvd3 chart utilities use dompurify.sanitize for tooltip strings | legacy-preset-chart-nvd3/src/utils.ts runs every tooltip string through dompurify.sanitize including wrapTooltip | legacy-preset-chart-nvd3/src/utils.ts |
| Input Validation And Sanitization | Client-side input validators (validateMaxValue, etc.) are UX-only; authoritative input validation is re-enforced server-side, consistent with the documented backend-as-enforcement boundary. | Dropped finding ASVS-221-LOW-001 | — |
| Authentication And Session Management | Password change functionality is implemented and accessible to users | ASVS 6.2.2 marked as Pass | — |
| Authentication And Session Management | Password composition allows any character types without restrictive rules on character class requirements | ASVS 6.2.5 marked as Pass | — |
| Authentication And Session Management | Password input fields use type=password to mask entry | ASVS 6.2.6 marked as Pass | — |
| Authentication And Session Management | Paste functionality and password managers are permitted in password fields | ASVS 6.2.7 marked as Pass | — |
| Authentication And Session Management | Passwords are verified exactly as received without truncation or case transformation | ASVS 6.2.8 marked as Pass | — |
| Authentication And Session Management | Passwords of at least 64 characters are permitted | ASVS 6.2.9 marked as Pass | — |
| Authentication And Session Management | Authentication responses do not reveal valid usernames through timing, error messages, or response codes | ASVS 6.3.8 marked as Pass | — |
| Authentication And Session Management | Password hints and knowledge-based authentication (secret questions) are not present | ASVS 6.4.2 marked as Pass | — |
| Authentication And Session Management | Session token verification performed using trusted backend service | ASVS section 7.2.1 marked as Pass | — |
| Authentication And Session Management | Application uses dynamically generated tokens for session management (not static API secrets) | ASVS section 7.2.2 marked as Pass | — |
| Authentication And Session Management | Logout functionality is easily accessible on authenticated pages | ASVS section 7.4.4 marked as Pass | — |
| Authentication And Session Management | Session creation requires user consent or explicit action | ASVS section 7.6.2 marked as Pass | — |
| Browser Security Headers | DOM clobbering protection is properly implemented | ASVS 3.2.3 verification passed | — |
| Browser Security Headers | URL-construction helpers refuse to treat javascript:/data: schemes as absolute URLs | SAFE_ABSOLUTE_URL_RE pattern in pathUtils.ts prevents scheme-based injection through path prefixing | pathUtils.ts |
| Data Handling And Storage | Centralized, namespaced client-storage key registry (LocalStorageKeys) enabling consistent classification of persisted items | Observed in data classification review (14.1.1) | — |
| Data Handling And Storage | Client Cache API restricted to HTTPS contexts and revalidated via ETag/If-None-Match | Observed in cache control implementation (14.2.2, 14.3.2) | — |
| Data Handling And Storage | Allow-list field projection and age/size eviction of persisted SQL Lab state | Observed in data retention mechanisms (14.2.4) | — |
| Data Handling And Storage | No tokens/credentials/secrets persisted to browser storage | Observed in client-side storage review (14.3.3) | — |
| Data Handling And Storage | Strong allow-list minimization and HTTPS-only caching enforced | Observed in cache configuration (14.3.2, 14.3.3) | — |
| Cryptography And Secrets | One-time API key display with explicit state erasure on modal close (setCreatedKey(null)); list view holds only key_prefix, never full key material; credential clearing on GSheets public-mode switch | Observed during audit, promoted from dropped finding ASVS-1172-INFO-001 | — |
| Logging And Monitoring | User-identity attachment on log records is performed server-side from the authenticated session (and resolved guest-token subject for embedded), not from client payload. | 16.2.1.md - Dropped finding ASVS-1621-INFO-001 | — |
| Logging And Monitoring | Guest token is transmitted in the request body (FormData), not the query string, avoiding capture by reverse-proxy access logs | 16.2.5.md - Dropped finding ASVS-1625-INFO-001 | — |
| Logging And Monitoring | Error normalization (getClientErrorObject) maps server HTML error pages and status codes to generic user-safe messages and never echoes backend internals | 16.5.1.md - Promoted from dropped finding ASVS-1651-INFO-001 | — |
| Dependency Management | deck.gl sandboxedEval JS hooks (js_data_mutator/js_tooltip) execute trusted author-supplied config; unbounded execution causing client-side-only browser slowdown is out of scope per trust model and resource-exhaustion carve-out. | source: Dropped finding ASVS-1522-INFO-001 | — |
| Dependency Management | Test/story exclusion, data-test stripping, dev-only DevTools gating, and tree-shaking keep dev/POC functionality out of production bundles; dependency-integrity enforcement is delegated to the build/deployment pipeline. | Promoted from dropped finding ASVS-1523-LOW-001 | — |
| Configuration And Deployment | Version/build fields (version_string, version_sha, build_number) are optional in the bootstrap type contract, allowing a hardened backend to omit them | Version disclosure is a documented out-of-scope hardening concern, not a vulnerability. This design allows production deployments to avoid exposing detailed version information of backend components. | — |
...
| 1.3.5 | Verify that the application sanitizes or disables user-supplied scriptable or expression template language content, such as Markdown, CSS or XSL stylesheets, BBCode, or similar. | **Fail** | See FINDING-003 |
...
| 3.5.1 | Verify that, if the application does not rely on the CORS preflight mechanism to prevent disallowed cross-origin requests to use sensitive functionality, these requests are validated to ensure they originate from the application itself. This may be done by using and validating anti-forgery tokens or requiring extra HTTP header fields that are not CORS-safelisted request-header fields. This is to defend against browser-based request forgery attacks, commonly known as cross-site request forgery (CSRF). | **Fail** | See FINDING-002 |
...
| 3.5.5 | Verify that messages received by the postMessage interface are discarded if the origin of the message is not trusted, or if the syntax of the message is invalid. | **Fail** | See FINDING-002 |
...
| 15.2.5 | Verify that the application implements additional protections around parts of the application which are documented as containing "dangerous functionality" or using third-party libraries considered to be "risky components". This could include techniques such as sandboxing, encapsulation, containerization or network level isolation to delay and deter attackers who compromise one part of an application from pivoting elsewhere in the application. | **Partial** | See FINDING-005 |
...
**Summary Statistics:**
- **Pass**: 62 requirements (18.0%)
- **Partial**: 2 requirements (0.6%)
- **N/A**: 277 requirements (80.3%)
- **Fail**: 4 requirements (1.2%)
...
|------------|----------|-------------------|------------------|---------------------|
| FINDING-001 | Medium | 1.3.1 | FINDING-003, FINDING-005 | superset-frontend/plugins/legacy-plugin-chart-world-map/src/WorldMap.ts |
| FINDING-002 | Medium | 3.5.1, 3.5.5 | — | superset-frontend/src/embedded/originValidation.ts |
| FINDING-003 | Low | 1.3.5 | FINDING-001, FINDING-005 | superset-frontend/packages/superset-ui-core/src/components/SafeMarkdown/SafeMarkdown.tsx |
| FINDING-004 | Informational | 13.1.3 | — | superset-frontend/src/theme/ThemeController.ts |
| FINDING-005 | Informational | 15.2.5 | FINDING-001, FINDING-003 | superset-frontend/src/visualizations/TimeTable/components/LeftCell/LeftCell.tsx |

**Total Unique Findings**: 5 (0 Critical, 0 High, 2 Medium, 1 Low, 2 Info)

*3 of 5 are actionable. Informational findings are recorded here but not opened as GitHub issues; see issues.md for the 3 actionable items.*
