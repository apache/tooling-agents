...
This audit assessed the apache/airflow/providers/google codebase against ASVS Level 3 (L3) requirements, with the audit scope spanning all reviewed security domains up to L3. All findings were included regardless of severity, with no minimum threshold applied.

### Top 5 Risks

1. **User identification relies on reassignable email claim instead of immutable iss+sub** [Medium] — Identity binding uses a mutable email claim rather than the immutable issuer/subject pair, allowing potential account confusion if an email is reassigned (ASVS 10.3.3).
2. **_CredentialsToken.refresh hardcodes 3600s token lifetime ignoring actual credential expiry** [Low] — Token refresh logic assumes a fixed 3600-second lifetime instead of reading the credential's true expiry, risking stale or prematurely trusted tokens (ASVS 13.3.4).
3. **Privileged-by-default SSH login user (user="root") in ComputeEngineSSHHook** [Low] — The SSH hook defaults to the root account, violating least-privilege principles for remote access (ASVS 6.3.2).

### Positive Controls Observed

- **Bearer-token validation model is intentional**; transport confidentiality delegated to Deployment Manager and proof-of-possession (mTLS/DPoP) delegated to concrete AuthManager implementation (gcp_credential_management).
- **Per-connection OAuth scope narrowing** is supported and honored; impersonation chains, Workload Identity Federation, anonymous credentials, and quota-project separation provide least-privilege options (gcp_credential_management).
- **JWT signature validation** — Self-contained tokens are validated using digital signatures to protect against tampering (ASVS 9.1.1, gcp_credential_management).
- **Algorithm allowlist enforcement** — Token algorithms are restricted to an allowlist, preventing use of the 'None' algorithm (ASVS 9.1.2, gcp_credential_management).
- **Trusted key source validation** — Key material for token validation comes from trusted pre-configured sources, preventing attacker-controlled key injection (ASVS 9.1.3, gcp_credential_management).
- **Temporal token validation** — Token validity time spans (nbf, exp claims) are verified before accepting tokens (ASVS 9.2.1, gcp_credential_management).
- **Token type validation** — Ensures correct token types are used for their intended purpose (access tokens vs ID tokens) (ASVS 9.2.2, gcp_credential_management).
- **Audience restriction validation** — Token audience (aud claim) is validated against a service-specific allowlist (ASVS 9.2.3, gcp_credential_management).
- **Documented cryptographic key management policy** following key management standards (ASVS 11.1.1, secret_manager_integration).
- **Cryptographic inventory maintained** and regularly updated, including all keys, algorithms, and certificates with documented usage boundaries (ASVS 11.1.2, secret_manager_integration).
- **Sensitive data identified and classified** into protection levels (ASVS 14.1.1, secret_manager_integration).
- **Documented protection requirements** for all sensitive data protection levels, including encryption, integrity verification, retention, logging, and access controls (ASVS 14.1.2, secret_manager_integration).
- **Encryption, integrity, and access controls implemented** as documented (ASVS 14.2.4, secret_manager_integration).
- **Outbound DB transport encryption** is a deployment-managed configuration choice; the hook exposes Cloud SQL Auth Proxy (encrypted), public-IP SSL, and plaintext options (ASVS 12.3.1, cloud_sql_proxy_connections).
- **Outbound DB SSL trust anchoring** to a caller-supplied server CA with no verification-disabling code paths (ASVS 12.3.2, cloud_sql_proxy_connections).
- **File size limits enforced** to prevent denial of service attacks (ASVS 5.2.1, gcs_storage_security).
- **Safe file path handling** — File paths are generated internally or properly validated to prevent path traversal (ASVS 5.3.2, gcs_storage_security).
- **Session token verification** performed using a trusted backend service (ASVS 7.2.1, kubernetes_engine_security).
- **Dynamic session tokens used** (not static API secrets) (ASVS 7.2.2, kubernetes_engine_security).
- **Benign, non-backtracking regex patterns** — Author-controlled regex patterns within the trusted boundary avoid ReDoS (input_validation_sanitization).
- **Structural input validation** — Type enforcement, non-emptiness checks, and custom-validation exception wrapping over trusted-author bodies, with GCP server-side validation as authoritative backstop (ASVS 1.3.3, input_validation_sanitization).
...
#### FINDING-001: User identification relies on reassignable email claim instead of immutable iss+sub
...
| Files | providers/google/src/airflow/providers/google/common/auth_backend/google_openid.py:75-94, providers/google/src/airflow/providers/google/common/auth_backend/google_openid.py:96-106, providers/google/src/airflow/providers/google/common/auth_backend/google_openid.py:115-144 |
| Source Reports | 10.3.3.md |
| Related | — |

**Description:**

The Google OpenID authentication backend in `google_openid.py` identifies Airflow users by matching the `email` claim from validated ID tokens against the user database. The immutable `sub` (subject) claim, which uniquely identifies the Google identity and cannot be reassigned, is never consulted. Email addresses are mutable and can be reassigned across different Google identities (e.g., through corporate email reassignment, domain recycling, or shared-audience email reuse). An attacker who obtains a valid Google-issued ID token for the configured OAuth2 audience, whose verified `email` matches a target Airflow user's email, can authenticate as that user even though the underlying Google identity (`sub`) is different. This enables account takeover within the Airflow API surface protected by this authentication backend, with privileges bounded by the target user's roles (including Admin).

**Remediation:**

Bind Airflow user identities to the immutable `iss`+`sub` pair instead of the `email` claim. Store the Google `sub` claim on user records and match federated identities using (iss, sub) as the composite key. If backward compatibility requires email-based matching, document that deployments MUST guarantee email addresses are never reassigned, and strongly recommend migrating to `sub`-based binding for new deployments.

---

### 3.4 Low

#### FINDING-002: 🔵 _CredentialsToken.refresh hardcodes 3600s token lifetime ignoring actual credential expiry
...
| **ASVS Level(s)** | L3 |
| **CWE** | CWE-613 |
| **ASVS Section(s)** | 13.3.4 |
| **File(s)** | providers/google/src/airflow/providers/google/common/hooks/base_google.py |
| **Source Report(s)** | 13.3.4.md |
| **Related** | None |

**Description:**

The `_CredentialsToken.refresh` method in `base_google.py` hardcodes `access_token_duration = 3600` seconds regardless of the underlying credential's actual expiry time. For short-lived credentials (notably impersonated credentials with expiry well under 3600s), this causes incorrect token-validity bookkeeping. The `ensure_token` method will not attempt to refresh the token until 1800s have elapsed, potentially allowing the use of an already-expired token in async/deferrable trigger paths. This is a correctness defect in expiration handling, not an attacker-controlled vulnerability. The practical impact is intermittent authentication failures (availability) when using short-lived credentials; the token cannot actually outlive GCP's server-side expiry, so there is no confidentiality or integrity exposure.

**Remediation:**

Derive the token duration from the credential's actual `expiry` attribute. Example:
```python
...
#### FINDING-003: 🔵 Privileged-by-default SSH login user (user="root") in ComputeEngineSSHHook
...
| **File(s)** | providers/google/src/airflow/providers/google/cloud/hooks/compute_ssh.py |
| **Source Report(s)** | 6.3.2.md |
| **Related** | None |

**Description:**

The ComputeEngineSSHHook constructor defaults to `user="root"`, which is used directly as the SSH `username` in `_connect_to_instance` when `use_oslogin=False`. This results in SSH connections being opened as `root` on the remote GCE instance. While not externally exploitable and requiring a DAG author (a trusted identity) to instantiate the hook without specifying `user` and connect to an instance that permits root SSH via metadata keys, this represents a privileged-by-default configuration. When the metadata-key flow is used and the target instance allows it, connections default to the most privileged account (`root`), maximizing the blast radius of any compromised ephemeral key or command. This is a privileged-by-default foot-gun rather than a shipped/enabled default account. The application does not create or ship a root/admin/sa account — root here is only the default target login name for the remote instance.

**Remediation:**

Default to a non-privileged user or require the user to be specified explicitly (e.g., `user: str | None = None` and raise if not set for the metadata-key path). At minimum, document the privilege implication prominently and recommend a dedicated low-privilege account for SSH connections.
...
|--------|---------|-----------------|------------------|
| Gcp Credential Management | Bearer-token validation model is intentional; transport confidentiality delegated to Deployment Manager and proof-of-possession (mTLS/DPoP) delegated to concrete AuthManager implementation | 10.3.5.md - dropped finding ASVS-1035-LOW-001 | — |
| Gcp Credential Management | Per-connection OAuth scope narrowing is supported and honored; impersonation chains, Workload Identity Federation, anonymous credentials, and quota-project separation provide least-privilege options | 13.3.2.md - promoted from dropped finding ASVS-1332-LOW-001 | — |
| Gcp Credential Management | Self-contained tokens (JWTs) are validated using digital signatures to protect against tampering | ASVS 9.1.1 passed - token signature validation implemented | — |
| Gcp Credential Management | Token algorithms are restricted to an allowlist, preventing use of 'None' algorithm | ASVS 9.1.2 passed - algorithm allowlist enforcement in place | — |
| Gcp Credential Management | Key material for token validation comes from trusted pre-configured sources, preventing attacker-controlled key injection | ASVS 9.1.3 passed - trusted key source validation implemented | — |
| Gcp Credential Management | Token validity time spans (nbf, exp claims) are verified before accepting tokens | ASVS 9.2.1 passed - temporal validation of tokens enforced | — |
| Gcp Credential Management | Token type validation ensures correct token types are used for their intended purpose (access tokens vs ID tokens) | ASVS 9.2.2 passed - token type verification implemented | — |
| Gcp Credential Management | Token audience (aud claim) is validated against service-specific allowlist | ASVS 9.2.3 passed - audience restriction validation in place | — |
| Secret Manager Integration | Documented cryptographic key management policy following key management standards | ASVS 11.1.1 passed - cryptographic key lifecycle and management policy in place | — |
| Secret Manager Integration | Cryptographic inventory maintained and regularly updated | ASVS 11.1.2 passed - cryptographic inventory includes all keys, algorithms, and certificates with documented usage boundaries | — |
| Secret Manager Integration | Sensitive data identified and classified into protection levels | ASVS 14.1.1 passed - all sensitive data created and processed by the application has been identified and classified | — |
| Secret Manager Integration | Documented protection requirements for all sensitive data protection levels | ASVS 14.1.2 passed - protection requirements documented including encryption, integrity verification, retention, logging, and access controls | — |
| Secret Manager Integration | Implementation of controls around sensitive data encryption, integrity, and access | ASVS 14.2.4 passed - controls for encryption, integrity verification, retention, logging, and access controls implemented as documented | — |
| Cloud Sql Proxy Connections | Outbound DB transport encryption is a deployment-managed configuration choice; the hook exposes Cloud SQL Auth Proxy (encrypted), public-IP SSL, and plaintext options, leaving transport selection to trusted deployment personnel. | 12.3.1.md - Promoted from dropped finding ASVS-1231-MED-001 | — |
| Cloud Sql Proxy Connections | Outbound DB SSL connections anchor trust to a caller-supplied server CA (sslrootcert/ca) with no verification-disabling code paths; certificate identity/hostname mode is left as a deployment-managed transport configuration choice. | 12.3.2.md - Promoted from dropped finding ASVS-1232-LOW-001 | — |
| Gcs Storage Security | File size limits are enforced to prevent denial of service attacks | ASVS 5.2.1 passed - application properly validates file sizes | — |
| Gcs Storage Security | File paths are generated internally or properly validated to prevent path traversal | ASVS 5.3.2 passed - application uses safe file path handling | — |
| Kubernetes Engine Security | Session token verification performed using trusted backend service | ASVS 7.2.1 marked as Pass | — |
| Kubernetes Engine Security | Dynamic session tokens used (not static API secrets) | ASVS 7.2.2 marked as Pass | — |
| Input Validation Sanitization | Regex validation uses benign, non-backtracking shipped/example patterns (^.+$, [^a-z]); patterns are author-controlled within the trusted boundary. | source: Dropped finding ASVS-1312-LOW-001 | — |
| Input Validation Sanitization | Type enforcement (dict/list/union), non-emptiness checks, and contained custom-validation exception wrapping provide structural validation as a developer aid over trusted-author bodies, with GCP server-side validation as authoritative backstop. | Observed in ASVS 1.3.3 audit (dropped finding ASVS-133-LOW-001) | — |
...
| 6.3.2 | Verify that default user accounts (e.g., "root", "admin", or "sa") are not present in the application or are disabled. | **Partial** | See FINDING-003 |
...
| 10.3.3 | Verify that if an access control decision requires identifying a unique user from an access token (JWT or related token introspection response), the resource server identifies the user from claims that cannot be reassigned to other users. Typically, it means using a combination of 'iss' and 'sub' claims. | **Fail** | See FINDING-001 |
...
| 13.3.4 | Verify that secrets are configured to expire and be rotated based on the application's documentation. | **Partial** | See FINDING-002 |
...
|------------|----------|-------------------|------------------|---------------------|
| FINDING-001 | Medium | 10.3.3 | — | providers/google/src/airflow/providers/google/common/auth_backend/google_openid.py, providers/google/src/airflow/providers/google/common/auth_backend/google_openid.py, providers/google/src/airflow/providers/google/common/auth_backend/google_openid.py |
| FINDING-002 | Low | 13.3.4 | — | providers/google/src/airflow/providers/google/common/hooks/base_google.py |
| FINDING-003 | Low | 6.3.2 | — | providers/google/src/airflow/providers/google/cloud/hooks/compute_ssh.py |
