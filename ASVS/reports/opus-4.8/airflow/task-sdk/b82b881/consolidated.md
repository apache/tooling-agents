...
*Informational findings are recorded in this report but not opened as GitHub issues — see issues.md for the 6 actionable items.*
...
This engagement was assessed against ASVS Level 3 (L3), the most rigorous verification tier, with all findings included regardless of severity threshold. Coverage spanned 26 security domains across the task-sdk, and the majority of in-scope controls passed; the residual findings are concentrated in concurrency safety, IPC state handling, and HTTP client hardening.

### Top 5 Risks

1. **Process-wide binary-digest LRU cache mutated without synchronization** [Medium] — A shared LRU cache holding binary digests is mutated without locking, exposing the process to data races under concurrent access (ASVS 15.4.1).
2. **Coordinator instance cache is check-then-act racy** [Low] — The coordinator instance cache uses a check-then-act pattern that can permit duplicate instantiation under concurrent calls (ASVS 15.4.1).
3. **Terminal-state IPC send failure can be misclassified as SUCCESS** [Low] — Failures sending SKIPPED / UP_FOR_RESCHEDULE / DEFERRED terminal states over IPC may be silently recorded as SUCCESS, corrupting task outcome fidelity (ASVS 16.5.3).
4. **Execution API HTTP client lacks explicit connection-pool limits in all paths** [Low] — Not all client construction paths apply an explicit connection-pool / max-parallel-connections limit, leaving resource bounds implicit (ASVS 13.2.6).
5. **Direct-database-access control raises but does not log bypass attempt** [Low] — The direct-database-access security control rejects the bypass but does not emit a log record, reducing detection and forensic visibility (ASVS 16.3.3).

### Positive Controls Observed

- **Artifact integrity delegated to trusted deployer** — Integrity of deployed bundle/JAR artifacts on disk is delegated to the trusted deployment-packaging layer; write access to `executables_root`/`jars_root` is a privileged trusted actor (coordinator_subprocess_bridge).
- **Function-level access control** — Application restricts function-level access to consumers with explicit permissions; enforced at the trusted service layer (ASVS 8.2.1, authorization_and_access_control).
- **Data-specific access control (IDOR/BOLA)** — Application restricts data-specific access to consumers with explicit permissions to specific data items (ASVS 8.2.2, authorization_and_access_control).
- **Authorization at trusted service layer** — Authorization rules enforced server-side without relying on client-side controls (ASVS 8.3.1, authorization_and_access_control).
- **Originating-subject authorization** — Access decisions respect the originating subject's permissions, not intermediary service permissions (ASVS 8.3.3, authorization_and_access_control).
- **Multi-tenant cross-tenant isolation** — Cross-tenant controls prevent unauthorized cross-tenant operations (ASVS 8.4.1, authorization_and_access_control).
- **TLS version configuration** — Only recommended TLS 1.2 / 1.3 versions are in use (ASVS 12.1.1, http_security_headers).
- **Strong cipher suites** — Recommended cipher suites are enabled with appropriate preferences (ASVS 12.1.2, http_security_headers).
- **TLS enforced for external connectivity** — No insecure HTTP fallback detected (ASVS 12.2.1, http_security_headers).
- **Publicly trusted TLS certificates** — Certificate trust validation successful for external-facing services (ASVS 12.2.2, http_security_headers).
- **Server-side authoritative input validation** — SDK response models populated by trusted Execution API server; authoritative validation enforced server-side (input_validation_and_encoding).
- **Injection defenses** — Protections verified against OS command injection (1.2.5), dynamic code execution (1.3.2), format string injection (1.3.10), mail injection (1.3.11), template/expression injection (1.3.5, 1.3.7), SSRF (1.3.6), and context-appropriate sanitization (1.3.3) (injection_prevention).
- **Structured log encoding** — Primary task-log path uses JSON structured encoding (`json_logs=True`, `structlog.BytesLogger`), neutralizing log injection on the in-scope path (logging_and_monitoring).
- **Mass assignment protection** — Explicit schema validation provides countermeasures against mass assignment (ASVS 15.3.3, advanced_http_security).
- **Type safety and strict comparisons** — Correct variable typing and strict equality operations enforced (ASVS 15.3.5, advanced_http_security).
- **HTTP parameter pollution defenses** — Defenses against HTTP parameter pollution in place (ASVS 15.3.7, advanced_http_security).
...
#### FINDING-001: Process-wide binary-digest LRU cache mutated without synchronization
...
| **Files** | task-sdk/src/airflow/sdk/coordinators/executable/coordinator.py |
| **Source Reports** | 15.4.1.md |
| **Related** | FINDING-002 |

**Description:**

Multiple supervisor task threads concurrently call `_read_bundle_metadata`, which performs unsynchronized get/put operations on a shared process-wide `_BinaryDigestCache` OrderedDict. Interleaving a reader's `move_to_end` with another thread's `popitem`/eviction loop can raise `RuntimeError`/`KeyError`, corrupt LRU ordering, or cause over-/under-eviction. The worst realistic outcome is an unhandled exception aborting a task's bundle resolution (local DoS) or inconsistent cache state. This is not attacker-driven; it is a correctness/availability defect triggered by concurrency the code itself anticipates, and the documented domain model requires coordinator shared caches to be lock-protected. There is no integrity bypass, as a cache hit still returns a digest compared against `footer.binary_sha256`.

**Remediation:**

Add a `threading.Lock` around `_BinaryDigestCache.get`/`put`/`clear` so all mutations of the shared OrderedDict are serialized. Ensure the lock is held for the entire duration of cache operations including LRU reordering and eviction logic to prevent race conditions between concurrent threads accessing the cache.

---

### 3.4 Low

#### FINDING-002: Coordinator instance cache is check-then-act racy, allowing duplicate instantiation
...
| **File(s)** | task-sdk/src/airflow/sdk/execution_time/coordinator.py |
| **Source Report(s)** | 15.4.1.md |
| **Related** | FINDING-001 |

**Description:**

CoordinatorManager._find_queue uses a check-then-act pattern on the shared _created_coordinators dict, so concurrent for_queue callers can both miss the cache and both construct a coordinator; the second assignment overwrites the first. The dict assignment is atomic so no corruption occurs; the only effect is redundant construction (last-writer-wins) and two short-lived instances. No C/I/A impact, concurrency-only, not attacker-controlled.

**Remediation:**

Guard the read-miss-construct-store sequence with a lock so construction happens once.

---

#### FINDING-003: Direct-database-access security control raises but does not log the bypass attempt
...
| **File(s)** | task-sdk/src/airflow/sdk/execution_time/supervisor.py |
| **Source Report(s)** | 16.3.3.md |
| **Related** | - |

**Description:**

BlockedDBSession.__init__ / block_orm_access (configure_orm / __getattr__ replacements) in supervisor.py raise RuntimeError/AttributeError when task code attempts ORM/DB access, but no log statement records that a bypass was attempted. The control fails closed; the gap is that a security-relevant event (attempt to circumvent metadata-DB isolation) is not recorded, reducing forensic visibility if a misconfigured or compromised dependency probes the boundary. Actor is a DAG author within the trust boundary.

**Remediation:**

Emit a log line at the denial point before raising (using a fork-safe module logger guarded against logging recursion), e.g. log.warning("Blocked direct ORM session creation from task process").

---

#### FINDING-004: Terminal-state IPC send failure for SKIPPED / UP_FOR_RESCHEDULE / DEFERRED can be misclassified as SUCCESS
...
| **File(s)** | task-sdk/src/airflow/sdk/execution_time/task_runner.py, task-sdk/src/airflow/sdk/execution_time/supervisor.py |
| **Source Report(s)** | 16.5.3.md |
| **Related** | - |

**Description:**

In task_runner.py run() finally block, when SUPERVISOR_COMMS.send(msg) raises during terminal-state reporting, _terminal_state_send_failed is set only for FAILED/UP_FOR_RETRY. For SKIPPED/UP_FOR_RESCHEDULE/DEFERRED it is intentionally not fail-closed, so main() exits 0 and supervisor.final_state (exit_code==0, no _terminal_state) returns SUCCESS. Integrity issue: a task that should be DEFERRED/RESCHEDULED/SKIPPED is recorded as SUCCESS, potentially advancing downstream tasks on incorrect state. Triggered by infrastructure/IPC failure, not attacker-controlled.

**Remediation:**

Extend the fail-closed signal so any terminal-state send failure (not only FAILED/UP_FOR_RETRY) prevents a clean exit-0 SUCCESS classification, and/or have final_state distinguish "exit 0 with no acknowledged terminal state" from genuine success.

---

#### FINDING-005: Execution API HTTP client does not apply explicit connection-pool / max-parallel-connections limit in all construction paths
...
| **File(s)** | task-sdk/src/airflow/sdk/api/client.py, task-sdk/src/airflow/sdk/execution_time/supervisor.py |
| **Source Report(s)** | 13.2.6.md |
| **Related** | - |

**Description:**

Documented connection configuration (max parallel connections / behavior at limit) → Client.__init__ → no httpx.Limits applied in base Client.__init__, so httpx default pool (max_connections=100, max_keepalive=20) silently governs concurrency instead of deployment-documented value. The supervisor's _ensure_client does apply limits = httpx.Limits(max_keepalive_connections=1, max_connections=10), but other client construction sites (InProcessTestSupervisor._api_client, connection_test_supervisor) construct Client without passing limits, falling back to httpx defaults. No concrete exploit path exists (single-client-per-supervisor model, sane httpx defaults). This is a Level 3 documentation-alignment / defense-in-depth gap, not a C/I/A-affecting defect; timeouts and retry strategy are applied.

**Remediation:**

Surface configurable pool limit and apply it explicitly in Client.__init__ so the documented value is honored (e.g. read execution_api_max_connections / execution_api_max_keepalive from [workers]), or document explicitly that the Execution API client intentionally relies on httpx defaults and that concurrency is bounded by the supervisor model.

---

#### FINDING-006: Execution API client relies on httpx's implicit default to avoid following redirects rather than explicitly disabling it
...
| **File(s)** | task-sdk/src/airflow/sdk/api/client.py |
| **Source Report(s)** | 15.3.2.md |
| **Related** | - |

**Description:**

httpx.Client defaults to follow_redirects=False, so the client does not follow redirects in the current configuration and the requirement is satisfied in practice. The gap is defense-in-depth: the safe behavior is never asserted explicitly, and the constructor forwards arbitrary **kwargs to super().__init__(), so a future caller or kwargs default could silently enable follow_redirects=True. Because the Bearer auth token is attached via BearerAuth.auth_flow, a followed cross-origin redirect could leak the task JWT to an unintended host. Requires control/MITM of the Execution API server response or a future misconfigured kwargs default; not remotely exploitable in the default deployment.

**Remediation:**

Set the value explicitly so the safe posture is intentional and cannot be silently overridden: use kwargs.setdefault("follow_redirects", False), or pass follow_redirects=False explicitly and strip it from forwarded kwargs.

### 3.5 Informational

#### FINDING-007: Remote-logging connection cached for process lifetime with no TTL or invalidation
...
Module-level `_REMOTE_LOGGING_CONN_CACHE` in `_fetch_remote_logging_conn()` caches the deployment-managed remote-logging connection for the process lifetime with no expiry. A rotated/revoked remote-logging credential continues to be used until process exit. The Supervisor process is invoked per task instance and is short-lived, so the window is bounded by a single task run; this is not a value on which a consumer's authorization is decided. Recorded for completeness as a cache-without-invalidation code defect, not an exploitable gap.

**Remediation:**

Key the cache by `(conn_id, fetched_at)` and apply a short TTL, or drop the module-level cache entirely and rely on the per-call fetch that the surrounding `_remote_logging_conn` context manager already performs.
...
|--------|---------|-----------------|------------------|
| Coordinator Subprocess Bridge | Integrity of deployed bundle/JAR artifacts on disk is delegated to the trusted-deployer / deployment-packaging layer; write access to executables_root/jars_root is a privileged trusted actor permitted arbitrary code execution. | source: Dropped finding ASVS-1542-LOW-001, ASVS-1542-LOW-002 | — |
| Authorization And Access Control | Function-level access control enforced at trusted service layer | ASVS 8.2.1 passed - application restricts function-level access to consumers with explicit permissions | — |
| Authorization And Access Control | Data-specific access control prevents IDOR/BOLA | ASVS 8.2.2 passed - application restricts data-specific access to consumers with explicit permissions to specific data items | — |
| Authorization And Access Control | Authorization enforced at trusted service layer | ASVS 8.3.1 passed - application enforces authorization rules at trusted service layer without relying on client-side controls | — |
| Authorization And Access Control | Authorization decisions respect originating subject permissions | ASVS 8.3.3 passed - access to objects based on originating subject's permissions, not intermediary service permissions | — |
| Authorization And Access Control | Multi-tenant cross-tenant isolation | ASVS 8.4.1 passed - multi-tenant applications use cross-tenant controls to prevent unauthorized cross-tenant operations | — |
| Http Security Headers | TLS 1.2 and TLS 1.3 are properly configured and enabled | ASVS 12.1.1 passed - only recommended TLS versions are in use | — |
| Http Security Headers | Strong cipher suites are configured with appropriate preferences | ASVS 12.1.2 passed - recommended cipher suites are enabled | — |
| Http Security Headers | TLS is enforced for all external HTTP-based service connectivity | ASVS 12.2.1 passed - no insecure fallback detected | — |
| Http Security Headers | Publicly trusted TLS certificates are used for external facing services | ASVS 12.2.2 passed - certificate trust validation successful | — |
| Input Validation And Encoding | SDK response models are populated by the trusted Execution API server; authoritative input validation is enforced server-side at the trusted service layer. | Dropped finding ASVS-211-INFO-001 | — |
| Input Validation And Encoding | Free-form JSON payload fields are intentional functional requirements for trusted DAG-author data; authoritative size/structure validation is enforced at the trusted server layer. | Dropped finding ASVS-221-LOW-001 | — |
| Input Validation And Encoding | Combined-data-item (cross-field temporal) validation is authoritatively enforced server-side at the trusted Execution API layer; SDK client models are a serialization contract, not the enforcement point. | Dropped finding ASVS-223-LOW-001 | — |
| Injection Prevention | Regex patterns are only interpolated from DAG-author-controlled identifiers at parse time, a trusted-role/no-C-I-A surface accepted per the trust model. | ASVS 1.2.9 - Regex escaping not required for trusted DAG-author input | — |
| Injection Prevention | Interpolated regex patterns are sourced only from pre-validated DAG-author identifiers (trusted role); re.escape() future-proofing is treated as optional hardening, not a security defect. | ASVS 1.3.12 - ReDoS protection via trusted input source control | — |
| Injection Prevention | Application does not use OS command execution or uses parameterized queries/contextual encoding. | ASVS 1.2.5 - Pass status indicates protection against OS command injection | — |
| Injection Prevention | Format strings are properly sanitized before processing. | ASVS 1.3.10 - Pass status indicates format string injection protection | — |
| Injection Prevention | User input passed to mail systems is sanitized against SMTP/IMAP injection. | ASVS 1.3.11 - Pass status indicates mail injection protection | — |
| Injection Prevention | Application avoids eval() and dynamic code execution, or sanitizes input appropriately. | ASVS 1.3.2 - Pass status indicates protection against dynamic code execution vulnerabilities | — |
| Injection Prevention | Data passed to dangerous contexts is sanitized with appropriate safety measures. | ASVS 1.3.3 - Pass status indicates context-appropriate sanitization | — |
| Injection Prevention | Scriptable or expression template language content is sanitized or disabled. | ASVS 1.3.5 - Pass status indicates protection against template language injection | — |
| Injection Prevention | Application protects against SSRF by validating untrusted data against allowlists and sanitizing dangerous characters. | ASVS 1.3.6 - Pass status indicates SSRF protection measures in place | — |
| Injection Prevention | Templates are not built based on untrusted input, or untrusted input is sanitized/validated during template creation. | ASVS 1.3.7 - Pass status indicates template injection protection | — |
| Api Security | Request-building input (dag_id, run_ids, task_ids, variable prefixes) originates from trusted DAG authors and receiving server/reverse proxy enforces request-size limits; over-long-request DoS is out of scope per trust model. | Dropped finding ASVS-425-LOW-001 - mitigated by infrastructure controls and trust model | — |
| File Upload And Processing | Resource-exhaustion DoS via files placed in Deployment-Manager-controlled artifact roots (executables_root/jars_root) is out of scope; those roots are controlled by a fully trusted identity. | Dropped finding ASVS-521-MED-001 | — |
| File Upload And Processing | JAR handling reads only a single named manifest entry (no full extraction); zip-bomb risk from deployment-controlled jars_root is out of scope as it requires a trusted actor. | Dropped finding ASVS-523-LOW-001 | — |
| Logging And Monitoring | Primary task-log path uses JSON structured encoding (json_logs=True, structlog.BytesLogger) which encodes newlines/control characters, neutralizing log injection on the in-scope path. | source: Dropped finding ASVS-1641-LOW-001 | — |
| Configuration And Deployment | In-process vs. HSM/KMS-backed crypto is a deployment-tier decision; FERNET_KEY config knob and pluggable secrets backends (Vault/AWS/GCP) provide the override surface to keep key material in an external module. | Dropped finding (scope analysis 13.3.3) | — |
| Advanced Http Security | Mass assignment protection through explicit schema validation | ASVS 15.3.3 passed - application implements countermeasures against mass assignment attacks | — |
| Advanced Http Security | Type safety and strict comparisons enforced | ASVS 15.3.5 passed - application ensures correct variable types and performs strict equality operations | — |
| Advanced Http Security | HTTP parameter pollution defenses in place | ASVS 15.3.7 passed - application has defenses against HTTP parameter pollution attacks | — |
...
| 4.2.2 | Verify that when generating HTTP messages, the Content-Length header field does not conflict with the length of the content as determined by the framing of the HTTP protocol, in order to prevent request smuggling attacks. | **Fail** |  |
...
| 12.3.5 | Verify that services communicating internally within a system (intra-service communications) use strong authentication to ensure that each endpoint is verified. Strong authentication methods, such as TLS client authentication, must be employed to ensure identity, using public-key infrastructure and mechanisms that are resistant to replay attacks. For microservice architectures, consider using a service mesh to simplify certificate management and enhance security. | **Pass** |  |
| **V13: Configuration** | | | |
...
| 13.2.1 | Verify that communications between backend application components that don't support the application's standard user session mechanism, including APIs, middleware, and data layers, are authenticated. Authentication must use individual service accounts, short-term tokens, or certificate-based authentication and not unchanging credentials such as passwords, API keys, or shared accounts with privileged access. | **Fail** |  |
| 13.2.2 | Verify that communications between backend application components, including local or operating system services, APIs, middleware, and data layers, are performed with accounts assigned the least necessary privileges. | **Fail** |  |
...
| 13.2.6 | Verify that where the application connects to separate services, it follows the documented configuration for each connection, such as maximum parallel connections, behavior when maximum allowed connections is reached, connection timeouts, and retry strategies. | **Partial** | See FINDING-005 |
...
| 15.3.2 | Verify that where the application backend makes calls to external URLs, it is configured to not follow redirects unless it is intended functionality. | **Partial** | See FINDING-006 |
...
| 16.3.2 | Verify that failed authorization attempts are logged. For L3, this must include logging all authorization decisions, including logging when sensitive data is accessed (without logging the sensitive data itself). | **Partial** |  |
| 16.3.3 | Verify that the application logs the security events that are defined in the documentation and also logs attempts to bypass the security controls, such as input validation, business logic, and anti-automation. | **Partial** | See FINDING-003 |
...
| 16.5.3 | Verify that the application fails gracefully and securely, including when an exception occurs, preventing fail-open conditions such as processing a transaction despite errors resulting from validation logic. | **Partial** | See FINDING-004 |
...
**Summary Statistics:**
- **Pass**: 59 requirements (17.1%)
- **Partial**: 7 requirements (2.0%)
- **N/A**: 274 requirements (79.4%)
- **Fail**: 5 requirements (1.4%)
...
|------------|----------|-------------------|------------------|---------------------|
| FINDING-001 | Medium | 15.4.1 | FINDING-002 | task-sdk/src/airflow/sdk/coordinators/executable/coordinator.py |
| FINDING-002 | Low | 15.4.1 | FINDING-001 | task-sdk/src/airflow/sdk/execution_time/coordinator.py |
| FINDING-003 | Low | 16.3.3 | — | task-sdk/src/airflow/sdk/execution_time/supervisor.py |
| FINDING-004 | Low | 16.5.3 | — | task-sdk/src/airflow/sdk/execution_time/task_runner.py, task-sdk/src/airflow/sdk/execution_time/supervisor.py |
| FINDING-005 | Low | 13.2.6 | — | task-sdk/src/airflow/sdk/api/client.py, task-sdk/src/airflow/sdk/execution_time/supervisor.py |
| FINDING-006 | Low | 15.3.2 | — | task-sdk/src/airflow/sdk/api/client.py |
| FINDING-007 | Informational | 8.3.2 | — | task-sdk/src/airflow/sdk/execution_time/supervisor.py |

**Total Unique Findings**: 7 (0 Critical, 0 High, 1 Medium, 5 Low, 1 Info)

*6 of 7 are actionable. Informational findings are recorded here but not opened as GitHub issues; see issues.md for the 6 actionable items.*
