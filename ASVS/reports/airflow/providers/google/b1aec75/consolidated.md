...
The audit of the `apache/airflow/providers/google` repository at ASVS Level 3 depth produced **20 actionable findings** across 19 audited security domains. The overall security posture is strong: no critical or high severity issues were identified, and the single medium-severity finding relates to identity management rather than an actively exploitable remote vulnerability. The majority of findings (19 of 20) are low-severity, reflecting hardening opportunities and defense-in-depth gaps rather than immediate exploitation risks.
...
The distribution indicates that most findings surface at L2 depth, consistent with the provider's role as an integration layer that delegates many L1 security primitives to the GCP platform and Airflow core framework.

### Top 5 Risks

1. **[Medium] User identification based on `email` claim instead of immutable `sub` claim (FINDING-001)** — The Google OpenID authentication backend identifies users by their email address rather than the stable `sub` claim. If a Google Workspace administrator reassigns an email to a different person, the new holder inherits the original user's Airflow identity and permissions. This is the most impactful finding in the audit.

2. **[Low] GCS blob names used in local path construction without traversal validation (FINDING-005, FINDING-006)** — Two separate code paths (`sync_to_local_dir` and `GCSTimeSpanFileTransformOperator._download`) construct local filesystem paths directly from GCS blob names without verifying the resulting path remains within the intended directory. A maliciously crafted blob name could write outside the target directory.

3. **[Low] Credential file written without explicit restrictive permissions (FINDING-004)** — Temporary credential files are written using default umask-derived permissions rather than explicitly setting `0600`, potentially exposing secrets to other processes on shared worker nodes.

4. **[Low] User identity lookup by email alone without IdP namespace prefix (FINDING-003)** — Identity resolution does not namespace users by identity provider, creating a theoretical collision risk in multi-IdP deployments.

5. **[Low] Exception objects logged without sanitization may contain sensitive connection details (FINDING-007)** — Raw exception messages propagated to logs may inadvertently include connection strings, hostnames, or partial credentials embedded in error payloads from downstream services.

### Positive Controls

The audit identified **48 verified positive controls** demonstrating mature security practices across the provider codebase:

- **Authentication & Token Security** — OIDC audience validation, access token claim enforcement, and cryptographic signature verification are correctly implemented. MFA and authentication strength enforcement are appropriately delegated to Google Workspace admin policies.
- **Cryptographic Operations** — All 17 ASVS Chapter 11 requirements assessed passed verification, including authenticated encryption (AES-GCM), secure PRNG with ≥128-bit entropy, constant-time operations, crypto agility, and proper nonce/IV management.
- **Secrets Management** — Secrets are properly delegated to GCP Secret Manager with fail-closed error handling, IAM-based access control, and automatic rotation pickup via `latest` version retrieval.
- **Authorization & Access Control** — GCP IAM enforces resource-level authorization server-side on every API call. The `@fallback_to_default_project_id` decorator provides safe default scoping, and fresh `AuthorizedSession` instances are created per request.
- **Transport Security** — TLS verification is enabled by default; configuration is delegated to Deployment Manager. No insecure protocol downgrades or WebSocket attack surfaces exist.
- **Input Validation** — All operator/hook parameters originate from trusted DAG authors within the Airflow security model. `GcpBodyFieldValidator` provides structural validation, URL domain allowlisting is enforced where applicable, and regex patterns are developer-defined constants.
- **Architecture & Separation of Concerns** — Session management, rate limiting, and multi-tenant isolation are cleanly delegated to appropriate platform layers (Airflow core AuthManager, GCP API quotas, and GCP IAM respectively).
...
#### FINDING-001: User identification based on `email` claim instead of immutable `sub` claim enables account takeover upon email reassignment
...
| **Files** | providers/google/src/airflow/providers/google/common/auth_backend/google_openid.py |
| **Source Reports** | 10.3.3.md, 10.5.2.md |
| **Related** | ASVS-1033-MED-001, ASVS-1052-MED-001 |

**Description:**

The deprecated `google_openid.py` auth backend identifies users by the `email` claim from Google ID tokens instead of the immutable `sub` claim. If a Google Workspace admin reassigns an email address from a departing employee to a new employee, the new holder inherits the previous user's Airflow identity, roles, and permissions. The `sub` claim is available in the `id_info` dictionary but is never used for user identification. This violates OIDC best practices requiring use of claims that cannot be reassigned (typically `iss` + `sub` combination). While the module is deprecated (removal planned in v15.0.0), this represents a real risk until removed. Mitigating factors include: email_verified check, user must pre-exist in Airflow, is_active check, and requires organizational process failure.
...
#### FINDING-002: Differential HTTP Response Codes Enable Limited User Registration Enumeration in Google OpenID Auth Backend
...
| **Affected File(s)** | providers/google/src/airflow/providers/google/common/auth_backend/google_openid.py |
| **Source Report(s)** | 6.3.8.md |
| **Related Finding(s)** | - |

**Description:**

The `requires_authentication` decorator in `google_openid.py` returns HTTP 401 for invalid/missing tokens but HTTP 403 when a valid token's email is not found in the local user database. This differential response allows an attacker with a valid Google account to determine whether their email is registered in the Airflow instance. Exploitability is limited since attackers can only check their own email (tokens are bound to the account owner), and the module is deprecated.
...
#### FINDING-003: User Identity Lookup by Email Alone Without IdP Namespace Prefix
...
| **ASVS Level(s)** | L2 |
| **CWE** | CWE-290 |
| **ASVS Section(s)** | 6.8.1 |
| **Affected File(s)** | providers/google/src/airflow/providers/google/common/auth_backend/google_openid.py |
| **Source Report(s)** | 6.8.1.md |
| **Related Finding(s)** | - |

**Description:**

The `_lookup_user` function in `google_openid.py` resolves users by email alone without an IdP namespace prefix. In deployments with multiple auth backends sharing the same user store, a user authenticated by one IdP could potentially map to a user intended for another IdP. Mitigated by issuer validation and user pre-existence requirement.
...
#### FINDING-004: Credential File Written Without Explicit Restrictive Permissions
...
| **ASVS Level(s)** | L2 |
| **CWE** | CWE-732 |
| **ASVS Section(s)** | 2.2.3 |
| **Affected File(s)** | providers/google/src/airflow/providers/google/cloud/hooks/cloud_sql.py |
| **Source Report(s)** | 2.2.3.md |
| **Related Finding(s)** | - |
...
#### FINDING-005: GCS blob names used directly in local path construction without traversal validation in sync_to_local_dir
...
| **Affected File(s)** | providers/google/src/airflow/providers/google/cloud/hooks/gcs.py |
...
#### FINDING-006: GCS blob names used directly in local path construction without traversal validation in GCSTimeSpanFileTransformOperator._download
...
| **CWE** | CWE-22 |
| **ASVS Section(s)** | 5.3.2 |
| **Affected File(s)** | providers/google/src/airflow/providers/google/cloud/operators/gcs.py |
| **Source Report(s)** | 5.3.2.md |
| **Related Finding(s)** | FINDING-005 |
...
#### FINDING-007: Exception objects logged without sanitization may contain sensitive connection details
...
| **Affected File(s)** | providers/google/src/airflow/providers/google/cloud/log/gcs_task_handler.py |
| **Source Report(s)** | 16.2.5.md |
| **Related Finding(s)** | FINDING-010 |

**Description:**

Exception objects from the Google Cloud Storage client library are logged directly using `%s` formatting. GCS client exceptions may include request details, authentication context, or connection parameters in their string representation. While these are operational handler logs (not task output logs), they go through the same Python logging infrastructure and could be captured by other handlers or shipped to monitoring systems.
...
#### FINDING-008: Silent swallowing of connection configuration failure in GCS log handler
...
#### FINDING-009: No alerting or retry mechanism when log transmission to remote storage fails
...
#### FINDING-010: Stackdriver handler read path may expose internal details via unhandled exceptions
...
| **Affected File(s)** | providers/google/src/airflow/providers/google/cloud/log/stackdriver_task_handler.py |
...
#### FINDING-011: Stackdriver handler read operation has no graceful degradation when Cloud Logging is unavailable
...
#### FINDING-012: GCS handler write method may silently truncate existing remote logs on transient read failure
...
#### FINDING-013: Stackdriver handler close() method lacks exception handling around transport flush
...
#### FINDING-014: No Automated SBOM Generation in Build Pipeline
...
| **Affected File(s)** | providers/google/pyproject.toml |
| **Source Report(s)** | 15.1.2.md |
| **Related Finding(s)** | - |
...
#### FINDING-015: Dead Code Paths for Unsupported Python Versions in Vendored Library
...
| **Affected File(s)** | providers/google/src/airflow/providers/google/_vendor/json_merge_patch.py |
| **Source Report(s)** | 15.2.3.md |
| **Related Finding(s)** | - |
...
#### FINDING-016: Majority of Dependencies Lack Explicit Source Repository Pinning
...
#### FINDING-017: External Service Communication Dependencies Not Consolidated in Code Documentation
...
| **Affected File(s)** | providers/google/src/airflow/providers/google/cloud/operators/dataflow.py, providers/google/src/airflow/providers/google/cloud/operators/dataproc.py, providers/google/src/airflow/providers/google/cloud/operators/cloud_batch.py, providers/google/src/airflow/providers/google/cloud/operators/kubernetes_engine.py, providers/google/src/airflow/providers/google/cloud/operators/vertex_ai/custom_job.py, providers/google/src/airflow/providers/google/cloud/triggers/cloud_batch.py |
| **Source Report(s)** | 13.1.1.md |
| **Related Finding(s)** | - |
...
#### FINDING-018: Inconsistent Timeout Defaults Across Operators for Same Service Category
...
| **Affected File(s)** | providers/google/src/airflow/providers/google/cloud/operators/cloud_batch.py, providers/google/src/airflow/providers/google/cloud/operators/dataproc.py, providers/google/src/airflow/providers/google/cloud/operators/dataflow.py, providers/google/src/airflow/providers/google/cloud/operators/kubernetes_engine.py, providers/google/src/airflow/providers/google/cloud/operators/vertex_ai/custom_job.py |
| **Source Report(s)** | 13.1.3.md |
| **Related Finding(s)** | - |
...
#### FINDING-019: Resource Management Strategies Implemented but Not Formally Documented with Retry Limits, Back-off Algorithms, and Timeout Rationale
...
| **CWE** | - |
| **ASVS Section(s)** | 13.1.3 |
| **Affected File(s)** | providers/google/src/airflow/providers/google/cloud/operators/dataproc.py, providers/google/src/airflow/providers/google/cloud/operators/kubernetes_engine.py, providers/google/src/airflow/providers/google/cloud/triggers/cloud_batch.py |
| **Source Report(s)** | 13.1.3.md |
| **Related Finding(s)** | - |
...
#### FINDING-020: No Explicit Token Type Claim Validation
...
The `_verify_id_token` function does not explicitly validate token type (e.g., by checking for `sub` claim presence per OIDC spec). While implicit type checking exists via claim requirements (iss, email_verified, email), no explicit token type validation control is applied. Currently not exploitable under Google's OAuth2 architecture where access tokens are opaque, but represents a defense-in-depth gap if Google were to introduce new JWT token types sharing the same signing keys.
...
|--------|---------|-----------------|------------------|
| Google Cloud Authentication | Scopes are configurable per-connection via the `scope` field, allowing operators to restrict scopes to the minimum required | Configuration allows scope limitation per OAuth client connection | — |
| Google Cloud Authentication | Resource server validates audience claims in access tokens | ASVS 10.3.1 marked as Pass | — |
| Google Cloud Authentication | Resource server enforces authorization decisions based on access token claims | ASVS 10.3.2 marked as Pass | — |
| Google Cloud Authentication | OIDC client validates ID Token audience matches client_id | ASVS 10.5.4 marked as Pass | — |
| Google Cloud Authentication | Google provider uses token-based authentication (OIDC/service accounts); no password lifecycle exists in this codebase | Password requirements N/A - token-based authentication only | — |
| Google Cloud Authentication | Token-based authentication with cryptographic signature verification inherently prevents credential stuffing; rate limiting delegated to deployment infrastructure | Report confirmation in 6.3.1.md | — |
| Google Cloud Authentication | MFA enforcement delegated to Google Workspace admin policy and concrete AuthManager implementations | Documented in 6.3.3.md (dropped finding ASVS-633-MED-001) | — |
| Google Cloud Authentication | OIDC authentication strength verification (acr/amr/auth_time) is delegated to concrete AuthManager implementations that define specific strength requirements | Documented in 6.8.4.md (dropped finding ASVS-684-LOW-001) | — |
| Secrets Management | Secrets management properly delegated to GCP Secret Manager with fail-closed error handling and IAM-based access control | Promoted from dropped finding ASVS-1331-LOW-001 | — |
| Secrets Management | Default retrieval of 'latest' secret version ensures rotated secrets are automatically picked up; GCP-native rotation scheduling supported via create_secret API passthrough | Promoted from dropped finding ASVS-1334-INFO-001 | — |
| Cryptographic Operations | KMS key lifecycle management delegated to Deployment Manager via Google Cloud KMS | Documented policy and lifecycle management in place per ASVS 11.1.1 | — |
| Cryptographic Operations | Cryptographic inventory maintenance delegated to Deployment Manager | Inventory maintained and regularly updated per ASVS 11.1.2 | — |
| Cryptographic Operations | Industry-validated cryptographic implementations in use | Verified compliance with ASVS 11.2.1 requirements | — |
| Cryptographic Operations | Crypto agility design implemented | System supports reconfiguration of cryptographic algorithms per ASVS 11.2.2 | — |
| Cryptographic Operations | Minimum 128-bit security for all cryptographic primitives | Verified compliance with ASVS 11.2.3 requirements | — |
| Cryptographic Operations | Constant-time cryptographic operations | Verified compliance with ASVS 11.2.4 requirements | — |
| Cryptographic Operations | Secure cryptographic error handling | Verified compliance with ASVS 11.2.5 requirements | — |
| Cryptographic Operations | No insecure block modes or weak padding schemes in use | Verified compliance with ASVS 11.3.1 requirements | — |
| Cryptographic Operations | Approved ciphers and modes (e.g., AES-GCM) in use | Verified compliance with ASVS 11.3.2 requirements | — |
| Cryptographic Operations | Authenticated encryption protecting encrypted data | Verified compliance with ASVS 11.3.3 requirements | — |
| Cryptographic Operations | Proper nonce and IV management | Verified compliance with ASVS 11.3.4 requirements | — |
| Cryptographic Operations | Encrypt-then-MAC mode for encryption/MAC combinations | Verified compliance with ASVS 11.3.5 requirements | — |
| Cryptographic Operations | Only approved hash functions in use | Verified compliance with ASVS 11.4.1 requirements | — |
| Cryptographic Operations | Cryptographically secure pseudo-random number generation with 128+ bits entropy | Verified compliance with ASVS 11.5.1 requirements | — |
| Cryptographic Operations | Secure random number generation under heavy demand | Verified compliance with ASVS 11.5.2 requirements | — |
| Cryptographic Operations | Approved cryptographic algorithms for key generation and digital signatures | Verified compliance with ASVS 11.6.1 requirements | — |
| Cryptographic Operations | Data minimization and encryption after use | Verified compliance with ASVS 11.7.2 requirements | — |
| Tls And Transport Security | TLS configuration for connections delegated to Deployment Manager; defaults are secure (verification enabled) | Promoted from dropped finding ASVS-1221-LOW-002 | — |
| Session Management | Session management delegated to AuthManager; provider code maintains clean architectural separation from session lifecycle concerns | Dropped finding ASVS-713-LOW-001 | — |
| Session Management | API credential storage in Airflow connections delegated to Deployment Manager who controls metadata DB encryption and secrets backends | Promoted from dropped finding ASVS-722-LOW-001 | — |
| Authorization And Access Control | Authorization enforcement delegated to GCP IAM server-side on every API call via authenticated credentials | Dropped finding ASVS-811-LOW-001 | — |
| Authorization And Access Control | GCP IAM enforces resource-level authorization server-side on every API call; DAG authors are trusted callers within the security model | source: Dropped finding ASVS-822-LOW-001 | — |
| Authorization And Access Control | Fresh AuthorizedSession created per sync request; async path checks credential validity before each call | source: Dropped finding ASVS-832-LOW-001 | — |
| Authorization And Access Control | Multi-tenant isolation enforced by GCP IAM at the platform level | DAG authors are trusted identities permitted to execute arbitrary code; cross-tenant isolation is enforced by GCP IAM (delegated to Deployment Manager per delegated_infrastructure_controls.md) | — |
| Authorization And Access Control | @fallback_to_default_project_id provides safe default scoping | Promoted from dropped finding ASVS-841-LOW-001 | — |
| Api Security | FastAPI framework automatically sets appropriate Content-Type headers with charset for all response types | Framework-level implementation verified during 4.1.1 audit | — |
| Api Security | Socket path length validation (UNIX_PATH_MAX=108) | Implemented in cloud_sql.py to prevent URI length issues | cloud_sql.py |
| Api Security | URL domain allowlisting and HTTPS enforcement | Implemented in GoogleBidManagerDownloadReportOperator | GoogleBidManagerDownloadReportOperator |
| Api Security | No WebSocket endpoints exist; architecture uses HTTP REST with JWT, eliminating CSWSH attack surface | Confirmed during WebSocket security audit (4.4.2) | — |
| Api Security | No WebSocket sessions exist; all session management is HTTP-based with short-lived scoped JWT tokens | Confirmed during WebSocket session management audit (4.4.3) | — |
| Api Security | No HTTPS-to-WebSocket protocol transitions exist; all authenticated sessions remain within HTTPS/gRPC boundary | Confirmed during protocol transition audit (4.4.4) | — |
| Input Validation And Injection | All operator/hook parameters originate from trusted DAG authors; no untrusted input reaches URL construction paths. | Observed in URL construction code paths (ASVS 1.2.2 audit) | — |
| Input Validation And Injection | Regex patterns are developer-defined constants; matched values originate exclusively from trusted DAG authors. | Observed in regex usage patterns (ASVS 1.3.12 audit) | — |
| Input Validation And Injection | All data flowing through Cloud SQL hooks originates from trusted DAG authors or authenticated GCP API responses; no untrusted input paths exist. | Observed in Cloud SQL hooks sanitization review (ASVS 1.3.3 audit) | — |
| Input Validation And Injection | GcpBodyFieldValidator provides comprehensive structural validation as a developer-experience tool for trusted DAG authors | Promoted from dropped finding ASVS-211-LOW-001 | — |
| Input Validation And Injection | Rate/frequency limiting delegated to GCP API quotas and Deployment Manager infrastructure controls | Dropped finding ASVS-213-INFO-001 | — |
| Input Validation And Injection | TLS integrity of binary downloads protected by infrastructure-level TLS termination managed by Deployment Manager | Dropped finding ASVS-221-LOW-002 | — |
| Business Logic Security | DAG authors are trusted to configure appropriate resource bounds (LIMIT clauses, timeouts); worker resource quotas are deployment-level controls | source: Dropped finding ASVS-232-LOW-001, ASVS-232-LOW-002 | — |
| Business Logic Security | Atomicity trade-offs are intentional design decisions for ETL performance; DAG authors are trusted to compose appropriate patterns (staging tables, idempotent retries) | Promoted from dropped findings ASVS-233-LOW-001, ASVS-233-LOW-002, ASVS-233-LOW-003 | — |
| Business Logic Security | Resource locking delegated to GCP APIs (server-side uniqueness constraints, etag-based optimistic locking) | Dropped finding ASVS-234-LOW-001 | — |
| Browser Security | Browser security headers delegated to Deployment Manager at reverse proxy level | Architectural validation - provider code does not serve HTTP responses directly | — |
| Browser Security | Content-Type enforcement delegated to API server/reverse proxy layer | Architectural validation (dropped finding ASVS-321-LOW-001) | — |
| Browser Security | Cookie attributes (Secure, __Secure-/__Host- prefixes) delegated to AuthManager implementations | Architectural validation | — |
| Browser Security | Cookie SameSite attributes delegated to AuthManager implementations | Architectural validation | — |
| Browser Security | HSTS (Strict-Transport-Security header) delegated to reverse proxy (Deployment Manager) | Architectural validation - provider code does not serve HTTP responses | — |
| Browser Security | CORS (Access-Control-Allow-Origin) delegated to FastAPI middleware and Deployment Manager | Architectural validation - provider code is server-to-server only | — |
| Browser Security | CSP (Content-Security-Policy) delegated to reverse proxy (Deployment Manager) | Architectural validation - provider code does not generate HTML or serve HTTP responses | — |
| Browser Security | X-Content-Type-Options header delegated to reverse proxy (Deployment Manager) | Architectural validation - provider code is backend execution only | — |
| Browser Security | Referrer-Policy header delegated to reverse proxy (Deployment Manager) | Architectural validation - provider code does not serve HTTP responses | — |
| Browser Security | CSP frame-ancestors directive delegated to reverse proxy (Deployment Manager) | Architectural validation - provider code produces no frameable content | — |
| Browser Security | CSP violation reporting delegated to reverse proxy (Deployment Manager) | Architectural validation | — |
| Browser Security | Cross-Origin-Opener-Policy delegated to reverse proxy (Deployment Manager); provider code does not serve HTML | Architectural validation | — |
| Browser Security | CSRF protection delegated to FastAPI API layer and AuthManager; provider code uses server-to-server authentication only | Architectural validation | — |
| Browser Security | CORS preflight mechanism delegated to FastAPI middleware and Deployment Manager; provider code exposes no HTTP endpoints | Architectural validation | — |
| Browser Security | HTTP method enforcement delegated to FastAPI route definitions; provider code defines no HTTP endpoints | Architectural validation | — |
| Browser Security | Hostname separation delegated to deployment infrastructure; provider code is a library with no network binding | Architectural validation | — |
| Browser Security | postMessage not used; provider code is server-side Python with no browser-side JavaScript | Architectural validation | — |
| Browser Security | JSONP not used anywhere; all data exchange uses standard JSON serialization via authenticated SDK clients | Architectural validation | — |
| Browser Security | No script resources served; data flows through authenticated backend channels (XCom, GCS, BigQuery) not browser-loadable scripts | Architectural validation | — |
| Browser Security | No authenticated resources served to browsers; provider code is backend-only with zero browser-facing surface area | Architectural validation | — |
| File Upload Security | File type/size enforcement delegated to GCS bucket policies configured by Deployment Manager | Dropped finding ASVS-511-INFO-001 | — |
| File Upload Security | File size quota enforcement delegated to GCS bucket quotas and GCP billing controls | Dropped finding ASVS-521-LOW-001 | — |
| File Upload Security | File type validation delegated to GCS bucket policies; implicit JSON validation via json.loads() in parse_json_from_gcs() | Dropped finding ASVS-522-LOW-001 | — |
| File Upload Security | Per-user storage quotas delegated to GCS bucket policies and GCP IAM/billing | Promoted from dropped finding ASVS-524-LOW-001 | — |
| File Upload Security | Antivirus scanning delegated to GCS bucket-level malware scanning configured by Deployment Manager | Dropped finding ASVS-543-INFO-001 | — |
| Data Protection And Privacy | Sensitive data has been identified and classified into protection levels per 14.1.1 | ASVS section 14.1.1 marked as Pass | — |
| Data Protection And Privacy | All sensitive data protection levels have documented protection requirements per 14.1.2 | ASVS section 14.1.2 marked as Pass | — |
| Data Protection And Privacy | Sensitive data is only sent in HTTP message body or headers, not in URLs or query strings per 14.2.1 | ASVS section 14.2.1 marked as Pass | — |
| Data Protection And Privacy | Application prevents sensitive data from being cached in server components or securely purges after use per 14.2.2 | ASVS section 14.2.2 marked as Pass | — |
| Data Protection And Privacy | Defined sensitive data is not sent to untrusted parties per 14.2.3 | ASVS section 14.2.3 marked as Pass | — |
| Data Protection And Privacy | Controls around sensitive data (encryption, integrity, retention, logging, access controls, privacy) are implemented per documentation per 14.2.4 | ASVS section 14.2.4 marked as Pass | — |
| Data Protection And Privacy | Application returns only minimum required sensitive data per 14.2.6 | ASVS section 14.2.6 marked as Pass | — |
| Data Protection And Privacy | Sensitive information is subject to data retention classification with automatic deletion per 14.2.7 | ASVS section 14.2.7 marked as Pass | — |
| Component And Dependency Security | Explicit documented policy following ASF severity ratings without fixed remediation timeframes | source: Report positive controls | — |
| Component And Dependency Security | Direct third-party dependencies declared in pyproject.toml with version constraints | Project declares all direct third-party dependencies in pyproject.toml files | providers/google/pyproject.toml |
| Component And Dependency Security | Committed dependency lock file providing complete snapshot of resolved versions | Maintains a committed uv.lock file that provides a complete snapshot of all resolved dependency versions (direct and transitive) | uv.lock |
| Component And Dependency Security | No fixed remediation timeframes per documented ASF policy; requirement is not applicable per profile | Profile out-of-scope ASVS 15.2.1 | — |
| Component And Dependency Security | Deferrable mode widely adopted across 20+ operators releasing worker slots during long-running operations | Observed across multiple operators in the codebase | — |
| Component And Dependency Security | Resource exhaustion mitigated by Airflow framework execution_timeout and GCP API quotas managed by Deployment Manager | Framework-level timeout controls and infrastructure-managed API quotas | — |
| Component And Dependency Security | Go module resolution delegated to trusted DAG authors who can already execute arbitrary code | Dropped finding ASVS-1524-LOW-002 | — |
| Component And Dependency Security | Binary downloads initiated by trusted DAG authors operate within existing arbitrary code execution trust boundary | source: Dropped finding ASVS-1525-LOW-001 | — |
| Component And Dependency Security | OS-level process isolation and host security delegated to Deployment Manager | source: Dropped finding ASVS-1525-LOW-002 | — |
| Api And Data Layer Security | DAG authors are trusted identities with arbitrary code execution; field filtering is convenience, not a security boundary | Dropped finding ASVS-1531-LOW-001 - field filtering not required as security control within trusted-caller model | — |
| Api And Data Layer Security | All network communication delegated to Google Cloud client libraries with fixed API endpoints; network security delegated to Deployment Manager | Dropped finding ASVS-1532-LOW-001 - redirect following controls not applicable as external URL calls use managed GCP libraries | — |
| Api And Data Layer Security | DAG authors are trusted identities; API field mask behavior is a usability design choice within trusted-caller model | Dropped finding ASVS-1533-LOW-001 - mass assignment protections not required within trusted-caller architecture | — |
| Api And Data Layer Security | Python type system with strict equality operations used throughout codebase | 15.3.5 assessment - no type juggling vulnerabilities identified in Python implementation | — |
| Compute Execution Security | All operators support configurable gcp_conn_id and impersonation_chain for per-service credential scoping, enabling least-privilege deployment configurations | Promoted from dropped finding ASVS-1322-LOW-001 | — |
| Compute Execution Security | Connection pool management delegated to Google Cloud client libraries with proper timeout, retry, and backoff controls implemented at operator level | Promoted from dropped finding ASVS-1326-LOW-001 | — |
| Compute Execution Security | Airflow's single-threaded-per-task execution model eliminates multi-threaded race conditions in operator code | Execution model delivers signals to the same thread running execute(), and Python's GIL ensures atomic reference assignment | — |
| Compute Execution Security | GCP API-level idempotency (AlreadyExists, NotFound, request_id) provides server-side atomicity for distributed operations | source: Dropped finding ASVS-1542-LOW-001 | — |
| Compute Execution Security | TOCTOU patterns in operator code only exploitable by trusted DAG authors with purely operational impact | source: Dropped finding ASVS-1542-LOW-002 | — |
...
| 2.2.3 | Verify that the application ensures that combinations of related data items are reasonable according to the pre-defined rules. | **Partial** | See FINDING-004 |
...
| 5.3.2 | Verify that when the application creates file paths for file operations, instead of user-submitted filenames, it uses internally generated or trusted data, or if user-submitted filenames or file metadata must be used, strict validation and sanitization must be applied. This is to protect against path traversal, local or remote file inclusion (LFI, RFI), and server-side request forgery (SSRF) attacks. | **Fail** | See FINDING-005, FINDING-006 |
...
| 6.3.8 | Verify that valid users cannot be deduced from failed authentication challenges, such as by basing on error messages, HTTP response codes, or different response times. Registration and forgot password functionality must also have this protection. | **Partial** | See FINDING-002 |
...
| 6.5.6 | Verify that any authentication factor (including physical devices) can be revoked in case of theft or other loss. | **Pass** |  |
...
| 6.8.1 | Verify that, if the application supports multiple identity providers (IdPs), the user's identity cannot be spoofed via another supported identity provider (eg. by using the same user identifier). The standard mitigation would be for the application to register and identify the user using a combination of the IdP ID (serving as a namespace) and the user's ID in the IdP. | **Partial** | See FINDING-003 |
...
| 10.3.3 | Verify that if an access control decision requires identifying a unique user from an access token (JWT or related token introspection response), the resource server identifies the user from claims that cannot be reassigned to other users. Typically, it means using a combination of 'iss' and 'sub' claims. | **Partial** | See FINDING-001 |
...
| 10.5.2 | Verify that the client uniquely identifies the user from ID Token claims, usually the 'sub' claim, which cannot be reassigned to other users (for the scope of an identity provider). | **Partial** | See FINDING-001 |
...
| 12.1.4 | Verify that proper certification revocation, such as Online Certificate Status Protocol (OCSP) Stapling, is enabled and configured. | **Pass** |  |
| 12.1.5 | Verify that Encrypted Client Hello (ECH) is enabled in the application's TLS settings to prevent exposure of sensitive metadata, such as the Server Name Indication (SNI), during TLS handshake processes. | **Pass** |  |
...
| 15.1.2 | Verify that an inventory catalog, such as software bill of materials (SBOM), is maintained of all third-party libraries in use, including verifying that components come from pre-defined, trusted, and continually maintained repositories. | **Partial** | See FINDING-014 |
...
| 15.2.3 | Verify that the production environment only includes functionality that is required for the application to function, and does not expose extraneous functionality such as test code, sample snippets, and development functionality. | **Pass** | See FINDING-015 |
| 15.2.4 | Verify that third-party components and all of their transitive dependencies are included from the expected repository, whether internally owned or an external source, and that there is no risk of a dependency confusion attack. | **Partial** | See FINDING-016 |
...
| 16.2.5 | Verify that when logging sensitive data, the application enforces logging based on the data's protection level. For example, it may not be allowed to log certain data, such as credentials or payment details. Other data, such as session tokens, may only be logged by being hashed or masked, either in full or partially. | **Partial** | See FINDING-007 |
...
| 16.3.4 | Verify that the application logs unexpected errors and security control failures such as backend TLS failures. | **Partial** | See FINDING-008 |
...
| 16.4.3 | Verify that logs are securely transmitted to a logically separate system for analysis, detection, alerting, and escalation. The aim is to ensure that if the application is breached, the logs are not compromised. | **Partial** | See FINDING-009 |
| 16.5.1 | Verify that a generic message is returned to the consumer when an unexpected or security-sensitive error occurs, ensuring no exposure of sensitive internal system data such as stack traces, queries, secret keys, and tokens. | **Partial** | See FINDING-010 |
| 16.5.2 | Verify that the application continues to operate securely when external resource access fails, for example, by using patterns such as circuit breakers or graceful degradation. | **Partial** | See FINDING-011 |
| 16.5.3 | Verify that the application fails gracefully and securely, including when an exception occurs, preventing fail-open conditions such as processing a transaction despite errors resulting from validation logic. | **Partial** | See FINDING-012 |
...
**Summary Statistics:**
- **Pass**: 109 requirements (31.6%)
- **Partial**: 17 requirements (4.9%)
- **N/A**: 218 requirements (63.2%)
- **Fail**: 1 requirements (0.3%)
...
|------------|----------|-------------------|------------------|---------------------|
| FINDING-001 | Medium | 10.3.3, 10.5.2 | — | providers/google/src/airflow/providers/google/common/auth_backend/google_openid.py |
| FINDING-002 | Low | 6.3.8 | — | providers/google/src/airflow/providers/google/common/auth_backend/google_openid.py |
| FINDING-003 | Low | 6.8.1 | — | providers/google/src/airflow/providers/google/common/auth_backend/google_openid.py |
| FINDING-004 | Low | 2.2.3 | — | providers/google/src/airflow/providers/google/cloud/hooks/cloud_sql.py |
| FINDING-005 | Low | 5.3.2 | FINDING-006 | providers/google/src/airflow/providers/google/cloud/hooks/gcs.py |
| FINDING-006 | Low | 5.3.2 | FINDING-005 | providers/google/src/airflow/providers/google/cloud/operators/gcs.py |
| FINDING-007 | Low | 16.2.5 | FINDING-010 | providers/google/src/airflow/providers/google/cloud/log/gcs_task_handler.py |
| FINDING-008 | Low | 16.3.4 | — | providers/google/src/airflow/providers/google/cloud/log/gcs_task_handler.py |
| FINDING-009 | Low | 16.4.3 | — | providers/google/src/airflow/providers/google/cloud/log/gcs_task_handler.py |
| FINDING-010 | Low | 16.5.1 | FINDING-007 | providers/google/src/airflow/providers/google/cloud/log/stackdriver_task_handler.py |
| FINDING-011 | Low | 16.5.2 | — | providers/google/src/airflow/providers/google/cloud/log/stackdriver_task_handler.py |
| FINDING-012 | Low | 16.5.3 | — | providers/google/src/airflow/providers/google/cloud/log/gcs_task_handler.py |
| FINDING-013 | Low | 16.5.4 | — | providers/google/src/airflow/providers/google/cloud/log/stackdriver_task_handler.py |
| FINDING-014 | Low | 15.1.2 | — | providers/google/pyproject.toml |
| FINDING-015 | Low | 15.2.3 | — | providers/google/src/airflow/providers/google/_vendor/json_merge_patch.py |
| FINDING-016 | Low | 15.2.4 | — | providers/google/pyproject.toml |
| FINDING-017 | Low | 13.1.1 | — | providers/google/src/airflow/providers/google/cloud/operators/dataflow.py, providers/google/src/airflow/providers/google/cloud/operators/dataproc.py, providers/google/src/airflow/providers/google/cloud/operators/cloud_batch.py, providers/google/src/airflow/providers/google/cloud/operators/kubernetes_engine.py, providers/google/src/airflow/providers/google/cloud/operators/vertex_ai/custom_job.py, providers/google/src/airflow/providers/google/cloud/triggers/cloud_batch.py |
| FINDING-018 | Low | 13.1.3 | — | providers/google/src/airflow/providers/google/cloud/operators/cloud_batch.py, providers/google/src/airflow/providers/google/cloud/operators/dataproc.py, providers/google/src/airflow/providers/google/cloud/operators/dataflow.py, providers/google/src/airflow/providers/google/cloud/operators/kubernetes_engine.py, providers/google/src/airflow/providers/google/cloud/operators/vertex_ai/custom_job.py |
| FINDING-019 | Low | 13.1.3 | — | providers/google/src/airflow/providers/google/cloud/operators/dataproc.py, providers/google/src/airflow/providers/google/cloud/operators/kubernetes_engine.py, providers/google/src/airflow/providers/google/cloud/triggers/cloud_batch.py |
| FINDING-020 | Low | 9.2.2 | — | providers/google/src/airflow/providers/google/common/auth_backend/google_openid.py |

**Total Unique Findings**: 20 (0 Critical, 0 High, 1 Medium, 19 Low, 0 Info)
