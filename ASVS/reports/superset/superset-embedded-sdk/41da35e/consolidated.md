...
*Informational findings are recorded in this report but not opened as GitHub issues — see issues.md for the 2 actionable items.*
...
This audit evaluated the superset-embedded-sdk against ASVS Level 3 requirements across 37 security domains. The SDK's minimal attack surface — functioning primarily as a thin iframe messaging proxy that delegates all authorization and validation to the Flask backend — resulted in a very low finding count, with the majority of ASVS controls satisfied by architectural delegation to server-side enforcement.

### Top 5 Risks

1. **No Timeout or Error Handling Documentation for fetchGuestToken Callback** [Low] — The SDK does not enforce or document timeout behaviour for the integrator-supplied `fetchGuestToken` callback, which could leave the embedded dashboard in an indeterminate state if the callback hangs or silently fails.
2. **Token Refresh Timer Not Cleaned Up on Unmount — Resource Leak** [Low] — When the SDK component is unmounted, the scheduled token refresh timer is not cancelled, causing a resource leak and potential attempts to communicate with a destroyed iframe.
3. **No formal data classification documentation within the embedded SDK component** [Info] — The SDK lacks explicit documentation classifying the sensitivity of data it handles (guest tokens, dashboard IDs, domain URLs), which is expected at L2+ maturity.
4. **No documented protection requirements for sensitive data handled by the SDK** [Info] — There is no accompanying documentation specifying protection requirements for token material and configuration data flowing through the SDK, as expected by ASVS 14.1.2.
5. No additional findings — the repository demonstrated strong security posture with only 4 total findings identified across 345 source reports.

### Positive Controls Observed

- **Trusted input boundary**: Host application configuration inputs (`supersetDomain`, `id`) are within the integrator trust boundary and not user-controllable (ASVS 1.2.2).
- **Canonical decoding**: Application performs canonical decoding only once before processing (ASVS 1.1.1).
- **Context-appropriate output encoding**: Framework-level output encoding performed as final step for HTTP responses and HTML documents (ASVS 1.1.2, 1.2.1).
- **JavaScript/JSON injection prevention**: JavaScript and JSON content properly encoded to prevent injection (ASVS 1.2.3).
- **OS command injection protection**: OS command calls use parameterized queries or contextual encoding (ASVS 1.2.5).
- **Regex metacharacter handling**: Regular expression special characters properly escaped (ASVS 1.2.9).
- **No dynamic code execution**: Dynamic code execution features (eval, SpEL) avoided (ASVS 1.3.2).
- **Input sanitization with length limits**: Input sanitized before use in dangerous contexts with length limits enforced (ASVS 1.3.3).
- **Trusted integrator model**: SDK inputs originate exclusively from trusted host application developers, not end-users.
- **Server-side validation authority**: Backend is the authoritative enforcement boundary for input validation; client-side SDK delegates all validation to server.
- **Explicit targetOrigin on postMessage**: `postMessage` uses explicit `targetOrigin` (supersetDomain), preventing cross-origin message leakage.
- **URLSearchParams safe encoding**: Query parameters safely encoded via `URLSearchParams`.
- **Restrictive iframe sandbox defaults**: Iframe sandbox attributes applied by default.
- **Safe deserialization**: No insecure deserialization mechanisms identified (ASVS 1.5.2, 1.5.3).
- **Type safety enforced**: Correct variable types used with strict equality and comparator operations (ASVS 15.3.5).
- **Authorization delegated to backend**: Embedded SDK delegates all authorization enforcement to Flask backend; no client-side auth decisions.
- **Server-side field filtering**: SDK treats server responses as already field-filtered; no client-side field-level authorization attempted.
- **Token-mediated access model**: SDK only forwards scoped guest tokens; server-side IDOR/BOLA prevention enforced by Flask backend.
- **Multi-tenant isolation**: Delegated to Flask backend via guest token scoping and RLS.
- **Dynamic session tokens**: Session token verification performed using backend service with dynamic generation, not static API secrets (ASVS 7.2.1, 7.2.2).
- **Token refresh propagation**: Token refresh mechanism ensures authorization changes propagate within token TTL minus 5-second buffer.
- **MessageChannel token transport**: Guest token transported via MessageChannel, not cookies — eliminating cookie-based attack vectors.
- **Configurable referrerPolicy**: SDK exposes configurable `referrerPolicy` attribute on iframe element.
- **Fail-closed on refresh failure**: Server-side token validation ensures fail-closed behavior when client-side refresh stops — no security bypass possible from SDK-side failures.
...
#### FINDING-001: No Timeout or Error Handling Documentation for fetchGuestToken Callback
...
The SDK awaits the host-provided fetchGuestToken callback without timeout protection. If the promise never resolves, the SDK hangs indefinitely (initial embed) or the refresh cycle stalls (subsequent refreshes). No documentation defines timeout settings, failure handling, or retry limits for this external service callback.

**Remediation:**

Wrap the host-provided callback with a configurable timeout to prevent indefinite hangs. Document the expected behavior of fetchGuestToken including timeout expectations and failure modes. Define resource-management strategies including timeout settings, failure handling, and retry logic for this external callback mechanism.

---

#### FINDING-002: Token Refresh Timer Not Cleaned Up on Unmount — Resource Leak
...
After the host application calls `unmount()` to terminate the embedded session, the `refreshGuestToken` recursive timer loop continues indefinitely. This means: 1) The guest token (an authentication credential) persists in the JavaScript closure after session termination. 2) New guest tokens continue being fetched from the host app's backend. 3) The `Switchboard` port reference (`ourPort`) is never disconnected. 4) If a host application embeds/unmounts multiple times, timer instances accumulate.

**Remediation:**

Store the setTimeout handle and clear it in unmount() to prevent post-teardown resource usage. Document resource-release procedures for timer handles in the application's resource-management strategy.

### 3.5 Informational

#### FINDING-003: No formal data classification documentation within the embedded SDK component
...
The `superset-embedded-sdk` package processes sensitive data — specifically guest tokens (JWTs containing authentication claims and resource access permissions) — but the SDK component itself contains no formal documentation classifying this data into protection levels. However, this observation must be contextualized: the SDK is explicitly documented as a client-side message broker (not a security component), and data classification is a project-level requirement.

**Remediation:**

Consider adding explicit documentation about what data the SDK handles and how it protects them in README or JSDoc.

---

#### FINDING-004: No documented protection requirements for sensitive data handled by the SDK
...
The embedded SDK handles guest tokens (authentication material) but does not include documentation specifying protection requirements for this data type. This is expected for the SDK's architectural role. Protection requirements for guest tokens are defined and enforced by the Superset backend.

**Remediation:**

Consider documenting SDK data handling responsibilities and delegation points in README or JSDoc.
...
|--------|---------|-----------------|------------------|
| Flask Backend Input Validation | Host application configuration inputs (supersetDomain, id) are within the integrator trust boundary and not user-controllable | ASVS 1.2.2 - Dynamic URL building with trusted inputs only | — |
| Flask Backend Input Validation | Application performs canonical decoding only once before processing | ASVS 1.1.1 - Pass status indicates proper input decoding architecture | — |
| Flask Backend Input Validation | Output encoding performed appropriately by framework or as final step | ASVS 1.1.2 - Pass status indicates proper output encoding architecture | — |
| Flask Backend Input Validation | Context-appropriate output encoding for HTTP responses and HTML documents | ASVS 1.2.1 - Pass status indicates proper context-aware encoding | — |
| Flask Backend Input Validation | JavaScript and JSON content properly encoded to prevent injection | ASVS 1.2.3 - Pass status indicates protection against JavaScript/JSON injection | — |
| Flask Backend Input Validation | OS command calls use parameterized queries or contextual encoding | ASVS 1.2.5 - Pass status indicates protection against OS command injection | — |
| Flask Backend Input Validation | Regular expression special characters properly escaped | ASVS 1.2.9 - Pass status indicates proper regex metacharacter handling | — |
| Flask Backend Input Validation | Dynamic code execution features (eval, SpEL) avoided or properly sanitized | ASVS 1.3.2 - Pass status indicates safe handling of dynamic code execution | — |
| Flask Backend Input Validation | Input sanitized before use in dangerous contexts with length limits enforced | ASVS 1.3.3 - Pass status indicates proper input sanitization for dangerous contexts | — |
| Flask Backend Input Validation | SDK inputs originate exclusively from trusted host application developers, not end-users | Documented in embedded_sdk_client_side_proxy.md and admin_role_trusted.md stating 'Extension developers: fully trusted' | — |
| Flask Backend Input Validation | Backend is the authoritative enforcement boundary for input validation; client-side SDK delegates all validation to server | Dropped finding ASVS-211-INFO-001 | — |
| Flask Backend Input Validation | postMessage uses explicit targetOrigin (supersetDomain), URLSearchParams safely encodes query params, restrictive iframe sandbox defaults applied | source: Dropped finding ASVS-221-INFO-001 | — |
| Flask Backend Input Validation | Safe deserialization practices observed - no insecure deserialization mechanisms identified | ASVS 1.5.2 and 1.5.3 passed | — |
| Flask Backend Input Validation | Type safety enforced - correct variable types used with strict equality and comparator operations | ASVS 15.3.5 passed | — |
| Flask Appbuilder Rbac | Embedded SDK delegates all authorization enforcement to Flask backend; no client-side auth decisions | Architecture observation | — |
| Flask Appbuilder Rbac | SDK treats server responses as already field-filtered; no client-side field-level authorization attempted | Architecture observation | — |
| Flask Appbuilder Rbac | SDK does not collect or inject environmental attributes; contextual security decisions delegated to backend | Architecture observation | — |
| Flask Appbuilder Rbac | No risk-evaluation or adaptive security logic in client SDK; all decisions centralized at backend | Architecture observation | — |
| Flask Appbuilder Rbac | SDK functions are message proxies only; function-level access enforced by Flask backend RBAC | Architecture observation | — |
| Flask Appbuilder Rbac | Token-mediated access model with server-side IDOR/BOLA prevention; SDK only forwards scoped guest tokens | Architecture observation | — |
| Flask Appbuilder Rbac | Field-level access restriction delegated to Flask backend schema projections and RLS | Architecture observation | — |
| Flask Appbuilder Rbac | Adaptive security controls delegated to Flask backend; SDK token refresh provides integration point for re-evaluation | Architecture observation | — |
| Flask Appbuilder Rbac | No client-side authorization enforcement; all auth delegated to trusted Flask backend service layer | Architecture observation | — |
| Flask Appbuilder Rbac | postMessage with explicit targetOrigin prevents cross-origin message leakage | Architecture observation | — |
| Flask Appbuilder Rbac | Token refresh mechanism ensures authorization changes propagate within token TTL minus 5-second buffer | Architecture observation | — |
| Flask Appbuilder Rbac | Consumer's guest token forwarded without substitution; no service identity injection | Architecture observation | — |
| Flask Appbuilder Rbac | Multi-tenant isolation delegated to Flask backend via guest token scoping and RLS | Architecture observation | — |
| Flask Session Management | Session token verification performed using backend service (Flask session management) | ASVS 7.2.1 marked as Pass | — |
| Flask Session Management | Dynamic session token generation (not static API secrets) | ASVS 7.2.2 marked as Pass | — |
| Browser Security Headers | SDK uses origin-scoped postMessage, MessageChannel, and iframe sandbox by default | Promoted from dropped finding ASVS-311-INFO-001 | — |
| Browser Security Headers | SDK exposes configurable referrerPolicy attribute on iframe element | source: Dropped finding ASVS-345-INFO-001 | — |
| Cookie Security | Guest token transport via MessageChannel, not cookies | SDK uses MessageChannel not cookies | — |
| Logging And Monitoring | Server-side token validation ensures fail-closed behavior when client-side refresh stops — no security bypass possible from SDK-side failures. | Dropped finding ASVS-1652-LOW-001 from 16.5.2.md | — |
...
**Summary Statistics:**
- **Pass**: 41 requirements (11.9%)
- **Partial**: 1 requirements (0.3%)
- **N/A**: 302 requirements (87.5%)
...
|------------|----------|-------------------|------------------|---------------------|
| FINDING-001 | Low | 13.1.3 | — | superset-embedded-sdk/src/index.ts |
| FINDING-002 | Low | 13.1.3, 14.3.1 | — | superset-embedded-sdk/src/index.ts |
| FINDING-003 | Informational | 14.1.1 | — | superset-embedded-sdk/src/index.ts, superset-embedded-sdk/src/guestTokenRefresh.ts |
| FINDING-004 | Informational | 14.1.2 | — | superset-embedded-sdk/src/index.ts |

**Total Unique Findings**: 4 (0 Critical, 0 High, 0 Medium, 2 Low, 2 Info)

*2 of 4 are actionable. Informational findings are recorded here but not opened as GitHub issues; see issues.md for the 2 actionable items.*
