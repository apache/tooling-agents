...
This audit evaluated the superset-websocket sidecar service against ASVS Level 3 requirements across 13 security domains. Coverage spans WebSocket-specific concerns (JWT authentication, connection management, backpressure) as well as supporting infrastructure (logging, memory safety, deployment delegation), with particular depth in session management and credential lifecycle controls delegated to Flask-AppBuilder.

### Top 5 Risks

1. **No Per-User or Total Connection Cap Allows Resource Starvation** [High] — The WebSocket server does not enforce any application-level limit on concurrent connections per user or globally, allowing a single actor to exhaust server resources.
2. **No Backpressure Handling Allows Slow-Client Resource Starvation** [High] — Messages are queued unboundedly for slow consumers without backpressure signaling, enabling memory exhaustion through clients that intentionally stall reads.
3. **Synchronous Event Processing Without Yielding Can Starve Event Loop** [Medium] — Large Redis event batches are processed synchronously in a single tick, blocking the Node.js event loop and degrading responsiveness for all connected clients.
4. **WebSocket sidecar forwards entire Redis payload without schema-based field projection** [Low] — The sidecar relays complete event payloads from Redis to subscribers without filtering fields, relying entirely on the publishing backend to exclude sensitive attributes.
5. **No `unhandledRejection` handler; last-resort exception handler does not prevent process termination** [Low] — Unhandled promise rejections will terminate the process under Node.js default behavior, and the existing uncaughtException handler lacks graceful shutdown logic.

### Positive Controls Observed

- **Connection rate limiting at ingress/WAF layer** — Per-user and global connection caps are enforced at the reverse proxy/WAF layer per documented deployment architecture, providing defense-in-depth upstream of the application.
- **Redis stream ID format validation** — The Redis backend independently validates stream ID format; invalid IDs are rejected and errors caught gracefully, preventing injection into the event stream.
- **Anti-automation protection delegated to ingress** — Resource exhaustion and anti-automation protections are delegated to the ingress/WAF layer per the documented deployment model.
- **HTTP security headers delegated to reverse proxy** — Response headers for the websocket sidecar are managed by the reverse proxy by documented design, ensuring consistent security header policy.
- **JWT-based authorization model with channel claims** — The sidecar authorization model uses JWT signature verification combined with channel claim extraction; data-content control is delegated to the Flask backend which publishes only appropriate fields to trusted Redis infrastructure.
- **Implicit token type discrimination** — A dedicated cookie name (`async-token`), purpose-specific signing key, and required `channel` claim provide implicit token type discrimination without explicit `typ` headers.
- **Purpose-specific jwtSecret with structural claim requirement** — The signing key is dedicated to the Flask backend ↔ WebSocket sidecar channel with structural claim requirements providing implicit audience restriction.
- **60-second ping/pong timeout** — The liveness timeout bounds the maximum exposure window for revoked sessions and provides connection cleanup for stale clients.
- **CORS/Origin as hardening-only when JWT present** — Missing Origin validation is documented as a hardening improvement rather than a vulnerability when JWT authentication is enforced.
- **Session lifecycle delegation to Flask-AppBuilder** — Session establishment, expiration, invalidation, concurrent session policy, federated session management, and absolute session lifetime are all delegated to Flask-AppBuilder with the sidecar validating JWT at connection time by design.
- **Account lifecycle management delegation** — Account disable/delete enforcement is delegated to Flask-AppBuilder; new sidecar connections require a valid JWT from the backend.
- **MFA and credential lifecycle delegation** — All MFA factor management, password reset, authentication factor revocation, and credential lifecycle operations are delegated to Flask-AppBuilder; the sidecar has no credential management capability.
- **Session token verification by trusted backend** — Dynamic JWT-based session management is implemented with token verification performed by the trusted backend service.
- **No automatic session creation** — Session creation requires explicit user action (WebSocket connection initiation), preventing automatic session creation without user interaction.
...
#### FINDING-001: No Per-User or Total Connection Cap Allows Resource Starvation
...
The WebSocket server implements no limits on the number of connections that can be established, either per-user (per-channel) or in total. Any authenticated user can open unlimited WebSocket connections, exhausting server resources and preventing other legitimate users from establishing connections. This directly violates the requirement for fair resource access.

**Remediation:**

Implement per-user and total connection limits in `trackClient` and reject connections when limits are exceeded. Configure MAX_CONNECTIONS_PER_CHANNEL and MAX_TOTAL_CONNECTIONS with appropriate thresholds.

---

#### FINDING-002: No Backpressure Handling Allows Slow-Client Resource Starvation
...
The service does not implement backpressure handling when broadcasting events to WebSocket clients. Slow or malicious clients that stop consuming messages cause unbounded buffering in the server's memory, eventually exhausting resources and degrading service quality for all users. This violates fair resource allocation by allowing slow clients to monopolize server memory.

**Remediation:**

Add WebSocket backpressure handling by checking `bufferedAmount` before sending and terminating clients that exceed a configurable buffer threshold.

---

### 3.3 Medium

#### FINDING-003: Synchronous Event Processing Without Yielding Can Starve Event Loop
...
The service processes Redis stream results synchronously without yielding to the Node.js event loop. Under high event volume, this monopolizes the event loop, delaying or preventing other critical operations like health checks, new connection handling, and ping/pong processing. This creates unfair resource allocation where bulk event processing starves connection management operations.

**Remediation:**

Introduce event-loop yielding in `processStreamResults` for large batches using `setImmediate()` to prevent starvation of connection management and health check operations.

---

### 3.4 Low

#### FINDING-004: 🔵 WebSocket sidecar forwards entire Redis payload without schema-based field projection
...
| **Files** | superset-websocket/src/index.ts:226-236 |
| **Source Reports** | 15.3.1.md |
| **Related** | None |

**Description:**

The `processStreamResults` function in `superset-websocket/src/index.ts` (lines 226-236) forwards entire Redis payload to WebSocket clients using spread operator `{ id, ...data }` without an allowlist-based field projection. While the Redis content is controlled by the Flask backend, if the backend's Redis publishing logic were to include additional internal metadata, the sidecar would forward them without filtering. Test evidence confirms `user_id` field is forwarded to client.

**Remediation:**

Apply an explicit allowlist when constructing the outbound message, only forwarding fields intended for client consumption (e.g., channel_id, job_id, status, errors, result_url).

---

#### FINDING-005: 🔵 Object literals used as lookup registries instead of Map or null-prototype objects
...
| **Files** | superset-websocket/src/index.ts:121-122 |
| **Source Reports** | 15.3.6.md |
| **Related** | None |

**Description:**

The `channels` and `sockets` registries in `superset-websocket/src/index.ts` (lines 121-122) use plain object literals (`{}`), which inherit from `Object.prototype`. The `in` operator checks the entire prototype chain, meaning keys like `__proto__` would match inherited properties. While exploitation requires forging a JWT signed with the server's secret (not achievable by external attacker), the pattern is defensively suboptimal per ASVS 15.3.6 which recommends Map() or Object.create(null).

**Remediation:**

Replace object literal registries with Map instances: `export let channels: Map<string, ChannelValue> = new Map();` and update all access patterns accordingly.

---

#### FINDING-006: 🔵 Log Entries Missing Timestamp Metadata
...
| **ASVS Level(s)** | L2 |
| **CWE** | CWE-778 |
| **ASVS sections** | 16.2.1, 16.2.2 |
| **Files** | superset-websocket/src/logger.ts |
| **Source Reports** | 16.2.1.md, 16.2.2.md |
| **Related** | FINDING-007, FINDING-008, FINDING-010, FINDING-011 |

**Description:**

The winston logger is configured with `format.json()` and `format.errors()` but does NOT include `winston.format.timestamp()`. In winston 3.x, timestamps are not added automatically; they must be explicitly included in the format pipeline. As a result, structured JSON log entries lack a `timestamp` field, omitting the "when" metadata required for timeline investigation. Without `winston.format.timestamp()`, log entries contain no application-level timestamp and consequently no UTC specification or explicit timezone offset. While container runtimes may prepend timestamps to stdout, this is not under application control and may use local time without explicit offset.

**Remediation:**

Add `winston.format.timestamp({ format: () => new Date().toISOString() })` to the format pipeline to produce ISO 8601 UTC timestamps with explicit timezone (Z suffix).

---

#### FINDING-007: 🔵 Security Events Lack "Who" Metadata
...
| **ASVS Level(s)** | L2 |
| **CWE** | CWE-778 |
| **ASVS sections** | 16.2.1, 16.3.1 |
| **Files** | superset-websocket/src/index.ts |
| **Source Reports** | 16.2.1.md, 16.3.1.md |
| **Related** | FINDING-006, FINDING-008, FINDING-010, FINDING-011 |

**Description:**

The websocket sidecar logs authentication failures with minimal metadata (only the exception object) and successful authentications only at debug level. Failed authentication log entries lack source IP, attempted channel, authentication type, or timestamp context. No distinction between different failure modes (expired token, invalid signature, missing token). Incident response teams cannot reconstruct authentication timelines.

**Remediation:**

Add structured authentication event logging in `httpUpgrade()` for both success (info level) and failure (warn level) paths, including sourceIp, channelId, authType, and failure reason.

---

#### FINDING-008: 🔵 Direct Console Output Bypasses Logger Configuration
...
| **ASVS Level(s)** | L2 |
| **CWE** | CWE-778 |
| **ASVS sections** | 16.2.3, 16.3.4 |
| **Files** | superset-websocket/src/index.ts:449 |
| **Source Reports** | 16.2.3.md, 16.3.4.md |
| **Related** | FINDING-006, FINDING-007, FINDING-010, FINDING-011 |

**Description:**

Multiple code paths output directly to `console.error` and `console.warn` instead of using the configured winston logger. These outputs bypass the JSON format, the `silent` flag, and the file transport, and are not documented as separate log destinations. The WebSocket error handler at line 449 (`ws.on('error', console.error)`) handles runtime errors that could contain diagnostic information.

**Remediation:**

Replace console.error/warn calls with logger calls to ensure all output routes through the configured log pipeline.

---

#### FINDING-009: 🔵 Debug-Level Logging of Raw Event Stream Data
...
When debug logging is enabled, the raw Redis stream results are logged via string interpolation. The `EventValue` interface shows these records contain `user_id`, `job_id`, `channel_id`, and potentially `result_url`. While debug-level logging is typically disabled in production, there is no data classification check or filtering mechanism preventing sensitive event metadata from appearing in logs when debug is enabled.
...
#### FINDING-010: 🔵 No Logging of Security Control Bypass Attempts
...
| **Related** | FINDING-006, FINDING-007, FINDING-008, FINDING-011 |

**Description:**

The service does not differentiate between benign errors and potential attack patterns. Repeated authentication failures, malformed requests, or unexpected request patterns are not logged as security events. No distinction between 'JWT expired' vs 'JWT wrong signature' vs 'JWT missing' in logged events. Unexpected HTTP requests logged generically without security context.
...
#### FINDING-011: 🔵 Redis Connection Errors Logged Without Context
...
| **Related** | FINDING-006, FINDING-007, FINDING-008, FINDING-010 |
...
#### FINDING-012: 🔵 No `unhandledRejection` handler; last-resort exception handler does not prevent process termination
...
| **Files** | superset-websocket/src/logger.ts, superset-websocket/src/index.ts |
| **Source Reports** | 16.5.4.md |
| **Related** | None |

**Description:**

No `process.on('unhandledRejection')` handler is registered. In Node.js 15+, unhandled promise rejections terminate the process by default with output to stderr (not to Winston's structured JSON logger). Winston's default `exitOnError: true` means the process terminates after logging synchronous exceptions. Async errors escaping try/catch blocks would terminate the process with unstructured stderr output.

**Remediation:**

Add `process.on('unhandledRejection')` and `process.on('uncaughtException')` handlers that log to the structured Winston logger. Consider setting `exitOnError: false` with graceful shutdown.
...
|--------|---------|-----------------|------------------|
| Flask Backend Api | Connection rate limiting and per-user caps are enforced at the ingress/WAF layer per documented deployment architecture | Promoted from dropped finding ASVS-213-LOW-001 | — |
| Flask Backend Api | Redis backend validates stream ID format independently; invalid IDs are rejected and errors caught gracefully | source: Dropped finding ASVS-221-LOW-001 | — |
| Flask Backend Api | Connection limits enforced at reverse proxy/WAF layer per documented deployment model | Dropped finding ASVS-232-LOW-001 | — |
| Flask Backend Api | Anti-automation and resource exhaustion protection delegated to ingress/WAF per deployment architecture | source: Dropped finding ASVS-241-LOW-001 | — |
| Flask Backend Api | HTTP response headers for websocket sidecar are delegated to reverse proxy by documented design | Documented in deployment_infrastructure_delegated.md and websocket_internal_sidecar.md | — |
| Flask Backend Api | Websocket sidecar authorization model documented as JWT signature verification + channel claim extraction, with data-content control delegated to Flask backend | Documented in websocket_internal_sidecar.md | — |
| Flask Backend Api | Field-level data control delegated to Flask backend which publishes only appropriate fields to trusted Redis infrastructure | Dropped finding ASVS-812-INFO-001 | — |
| Flask Backend Api | Redis is trusted backend infrastructure; Flask backend controls what data enters Redis pipeline | Dropped finding ASVS-823-INFO-001 | — |
| Flask Backend Api | 60-second ping/pong timeout bounds maximum exposure window for revoked sessions on websocket sidecar | Promoted from dropped finding ASVS-824-LOW-001 | — |
| Websocket Jwt Authentication | CORS/Origin on websocket sidecar documented as hardening-only when JWT authentication is present | Documented in websocket_internal_sidecar.md: Missing Origin validation when JWT is present is a hardening improvement, not a vulnerability | — |
| Websocket Jwt Authentication | Implicit token type discrimination via dedicated cookie name (async-token), purpose-specific signing key, and required channel claim | Promoted from dropped finding ASVS-922-LOW-001 | — |
| Websocket Jwt Authentication | Purpose-specific jwtSecret dedicated to Flask backend ↔ WebSocket sidecar channel with structural claim requirement providing implicit audience restriction | Promoted from dropped finding ASVS-923-LOW-001 | — |
| Flask Appbuilder Rbac | Password reset delegated to Flask-AppBuilder; websocket sidecar has no credential lifecycle management | Audit confirmed N/A for 6.4.3 | — |
| Flask Appbuilder Rbac | MFA factor management delegated to Flask-AppBuilder; websocket sidecar has no MFA awareness | Audit confirmed N/A for 6.4.4 | — |
| Flask Appbuilder Rbac | Authentication mechanism renewal delegated to Flask-AppBuilder; sidecar enforces JWT exp via jsonwebtoken library | Audit confirmed N/A for 6.4.5 | — |
| Flask Appbuilder Rbac | Administrative password reset delegated to Flask-AppBuilder; websocket sidecar has no user management capability | Audit confirmed N/A for 6.4.6 | — |
| Flask Appbuilder Rbac | One-time-use MFA factor enforcement delegated to Flask-AppBuilder; websocket sidecar consumes JWTs only | Audit confirmed N/A for 6.5.1 | — |
| Flask Appbuilder Rbac | Lookup secret storage hashing delegated to Flask-AppBuilder; no lookup secrets stored in websocket sidecar | Audit confirmed N/A for 6.5.2 | — |
| Flask Appbuilder Rbac | MFA secret generation delegated to Flask-AppBuilder; sidecar uses crypto.randomUUID() (CSPRNG) for internal IDs | Audit confirmed N/A for 6.5.3 | — |
| Flask Appbuilder Rbac | Lookup secret and OOB code generation delegated to Flask-AppBuilder; websocket sidecar generates no authentication codes | Audit confirmed N/A for 6.5.4 | — |
| Flask Appbuilder Rbac | OOB/TOTP lifetime enforcement delegated to Flask-AppBuilder; sidecar validates JWT exp via jsonwebtoken library defaults | Audit confirmed N/A for ASVS 6.5.5 | — |
| Flask Appbuilder Rbac | Authentication factor revocation delegated to Flask-AppBuilder; sidecar implements connection timeout for bounded session lifetime | Audit confirmed N/A for ASVS 6.5.6 | — |
| Flask Appbuilder Rbac | Biometric authentication delegated to Flask-AppBuilder/external IdP; websocket sidecar uses JWT-only authentication | Audit confirmed N/A for ASVS 6.5.7 | — |
| Flask Appbuilder Rbac | TOTP time source validation delegated to Flask-AppBuilder; websocket sidecar has no TOTP logic | Audit confirmed N/A for ASVS 6.5.8 | — |
| Flask Appbuilder Rbac | PSTN OTP delivery delegated to Flask-AppBuilder; websocket sidecar has no telephony integration | Audit confirmed N/A for ASVS 6.6.1 | — |
| Flask Appbuilder Rbac | OOB authentication binding delegated to Flask-AppBuilder; JWT channel claim provides session-level binding at sidecar | Audit confirmed N/A for ASVS 6.6.2 | — |
| Flask Appbuilder Rbac | OOB code brute-force protection delegated to Flask-AppBuilder; sidecar destroys socket on JWT validation failure with no retry | Audit confirmed N/A for ASVS 6.6.3 | — |
| Flask Appbuilder Rbac | Push notification MFA delegated to Flask-AppBuilder; websocket data push is application delivery, not MFA | Audit confirmed N/A for ASVS 6.6.4 | — |
| Flask Appbuilder Rbac | Certificate-based cryptographic authentication delegated to Flask-AppBuilder; sidecar uses HMAC JWT only | Audit confirmed N/A for ASVS 6.7.1 | — |
| Flask Appbuilder Rbac | Challenge-response cryptographic authentication delegated to Flask-AppBuilder; sidecar uses bearer token model | Audit confirmed N/A for ASVS 6.7.2 | — |
| Flask Appbuilder Rbac | Multi-IdP identity spoofing prevention delegated to Flask-AppBuilder; sidecar has no IdP integration | Audit confirmed N/A for ASVS 6.8.1 | — |
| Flask Appbuilder Rbac | IdP assertion signature validation delegated to Flask-AppBuilder; sidecar validates internal JWT signatures with algorithm allowlist | Audit confirmed N/A for ASVS 6.8.2 | — |
| Flask Appbuilder Rbac | SAML assertion replay prevention delegated to Flask-AppBuilder; no SAML processing in websocket sidecar | Audit confirmed N/A for ASVS 6.8.3 | — |
| Flask Appbuilder Rbac | Authentication strength/acr/amr verification delegated to Flask-AppBuilder; sidecar operates below IdP claim layer | Audit confirmed N/A for ASVS 6.8.4 | — |
| Flask Appbuilder Rbac | Session lifecycle (establishment, expiration, invalidation) delegated to Flask-AppBuilder; sidecar validates JWT at connection time only by design | ASVS 7.1.1 - source: Dropped finding ASVS-711-INFO-001 | — |
| Flask Appbuilder Rbac | Concurrent session policy delegated to Flask-AppBuilder; sidecar supports multiple connections per channel by design (multi-tab support) | ASVS 7.1.2 - Dropped finding ASVS-712-INFO-001 | — |
| Flask Appbuilder Rbac | Federated session management (SSO/OIDC/SAML coordination) delegated to Flask-AppBuilder's AUTH_TYPE-specific manager | ASVS 7.1.3 - Promoted from dropped finding ASVS-713-INFO-001 | — |
| Flask Appbuilder Rbac | Session token verification performed by trusted backend service; dynamic JWT-based session management implemented | ASVS 7.2.1 and 7.2.2 - Pass status | — |
| Flask Appbuilder Rbac | Multiple concurrent WebSocket connections per channel supported by design (multi-tab/window use case); session token rotation is Flask-AppBuilder's responsibility | ASVS 7.2.4 - source: Dropped finding ASVS-724-LOW-001 | — |
| Flask Appbuilder Rbac | Ping/pong liveness timeout (60s default) provides connection cleanup; session inactivity policy delegated to Flask-AppBuilder | ASVS 7.3.1 - source: Dropped finding ASVS-731-LOW-001 | — |
| Flask Appbuilder Rbac | Absolute session lifetime policy delegated to Flask-AppBuilder and JWT exp claim set by token issuer | ASVS 7.3.2 - Session lifecycle delegated to Flask-AppBuilder per flask_appbuilder_security_controls.md; ASVS V3 out of scope | — |
| Flask Appbuilder Rbac | Session termination enforcement delegated to Flask-AppBuilder; sidecar delivers only pre-authorized notification metadata | ASVS 7.4.1 - Dropped finding ASVS-741-LOW-001 | — |
| Flask Appbuilder Rbac | Account lifecycle management (disable/delete) delegated to Flask-AppBuilder; new connections require valid JWT from backend | ASVS 7.4.2 - Promoted from dropped finding ASVS-742-LOW-001 | — |
| Flask Appbuilder Rbac | Authentication factor management and resulting session invalidation delegated to Flask-AppBuilder | ASVS 7.4.3 - Dropped finding ASVS-743-INFO-001 | — |
| Flask Appbuilder Rbac | Admin session termination capability delegated to Flask-AppBuilder; sidecar connections bounded by ping timeout and JWT expiration | ASVS 7.4.5 - Promoted from dropped finding ASVS-745-LOW-001 | — |
| Flask Appbuilder Rbac | User session management UI delegated to Flask-AppBuilder; sidecar is internal event delivery with no user-facing session controls | ASVS 7.5.2 - Dropped finding ASVS-752-LOW-001 | — |
| Flask Appbuilder Rbac | JWT exp validated at connection time; token lifetime policy is issuer's (Flask backend) responsibility per documented design | ASVS 7.6.1 - Promoted from dropped finding ASVS-761-INFO-001 | — |
| Flask Appbuilder Rbac | Session creation requires explicit user action (WebSocket connection initiation); prevents automatic session creation without user interaction | ASVS 7.6.2 - Pass status | — |
| Frontend Browser Security | Session/cookie lifecycle management delegated to Flask backend; sidecar validates but does not own session tokens | Dropped finding ASVS-1431-LOW-001 from 14.3.1 audit | — |
| Frontend Browser Security | Security headers on websocket sidecar HTTP responses are delegated to the reverse proxy by design | Dropped finding ASVS-321-INFO-001 from 3.2.1 audit | — |
| Frontend Browser Security | WebSocket sidecar uses JWT as primary access control mechanism | Origin validation on websocket sidecar when JWT is present is documented as hardening improvement; Sec-Fetch-* validation delegated to reverse proxy | — |
| Frontend Browser Security | Application uses appropriate HTTP methods for sensitive functionality | ASVS 3.5.3 passed - POST, PUT, PATCH, DELETE used appropriately for state-changing operations | — |
| Frontend Browser Security | Applications properly separated by hostname for same-origin policy enforcement | ASVS 3.5.4 passed - separate applications hosted on different hostnames | — |
| Frontend Browser Security | JSONP functionality disabled across application | ASVS 3.5.6 passed - no JSONP endpoints found, preventing XSSI attacks | — |
| Frontend Browser Security | Script resources do not contain authorization-requiring data | ASVS 3.5.7 passed - JavaScript files do not include data requiring authorization | — |
| Frontend Browser Security | Authenticated resources protected from unintended embedding | ASVS 3.5.8 passed - authenticated resources properly validated for intended loading context | — |
| Frontend Browser Security | Application uses only supported and secure client-side technologies | ASVS 3.7.1 passed - no deprecated technologies (Flash, ActiveX, Silverlight, etc.) detected | — |
| Deployment Infrastructure | Encrypted TLS connections used for all inbound and outbound connections | ASVS 12.3.1 passed - TLS is used for all connectivity to and from the application | — |
| Deployment Infrastructure | TLS certificate validation implemented for client connections | ASVS 12.3.2 passed - TLS clients validate certificates before communicating | — |
| Deployment Infrastructure | Trusted certificates used for internal service communications | ASVS 12.3.4 passed - TLS connections between internal services use trusted certificates | — |
| Deployment Infrastructure | Communication requirements documented | ASVS 13.1.1 passed - All communication needs for the application are documented | — |
| Deployment Infrastructure | Connection limiting delegated to reverse proxy/ingress layer for internal sidecar | Rate limiting and connection limits delegated to ingress/WAF layer per deployment_infrastructure_delegated.md; websocket sidecar is internal behind reverse proxy per websocket_internal_sidecar.md | — |
| Deployment Infrastructure | Redis credential and authentication method selection delegated to operator | Promoted from dropped finding ASVS-1321-LOW-001 - authentication configuration is externalized to deployment layer | — |
| Deployment Infrastructure | Connection management and capacity limits delegated to deployment infrastructure layer | Dropped finding ASVS-1326-LOW-001 - connection pooling, timeouts, and retry logic handled at infrastructure level | — |
| Deployment Infrastructure | Source control metadata excluded from production deployments | ASVS 13.4.1 Pass - no .git or .svn folders accessible in deployed application | — |
| Deployment Infrastructure | Debug modes disabled in production | ASVS 13.4.2 Pass - all components configured without debug features enabled | — |
| Deployment Infrastructure | Directory listings disabled on web servers | ASVS 13.4.3 Pass - web server configured to prevent directory browsing | — |
| Deployment Infrastructure | HTTP TRACE method disabled | ASVS 13.4.4 Pass - TRACE method not supported in production environment | — |
| Deployment Infrastructure | Internal documentation and monitoring endpoints protected | ASVS 13.4.5 Pass - internal APIs and monitoring not publicly exposed | — |
| Deployment Infrastructure | Backend component version information suppressed | ASVS 13.4.6 Pass - detailed version headers and banners removed from responses | — |
| Deployment Infrastructure | Web tier file extension allowlist configured | ASVS 13.4.7 Pass - server configured to only serve approved file types | — |
| Deployment Infrastructure | Least privilege service accounts used for backend communications | ASVS 13.2.2 Pass - components communicate with minimal necessary privileges | — |
| Deployment Infrastructure | Default credentials not used for service authentication | ASVS 13.2.3 Pass - all service accounts use non-default credentials | — |
| Deployment Infrastructure | External resource communication allowlist implemented | ASVS 13.2.4 Pass - application restricted to communicating with approved external systems | — |
| Deployment Infrastructure | Web/application server resource allowlist configured | ASVS 13.2.5 Pass - server-level restrictions on accessible resources and systems | — |
| Deployment Infrastructure | Secrets rotation schedule defined | ASVS 13.1.4 Pass - documentation includes secret rotation requirements based on threat model | — |
| Secrets And Cryptography | JWT secret externalized via environment variable for operator-managed rotation | Application delegates cryptographic key management to operators through environment variable configuration, enabling external key rotation without code changes | — |
| Secrets And Cryptography | HS256 default provides 128-bit security with proper operator-provisioned key | HMAC-SHA256 algorithm meets minimum 128-bit security requirement when configured with appropriately sized keys (≥256 bits) | — |
| Secrets And Cryptography | JWT_SECRET environment variable enables operator provisioning from secret managers; example config signals replacement required with 'CHANGE-ME' | Configuration design pattern observed in 11.5.1.md | — |
| Secrets And Cryptography | Default jwtAlgorithms is ['HS256'] (approved); algorithm list not overridable via environment variable, requiring deliberate file change | Hardened configuration pattern observed in 11.6.1.md | — |
| Secrets And Cryptography | Key exchange is delegated to infrastructure-level TLS termination; no DH/ECDH implemented in application code | Architecture observation from 11.6.2.md | — |
| Secrets And Cryptography | Full memory encryption is an infrastructure concern delegated to the deployment layer | Architecture observation from 11.7.1.md | — |
| Secrets And Cryptography | Secrets loaded from environment variables avoiding file persistence; architecture limits secret-holding to single sidecar process | Architecture pattern observed in 11.7.2.md | — |
| Secrets And Cryptography | JWT secret minimum length (32 bytes) enforced at startup with hard exit on failure | Runtime validation observed in 13.3.1.md | src/index.ts:70-72 |
| Secrets And Cryptography | Environment variable injection supports external secrets management integration without native vault coupling | Integration pattern observed in 13.3.1.md | — |
| Secrets And Cryptography | Secrets configurable via environment variables, enabling rotation through container orchestration rolling restarts without application-native rotation support | Deployment pattern observed in 13.3.4.md | — |
| Data Protection | Backend infrastructure (Redis) is a trusted component; data content and authorization enforced at the publisher (Flask backend) layer | Dropped finding ASVS-1426-LOW-001 - authorization controls implemented at application layer | — |
| Logging And Monitoring | Authorization is architecturally embedded in the JWT channelId claim — no separate authorization decision exists in the sidecar to log | Dropped finding ASVS-1632-LOW-001 | websocket_internal_sidecar.md |
| Logging And Monitoring | Log file access control delegated to container/deployment infrastructure with JSON format suitable for centralized log collection | Promoted from dropped finding ASVS-1642-LOW-001 | — |
| Dependency Management | ASF security process governs dependency vulnerability remediation timeframes | Third-party dependency remediation policy delegated to ASF process per third_party_dependency_policy.md | — |
| Dependency Management | package-lock.json with npm ci provides machine-readable dependency inventory with integrity hashes | Provides SBOM functionality for Node.js dependencies with cryptographic integrity verification | package-lock.json |
| Dependency Management | Risky component identification managed through ASF security process | Third-party component risk assessment integrated into organizational security workflow | — |
| Dependency Management | Container image pinning and build validation delegated to deployment pipeline | Infrastructure-level controls ensure component versions remain within documented update timeframes | — |
| Dependency Management | Container image integrity verification delegated to deployment pipeline | Deployment infrastructure validates component sources and prevents dependency confusion attacks | — |
...
| 15.2.1 | Verify that the application only contains components which have not breached the documented update and remediation time frames. | **Partial** |  |
...
| 15.2.4 | Verify that third-party components and all of their transitive dependencies are included from the expected repository, whether internally owned or an external source, and that there is no risk of a dependency confusion attack. | **Partial** |  |
...
| 15.3.1 | Verify that the application only returns the required subset of fields from a data object. For example, it should not return an entire data object, as some individual fields should not be accessible to users. | **Partial** | See FINDING-004 |
...
| 15.3.6 | Verify that JavaScript code is written in a way that prevents prototype pollution, for example, by using Set() or Map() instead of object literals. | **Partial** | See FINDING-005 |
...
| 15.4.4 | Verify that resource allocation policies prevent thread starvation by ensuring fair access to resources, such as by leveraging thread pools, allowing lower-priority threads to proceed within a reasonable timeframe. | **Fail** | See FINDING-001, FINDING-002, FINDING-003 |
...
| 16.2.1 | Verify that each log entry includes necessary metadata (such as when, where, who, what) that would allow for a detailed investigation of the timeline when an event happens. | **Fail** | See FINDING-006, FINDING-007 |
| 16.2.2 | Verify that time sources for all logging components are synchronized, and that timestamps in security event metadata use UTC or include an explicit time zone offset. UTC is recommended to ensure consistency across distributed systems and to prevent confusion during daylight saving time transitions. | **Fail** | See FINDING-006 |
| 16.2.3 | Verify that the application only stores or broadcasts logs to the files and services that are documented in the log inventory. | **Fail** | See FINDING-008 |
...
| 16.3.1 | Verify that all authentication operations are logged, including successful and unsuccessful attempts. Additional metadata, such as the type of authentication or factors used, should also be collected. | **Fail** | See FINDING-007 |
...
| 16.3.3 | Verify that the application logs the security events that are defined in the documentation and also logs attempts to bypass the security controls, such as input validation, business logic, and anti-automation. | **Fail** | See FINDING-010 |
| 16.3.4 | Verify that the application logs unexpected errors and security control failures such as backend TLS failures. | **Fail** | See FINDING-008, FINDING-011 |
...
**Summary Statistics:**
- **Pass**: 111 requirements (32.2%)
- **Partial**: 4 requirements (1.2%)
- **N/A**: 221 requirements (64.1%)
...
|------------|----------|-------------------|------------------|---------------------|
| FINDING-001 | High | 15.4.4 | — | superset-websocket/src/index.ts |
| FINDING-002 | High | 15.4.4 | FINDING-003 | superset-websocket/src/index.ts |
| FINDING-003 | Medium | 15.4.4 | FINDING-002 | superset-websocket/src/index.ts |
| FINDING-004 | Low | 15.3.1 | — | superset-websocket/src/index.ts |
| FINDING-005 | Low | 15.3.6 | — | superset-websocket/src/index.ts |
| FINDING-006 | Low | 16.2.1, 16.2.2 | FINDING-007, FINDING-008, FINDING-010, FINDING-011 | superset-websocket/src/logger.ts |
| FINDING-007 | Low | 16.2.1, 16.3.1 | FINDING-006, FINDING-008, FINDING-010, FINDING-011 | superset-websocket/src/index.ts |
| FINDING-008 | Low | 16.2.3, 16.3.4 | FINDING-006, FINDING-007, FINDING-010, FINDING-011 | superset-websocket/src/index.ts |
| FINDING-009 | Low | 16.2.5 | — | superset-websocket/src/index.ts |
| FINDING-010 | Low | 16.3.3 | FINDING-006, FINDING-007, FINDING-008, FINDING-011 | superset-websocket/src/index.ts |
| FINDING-011 | Low | 16.3.4 | FINDING-006, FINDING-007, FINDING-008, FINDING-010 | superset-websocket/src/index.ts |
| FINDING-012 | Low | 16.5.4 | — | superset-websocket/src/logger.ts, superset-websocket/src/index.ts |

**Total Unique Findings**: 12 (0 Critical, 2 High, 1 Medium, 9 Low, 0 Info)
...
**Total consolidated findings: 12**
