...
This report consolidates findings from 345 source reports covering 27 security domains within the Apache Superset frontend codebase. The audit was conducted at ASVS Level 3 with no severity threshold, meaning all identified issues are included regardless of impact. No critical or high severity findings were identified; the 12 actionable issues cluster around output encoding, injection prevention, and cross-origin security controls.
...
All 27 audited domains were assessed against ASVS Level 3 requirements. Findings span L1, L2, and L3 verification levels, with the majority of issues identified at L2 where frontend-specific encoding and validation controls are expected to provide defense-in-depth alongside server-side enforcement.

### Top 5 Risks

1. **Unsanitized HTML in NVD3 Tooltip Generators Allows Stored XSS** [Medium] — Tooltip generator functions in NVD3 visualizations concatenate user-controlled data into HTML strings without encoding, enabling stored cross-site scripting when malicious payloads are persisted in chart data.
2. **AG Grid Filter Column Name Validation Allows SQL Injection via complexWhere Path** [Medium] — Insufficient validation of column names in AG Grid filter model processing permits attacker-controlled identifiers to reach SQL construction logic through the complexWhere code path.
3. **SafeMarkdown disables URI protocol sanitization via transformLinkUri={null}, allowing javascript: URLs in markdown links** [Medium] — The SafeMarkdown component explicitly nullifies the default URI protocol sanitization, permitting `javascript:` scheme URLs to be rendered as clickable links in markdown content.
4. **postMessage origin validation is commented out in embedded page initializer** [Medium] — The origin check for incoming postMessage events in the embedded dashboard initializer is commented out, allowing any origin to send control messages to the embedded frame.
5. **Number filter values interpolated into SQL without runtime type validation** [Medium] — Numeric filter values from user input are interpolated directly into SQL filter clauses without runtime type assertion, creating a potential injection vector if non-numeric values bypass client-side controls.

### Positive Controls Observed

- **Password reset flow delegation**: Password reset flow fully delegated to Flask-AppBuilder server-side endpoint, eliminating frontend credential handling.
- **Authentication strength verification**: Authentication strength verification (acr/amr/auth_time) is delegated to Flask-AppBuilder's security manager; frontend performs visibility-only RBAC checks.
- **Session lifecycle management**: Session timeout, concurrent session policy, token generation, logout, admin termination, re-authentication, step-up authentication, and federated session coordination are all delegated to Flask-AppBuilder.
- **Server-side permission enforcement**: Backend enforces current permissions on every API request regardless of frontend permission cache state.
- **Admin role trust boundary**: Admin role trust boundary clearly documented; multi-factor and step-up authentication delegated to FAB auth provider configuration.
- **Two-tier validation architecture**: Server-side enforcement acts as final authority, ensuring client-side parser inconsistencies have no security impact.
- **Input preservation with server validation**: TextControl intentionally preserves raw input alongside error state for UX correction workflow; server-side validates all submitted values.
- **Admin-controlled annotation data**: Annotation layer data requires admin-equivalent access to modify, limiting injection surface.
- **Backend SQL parameterization**: Frontend SQL filter construction is defense-in-depth; backend ORM provides primary SQL injection defense via parameterization.
- **JavaScript/JSON output encoding**: JavaScript and JSON output encoding appears adequate based on ASVS 1.2.3 assessment.
- **React JSX auto-escaping**: React JSX auto-escaping used for all user-facing rendering; innerHTML limited to static translated strings with no user-controlled input path.
- **Cookie security delegation**: Cookie creation delegated entirely to server-side; frontend only reads cookies.
- **Security headers delegation**: Security headers (HSTS, CSP, X-Frame-Options) explicitly delegated to deployment infrastructure.
- **HSTS preload delegation**: HSTS preload configuration handled at infrastructure layer (reverse proxy/load balancer).
- **Same-origin policy enforcement**: Separate applications hosted on different hostnames with proper same-origin policy enforcement.
- **No JSONP functionality**: JSONP functionality is not enabled across the application.
- **No sensitive data in script resources**: Authorized data is not included in script resource responses.
- **Sec-Fetch header validation**: Authenticated resources validate request origin using Sec-Fetch-* headers.
- **HTTP security controls**: Content-Type headers correctly set with charset; HTTP methods properly restricted; message boundaries correctly determined; Content-Length consistent with framing; connection-specific headers properly handled in HTTP/2 and HTTP/3; header fields validated to prevent CR/LF injection.
- **URI length limit backstop**: Server-side URI length limit (414) acts as backstop; retry logic excludes 4xx preventing amplification.
- **File upload security**: Server-side file size enforcement, extension allowlist, MIME type validation, and filename sanitization confirmed; frontend provides defense-in-depth validation.
- **OAuth2 scope restriction**: OAuth2 scope configuration restricted to admin-only database connection modal with minimal required scopes.
- **Configuration security delegation**: Blocking .git/.svn directories, directory listing prevention, HTTP TRACE blocking, and file extension filtering delegated to deployment infrastructure.
- **Version disclosure out of scope**: Version/banner disclosure explicitly out of scope per SECURITY.md.
- **Data protection controls**: HTTPS-only caching, selective state persistence, query results stripped from localStorage, cache bypass option available for sensitive requests, explicit sanitization-before-persist pattern.
- **Extension loading gated**: Extension loading gated behind FeatureFlag.EnableExtensions and authentication.
- **Dashboard overwrite precheck**: Optimistic locking available via ConfirmDashboardDiff feature flag.
- **Anti-automation delegation**: Anti-automation and rate limiting delegated to infrastructure WAF/ingress layer by documented design.
...
#### FINDING-001: Unsanitized HTML in NVD3 Tooltip Generators Allows Stored XSS
...
| **Files** | superset-frontend/plugins/legacy-preset-chart-nvd3/src/utils.ts:126, superset-frontend/plugins/legacy-preset-chart-nvd3/src/utils.ts:188 |
| **Source Reports** | 1.1.2.md, 1.3.1.md, 1.3.7.md |
| **Related** | FINDING-003, FINDING-004, FINDING-009 |

**Description:**

Multiple tooltip generator functions in superset-frontend/plugins/legacy-preset-chart-nvd3/src/utils.ts return HTML without sanitization before rendering via innerHTML. Database column values flow through chart data into generateMultiLineTooltipContent (~line 126), generateBubbleTooltipContent (~line 188), and tipFactory without DOMPurify sanitization, unlike generateCompareTooltipContent which correctly applies dompurify.sanitize(). An authenticated user with write access to a datasource can insert malicious HTML/JS into database columns that become chart series keys or entity names, achieving stored XSS against any user viewing the tooltips.

**Remediation:**

Apply dompurify.sanitize() to the final HTML string returned by generateMultiLineTooltipContent, generateBubbleTooltipContent, and tipFactory, matching the pattern used by generateCompareTooltipContent. Change getFormattedKey(series.key, false) to getFormattedKey(series.key, true) in affected functions.

---

#### FINDING-002: AG Grid Filter Column Name Validation Allows SQL Injection via complexWhere Path
...
The COLUMN_NAME_REGEX in superset-frontend/plugins/plugin-chart-ag-grid-table/src/utils/agGridFilterConverter.ts allows spaces, parentheses, and other characters that permit SQL keyword injection through column names. A Gamma+ user crafting direct API requests can inject SQL via the complexWhere/havingClause path which is designed to carry raw SQL fragments. The frontend SQL filter construction creates SQL clauses that may be processed by the backend without sufficient validation.

**Remediation:**

Restrict column name regex to disallow SQL keywords/patterns, or quote column identifiers with proper escaping, or preferably send structured filter objects to the backend instead of constructing SQL on the frontend.

---

#### FINDING-003: NVD3 tooltip generators produce HTML without context-appropriate encoding for HTML element content
...
File: superset-frontend/plugins/legacy-preset-chart-nvd3/src/utils.ts. Functions: generateMultiLineTooltipContent (line ~126), generateBubbleTooltipContent (line ~188). Values like series.key, point[entity], and point.group are interpolated into HTML &lt;td&gt; element content without HTML entity encoding or sanitization. Authenticated user with datasource write access can store XSS payload that executes against chart viewers. This violates ASVS 1.2.1 requirement for context-appropriate output encoding in HTML documents.

**Remediation:**

Apply dompurify.sanitize() to the final HTML string, or use context-appropriate HTML entity encoding for all dynamic values interpolated into HTML element content.

---

#### FINDING-004: SafeMarkdown disables URI protocol sanitization via transformLinkUri={null}, allowing javascript: URLs in markdown links
...
File: superset-frontend/packages/superset-ui-core/src/components/SafeMarkdown/SafeMarkdown.tsx. When the EscapeMarkdownHtml feature flag is enabled, rehype-sanitize is NOT applied, yet transformLinkUri={null} overrides react-markdown's built-in javascript: protocol blocking. An authenticated user with permission to create/edit dashboard markdown components can create a stored XSS requiring one click via [text](javascript:...) links. Affected configurations: (1) EscapeMarkdownHtml enabled: vulnerable, (2) htmlSanitization=false: vulnerable, (3) EscapeMarkdownHtml disabled AND htmlSanitization true: protected. This violates ASVS 1.2.2 requirement to ensure only safe URL protocols are permitted when dynamically building URLs.

**Remediation:**

Remove transformLinkUri={null} to use react-markdown's default URI sanitizer, or provide an explicit safe-protocol allowlist function that blocks javascript:, vbscript:, and data: protocols.

---

#### FINDING-005: postMessage origin validation is commented out in embedded page initializer
...
| **CWE** | CWE-346 |
| **ASVS sections** | 3.5.5 |
| **Files** | superset-frontend/src/embedded/index.tsx |
| **Source Reports** | 3.5.5.md |
| **Related** | |

**Description:**

The embedded dashboard's `validateMessageEvent()` function has origin validation completely commented out. Any origin can establish a MessagePort connection with the embedded page and invoke all registered Switchboard methods. If an attacker possesses a valid guest token, they can establish a full Switchboard channel from an unauthorized origin, call `guestToken` to authenticate, then invoke `getChartDataPayloads` and `getDataMask` to exfiltrate dashboard data that should only be accessible from authorized embedding origins.

**Remediation:**

Uncomment and implement the origin validation using the server-provided allowed domains configuration. The allowed origins should be provided by the server as part of the bootstrap data for the embedded page, derived from the same domain allowlist used for guest token generation.

---

#### FINDING-006: Number filter values interpolated into SQL without runtime type validation
...
| **Files** | superset-frontend/plugins/plugin-chart-ag-grid-table/src/stateConversion.ts |
| **Source Reports** | 15.3.5.md |
| **Related** | FINDING-007 |

**Description:**

In `convertFilterToSQL`, number filter values are interpolated directly into SQL strings without runtime numeric validation. An attacker who can manipulate AG Grid chart state (via permalink, browser tools, or saved state) can inject arbitrary SQL if the backend trusts frontend-generated SQL fragments. Text filters are properly escaped via `escapeStringValue()`, but number filter values have zero runtime validation.

**Remediation:**

Add `Number.isFinite(Number(filter.filter))` validation before interpolating number values into SQL strings. Reject non-numeric values by returning null.

---

#### FINDING-007: Complex filter operator value interpolated into SQL without allowlist validation
...
Validate `filter.operator` against a `Set(['AND', 'OR'])` allowlist before interpolating into SQL. Reject invalid operators by returning null.

### 3.4 Low

#### FINDING-008: Missing Escaping in AG Grid IN_RANGE Filter SQL Clause Construction
...
| **Affected File(s)** | superset-frontend/plugins/plugin-chart-ag-grid-table/src/utils/agGridFilterConverter.ts |
| **Source Report(s)** | 1.3.3.md |
| **Related Finding(s)** | FINDING-002 |

**Description:**

The simpleFilterToWhereClause function in superset-frontend/plugins/plugin-chart-ag-grid-table/src/utils/agGridFilterConverter.ts does not apply escapeSQLString() to IN_RANGE filter values, despite applying it to other string-type filter operators. If the backend processes the complexWhere string without additional validation, this could allow SQL injection through IN_RANGE filter values.

**Remediation:**

Apply escapeSQLString() to IN_RANGE clause values for string-typed filter values, or preferably move SQL construction to the backend with parameterized queries.

---

#### FINDING-009: CSS Injection via Unfiltered Style Attributes in XSS Filter
...
| **Affected File(s)** | superset-frontend/packages/superset-ui-core/src/utils/html.tsx |
| **Source Report(s)** | 1.3.5.md |
| **Related Finding(s)** | FINDING-001, FINDING-003, FINDING-004 |

**Description:**

The FilterXSS configuration in superset-frontend/packages/superset-ui-core/src/utils/html.tsx permits style attributes on whitelisted elements but sets css: false, disabling CSS property validation. An authenticated user who can write data to a queried database can inject CSS enabling UI overlay/phishing attacks against other users viewing charts with allowRenderHtml=true.

**Remediation:**

Enable CSS property filtering by setting the css option to a restrictive allowlist of safe properties, or remove style from the attribute whitelist.

---

#### FINDING-010: EncryptedField Component May Render Unmasked Credentials from Backend Response
...
| **ASVS Level(s)** | L2 |
| **CWE** | CWE-200 |
| **ASVS Section(s)** | 13.3.1 |
| **Affected File(s)** | superset-frontend/src/features/databases/DatabaseModal/DatabaseConnectionForm/EncryptedField.tsx |
| **Source Report(s)** | 13.3.1.md |
| **Related Finding(s)** | |

**Description:**

The EncryptedField component derives encryptedValue directly from backend-provided database parameters. While a useEffect on mount calls onParametersChange to clear the field, React renders synchronously before effects execute, meaning the first render will display whatever the backend returned in db.parameters. If the backend returns unmasked service account credentials (e.g., BigQuery credentials_info), they briefly appear in the DOM. This relates to the project's documented requirement that credential masking is required regardless of caller privilege.

**Remediation:**

Never display existing credential values in edit mode. Initialize encryptedValue to empty string when isEditMode is true, rather than relying on useEffect clearing. Additionally ensure backend masks credentials in database parameter responses via mask_password_info().

---

#### FINDING-011: Object literal used as dictionary with user-influenced keys in `convertFilterModel`
...
| **Affected File(s)** | superset-frontend/plugins/plugin-chart-ag-grid-table/src/stateConversion.ts |
| **Source Report(s)** | 15.3.6.md |
| **Related Finding(s)** | |

**Description:**

In `convertFilterModel`, an object literal `Record<string, string>` is used as a dictionary with user-influenced keys (column IDs from AG Grid filter state). While `validateColumnName` likely rejects prototype-pollution key names, the protection is indirect. The finding is defense-in-depth with very low exploitability.

**Remediation:**

Use `Object.create(null)` for the `sqlClauses` dictionary or use a `Map<string, string>` instead of an object literal.

---

#### FINDING-012: Request Parameter Override Potential in URL Construction
...
| **Affected File(s)** | superset-frontend/src/explore/exploreUtils/index.ts |
| **Source Report(s)** | 15.3.7.md |
| **Related Finding(s)** | |
...
Add parameter precedence validation with a RESERVED_PARAMS set to prevent caller-supplied requestParams from overwriting security-relevant parameters.
...
|--------|---------|-----------------|------------------|
| Authentication Mechanisms | Password reset flow fully delegated to Flask-AppBuilder server-side endpoint | source: Dropped finding ASVS-643-INFO-001 | — |
| Authentication Mechanisms | Authentication strength verification (acr/amr/auth_time) is delegated to Flask-AppBuilder's security manager; frontend performs visibility-only RBAC checks | Dropped finding ASVS-684-LOW-001 | — |
| Session Management | Session timeout configuration delegated to Flask-AppBuilder | Session lifecycle management including inactivity and absolute timeouts is handled by Flask-AppBuilder framework | — |
| Session Management | Concurrent session policy delegated to Flask-AppBuilder | Session lifecycle and concurrent-session policy delegated to Flask-AppBuilder per flask_appbuilder_security_controls.md | — |
| Session Management | Federated identity session coordination delegated to Flask-AppBuilder AUTH_TYPE-specific manager | OAuth/OIDC/SAML protocol-level enforcement and session lifecycle delegated to Flask-AppBuilder per flask_appbuilder_security_controls.md | — |
| Session Management | Session token generation delegated to Flask-AppBuilder (uses Python secrets module/CSPRNG) | Session lifecycle delegated to Flask-AppBuilder per flask_appbuilder_security_controls.md; token generation is server-side FAB responsibility | — |
| Session Management | Logout functionality provided by Flask-AppBuilder session lifecycle delegation | Session lifecycle and logout functionality delegated to Flask-AppBuilder per flask_appbuilder_security_controls.md | — |
| Session Management | Admin session termination delegated to Flask-AppBuilder | Administrative controls for session termination handled by Flask-AppBuilder framework | — |
| Session Management | Re-authentication enforcement delegated to Flask-AppBuilder session/auth layer | Re-authentication requirements for sensitive operations handled by Flask-AppBuilder authentication layer | — |
| Session Management | User session viewing/termination delegated to Flask-AppBuilder | Session lifecycle including concurrent-session management delegated to Flask-AppBuilder per flask_appbuilder_security_controls.md | — |
| Session Management | Step-up authentication enforcement delegated to Flask-AppBuilder auth layer | Additional authentication factors for highly sensitive operations handled by Flask-AppBuilder | — |
| Session Management | Federated re-authentication enforcement delegated to Flask-AppBuilder | OAuth/OIDC/SAML protocol-level enforcement delegated to Flask-AppBuilder AUTH_TYPE-specific manager per flask_appbuilder_security_controls.md | — |
| Authorization Rbac | Backend enforces current permissions on every API request regardless of frontend permission cache state | API requests validate permissions server-side in real-time without relying on cached permission states | — |
| Authorization Rbac | Admin role trust boundary clearly documented; multi-factor and step-up authentication delegated to FAB auth provider configuration | Administrative access controls leverage external authentication provider for MFA and adaptive authentication | — |
| Input Validation | Two-tier validation architecture with server-side enforcement as final authority ensures client-side parser inconsistencies have no security impact | Promoted from dropped finding ASVS-153-LOW-001 | — |
| Input Validation | TextControl intentionally preserves raw input alongside error state for UX correction workflow; server-side validates all submitted values | Promoted from dropped finding ASVS-221-LOW-001 | — |
| Injection Prevention | Annotation layer data is admin-controlled, requiring admin-equivalent access to modify | Dropped finding ASVS-112-LOW-001 and ASVS-131-LOW-001 | — |
| Injection Prevention | Frontend SQL filter construction is defense-in-depth; backend ORM provides primary SQL injection defense via parameterization | Promoted from dropped finding ASVS-124-MED-001 | — |
| Output Encoding | JavaScript and JSON output encoding appears adequate based on ASVS 1.2.3 assessment | No findings reported for dynamic JavaScript/JSON content generation | — |
| Business Logic Security | Dashboard overwrite precheck (optimistic locking) available via ConfirmDashboardDiff feature flag | Promoted from dropped finding ASVS-234-INFO-001 | — |
| Business Logic Security | Anti-automation and rate limiting delegated to infrastructure WAF/ingress layer by documented design | Dropped finding ASVS-242-LOW-001 | — |
| Browser Security Controls | Security headers (HSTS, CSP, X-Frame-Options) explicitly delegated to deployment infrastructure | Documented delegation approach for security headers implementation | — |
| Browser Security Controls | React JSX auto-escaping used for all user-facing rendering; innerHTML limited to static translated strings with no user-controlled input path | Safe rendering practices prevent unintended content execution | — |
| Browser Security Controls | Cookie creation delegated entirely to server-side; frontend only reads cookies | Cookie security attributes managed by backend, reducing client-side risk | — |
| Browser Security Controls | HSTS preload is delegated to deployment infrastructure (reverse proxy/load balancer) | HSTS preload configuration handled at infrastructure layer | — |
| Cross Origin Security | Separate applications are hosted on different hostnames with proper same-origin policy enforcement | Application architecture properly separates applications by hostname | — |
| Cross Origin Security | JSONP functionality is not enabled across the application | No JSONP endpoints or callback parameters found in API responses | — |
| Cross Origin Security | Authorized data is not included in script resource responses | JavaScript files do not contain sensitive or user-specific data | — |
| Cross Origin Security | Authenticated resources validate request origin using Sec-Fetch-* headers | Resource endpoints properly validate Sec-Fetch-Site and Sec-Fetch-Mode headers | — |
| Http Security | Content-Type headers correctly set with charset parameter for all HTTP responses | ASVS 4.1.1 verification passed | — |
| Http Security | HTTP methods properly restricted to explicitly supported methods | ASVS 4.1.4 verification passed | — |
| Http Security | HTTP message boundaries correctly determined to prevent request smuggling | ASVS 4.2.1 verification passed | — |
| Http Security | Content-Length headers consistent with message framing | ASVS 4.2.2 verification passed | — |
| Http Security | Connection-specific header fields properly handled in HTTP/2 and HTTP/3 | ASVS 4.2.3 verification passed | — |
| Http Security | HTTP/2 and HTTP/3 header fields validated to prevent CR/LF injection | ASVS 4.2.4 verification passed | — |
| Http Security | Server-side URI length limit (414) acts as backstop; retry logic excludes 4xx preventing amplification | ASVS 4.2.5 positive control observed | — |
| File Upload Security | Server-side file size enforcement confirmed by domain context; frontend delegates authoritative validation to backend API endpoints | Dropped finding ASVS-521-LOW-001 | — |
| File Upload Security | Server-side extension allowlist and MIME type validation confirmed; UploadDataModel implements client-side validateUploadFileExtension() as defense-in-depth | Dropped finding ASVS-522-LOW-001 | — |
| File Upload Security | Backend filename sanitization confirmed by domain context; numeric endpoint construction prevents URL path injection; server determines storage path | Promoted from dropped finding ASVS-532-LOW-001 | — |
| Oauth Oidc Security | OAuth2 scope configuration is restricted to admin-only database connection modal | Dropped finding ASVS-1023-INFO-001 - OAuth client requests minimal required scopes | — |
| Configuration Security | Blocking access to .git/.svn directories is delegated to deployment infrastructure (reverse proxy) | Report finding scope assessment | — |
| Configuration Security | Directory listing prevention delegated to deployment infrastructure (reverse proxy) | Report finding scope assessment | — |
| Configuration Security | HTTP TRACE method blocking delegated to deployment infrastructure (reverse proxy) | Report finding scope assessment | — |
| Configuration Security | Version/banner disclosure explicitly out of scope per SECURITY.md: Out of Scope | Report INFO finding ASVS-1346-INFO-001 | — |
| Configuration Security | Web server file extension filtering delegated to deployment infrastructure (reverse proxy) | Report scope assessment | — |
| Data Protection | HTTPS-only caching, GET-only caching, ETag presence required, 200-status-only caching in Browser Cache API | source: Dropped finding ASVS-1425-LOW-001 | — |
| Data Protection | Query results stripped from localStorage persistence, selective state persistence with backend persistence feature flag | Promoted from dropped finding ASVS-1431-LOW-001 | — |
| Data Protection | Cache bypass option (cache: 'no-store') available to callers for sensitive requests; in-memory Map cache cleared on page refresh | source: Dropped finding ASVS-1432-LOW-001 | — |
| Data Protection | HTTPS-only Cache Storage, ETag presence required, query results stripped from localStorage via emptyQueryResults, explicit sanitization-before-persist pattern | Promoted from dropped finding ASVS-1433-LOW-001 | — |
| Dependency Management | Extension loading gated behind FeatureFlag.EnableExtensions and authentication | 15.1.3.md - Noted in dropped finding ASVS-1513-INFO-001 | — |
| Dependency Management | Extension system gated by feature flag, authentication, and per-extension error isolation | 15.2.2.md - Promoted from dropped finding ASVS-1522-INFO-001 | — |
| Dependency Management | Extensions are trusted code with documented trust boundary at install/deploy step; feature flag kill-switch, authentication gate, and admin-only installation provide layered access controls | 15.2.5.md - Dropped finding ASVS-1525-INFO-001 | — |
| Application Security Architecture | Explicit field allowlist in PUT payload construction (saveData built field-by-field) | source: Dropped finding ASVS-1533-INFO-001 | — |
| Logging Monitoring | Authentication logging delegated to Flask-AppBuilder server-side; frontend is not authoritative source for auth events | Report architecture observation in 16.3.1.md | — |
| Logging Monitoring | Authorization failure logging delegated to Flask-AppBuilder server-side; frontend only displays results | Report architecture observation in 16.3.2.md | — |
| Logging Monitoring | Security event logging delegated to server-side Flask-AppBuilder; frontend telemetry is UX analytics only | Dropped finding ASVS-1633-LOW-001 in 16.3.3.md | — |
| Logging Monitoring | Error logging handled server-side where errors originate; frontend handles display only | Dropped finding ASVS-1634-LOW-001 in 16.3.4.md | — |
| Logging Monitoring | safeStringify provides JSON encoding preventing structural log injection in transmission format | Noted in dropped finding ASVS-1641-LOW-001 in 16.4.1.md | — |
| Logging Monitoring | Log transmission authenticated via session cookies or guest token; log protection (immutability, access control) enforced server-side | Report positive patterns in 16.4.2.md | — |
...
| 1.2.4 | Verify that data selection or database queries (e.g., SQL, HQL, NoSQL, Cypher) use parameterized queries, ORMs, entity frameworks, or are otherwise protected from SQL Injection and other database injection attacks. This is also relevant when writing stored procedures. | **Partial** |  |
...
| 1.3.1 | Verify that all untrusted HTML input from WYSIWYG editors or similar is sanitized using a well-known and secure HTML sanitization library or framework feature. | **Fail** | See FINDING-001 |
...
| 1.3.3 | Verify that data being passed to a potentially dangerous context is sanitized beforehand to enforce safety measures, such as only allowing characters which are safe for this context and trimming input which is too long. | **Partial** | See FINDING-008 |
| 1.3.4 | Verify that user-supplied Scalable Vector Graphics (SVG) scriptable content is validated or sanitized to contain only tags and attributes (such as draw graphics) that are safe for the application, e.g., do not contain scripts and foreignObject. | **Pass** |  |
| 1.3.5 | Verify that the application sanitizes or disables user-supplied scriptable or expression template language content, such as Markdown, CSS or XSL stylesheets, BBCode, or similar. | **Partial** | See FINDING-009 |
...
| 3.2.1 | Verify that security controls are in place to prevent browsers from rendering content or functionality in HTTP responses in an incorrect context (e.g., when an API, a user-uploaded file or other resource is requested directly). Possible controls could include: not serving the content unless HTTP request header fields (such as Sec-Fetch-\*) indicate it is the correct context, using the sandbox directive of the Content-Security-Policy header field or using the attachment disposition type in the Content-Disposition header field. | **Partial** |  |
...
| 3.5.5 | Verify that messages received by the postMessage interface are discarded if the origin of the message is not trusted, or if the syntax of the message is invalid. | **Fail** | See FINDING-005 |
...
| **V5: File Handling** | | | |
| 5.1.1 | Verify that the documentation defines the permitted file types, expected file extensions, and maximum size (including unpacked size) for each upload feature. Additionally, ensure that the documentation specifies how files are made safe for end-users to download and process, such as how the application behaves when a malicious file is detected. | **Pass** |  |
...
| 6.1.2 | Verify that a list of context-specific words is documented in order to prevent their use in passwords. The list could include permutations of organization names, product names, system identifiers, project codenames, department or role names, and similar. | **Pass** |  |
...
| 6.2.4 | Verify that passwords submitted during account registration or password change are checked against an available set of, at least, the top 3000 passwords which match the application's password policy, e.g. minimum length. | **Pass** |  |
...
| 6.2.12 | Verify that passwords submitted during account registration or password changes are checked against a set of breached passwords. | **Pass** |  |
...
| 15.3.5 | Verify that the application explicitly ensures that variables are of the correct type and performs strict equality and comparator operations. This is to avoid type juggling or type confusion vulnerabilities caused by the application code making an assumption about a variable type. | **Fail** | See FINDING-006, FINDING-007 |
| 15.3.6 | Verify that JavaScript code is written in a way that prevents prototype pollution, for example, by using Set() or Map() instead of object literals. | **Partial** | See FINDING-011 |
| 15.3.7 | Verify that the application has defenses against HTTP parameter pollution attacks, particularly if the application framework makes no distinction about the source of request parameters (query string, body parameters, cookies, or header fields). | **Partial** | See FINDING-012 |
...
| 16.2.4 | Verify that logs can be read and correlated by the log processor that is in use, preferably by using a common logging format. | **N/A** |  |
...
| 16.5.3 | Verify that the application fails gracefully and securely, including when an exception occurs, preventing fail-open conditions such as processing a transaction despite errors resulting from validation logic. | **N/A** |  |
...
**Summary Statistics:**
- **Pass**: 121 requirements (35.1%)
- **Partial**: 8 requirements (2.3%)
- **N/A**: 210 requirements (60.9%)
- **Fail**: 6 requirements (1.7%)
...
|------------|----------|-------------------|------------------|---------------------|
| FINDING-001 | Medium | 1.1.2, 1.3.1, 1.3.7 | FINDING-003, FINDING-004, FINDING-009 | superset-frontend/plugins/legacy-preset-chart-nvd3/src/utils.ts, superset-frontend/plugins/legacy-preset-chart-nvd3/src/utils.ts |
| FINDING-002 | Medium | 1.3.7 | FINDING-008 | superset-frontend/plugins/plugin-chart-ag-grid-table/src/utils/agGridFilterConverter.ts |
| FINDING-003 | Medium | 1.2.1 | FINDING-001, FINDING-004, FINDING-009 | superset-frontend/plugins/legacy-preset-chart-nvd3/src/utils.ts, superset-frontend/plugins/legacy-preset-chart-nvd3/src/utils.ts |
| FINDING-004 | Medium | 1.2.2 | FINDING-001, FINDING-003, FINDING-009 | superset-frontend/packages/superset-ui-core/src/components/SafeMarkdown/SafeMarkdown.tsx |
| FINDING-005 | Medium | 3.5.5 | — | superset-frontend/src/embedded/index.tsx |
| FINDING-006 | Medium | 15.3.5 | FINDING-007 | superset-frontend/plugins/plugin-chart-ag-grid-table/src/stateConversion.ts |
| FINDING-007 | Medium | 15.3.5 | FINDING-006 | superset-frontend/plugins/plugin-chart-ag-grid-table/src/stateConversion.ts |
| FINDING-008 | Low | 1.3.3 | FINDING-002 | superset-frontend/plugins/plugin-chart-ag-grid-table/src/utils/agGridFilterConverter.ts |
| FINDING-009 | Low | 1.3.5 | FINDING-001, FINDING-003, FINDING-004 | superset-frontend/packages/superset-ui-core/src/utils/html.tsx |
| FINDING-010 | Low | 13.3.1 | — | superset-frontend/src/features/databases/DatabaseModal/DatabaseConnectionForm/EncryptedField.tsx |
| FINDING-011 | Low | 15.3.6 | — | superset-frontend/plugins/plugin-chart-ag-grid-table/src/stateConversion.ts |
| FINDING-012 | Low | 15.3.7 | — | superset-frontend/src/explore/exploreUtils/index.ts |

**Total Unique Findings**: 12 (0 Critical, 0 High, 7 Medium, 5 Low, 0 Info)
