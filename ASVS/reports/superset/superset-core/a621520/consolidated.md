...
This audit evaluated the superset-core repository against ASVS Level 3 requirements across 25 security domains. The codebase demonstrates strong adherence to L1 and L2 controls, with all four findings identified at L1–L2 requirements and no gaps detected at L3-exclusive controls within the audited scope.

### Top 5 Risks

1. **TaskProperties Schema Exposes Stack Traces and Exception Types to Task Consumers** [Low] — The task properties schema includes raw exception type names and stack trace information that could leak internal implementation details to downstream task consumers.
2. **Abstract DAO Interface Accepts Unfiltered Attribute Dictionaries** [Info] — The BaseDAO abstract interface accepts `dict[str, Any]` for create/update attributes without declaring a contract for upstream validation, relying entirely on callers to sanitize inputs.
3. **DAO Interface Uses `Any` Type for Filter and Attribute Parameters Without Runtime Enforcement** [Info] — Generic `Any`-typed parameters in the DAO layer bypass static type checking, meaning type confusion bugs would not be caught until runtime.
4. **Type definitions include security-sensitive fields without role-based projection annotations** [Info] — Model type definitions expose fields such as connection credentials and internal identifiers without declarative annotations indicating which roles may access them.
5. No additional findings — the repository exhibited strong security posture across all evaluated domains.

### Positive Controls Observed

- **Extension trust model**: Extension installation is gated to admin-only; extension developers are fully trusted principals operating inside the application trust boundary with full privilege by design.
- **Python type safety via Pydantic models**: The server-side extension system uses Pydantic models with strict typing, eliminating JavaScript-specific vulnerability classes such as prototype pollution.
- **DAO architectural separation**: DAOs never directly access HTTP request parameters, reducing attack surface for parameter pollution at the data layer; validation occurs at the API boundary via Marshmallow schemas.
- **Consistent DAO pattern with base_filter**: All entity types follow the same BaseDAO pattern with `base_filter` support, providing uniform RBAC application across data access operations.
- **Task deduplication with database-level unique index**: `TaskDAO.find_by_task_key()` combined with a unique index on `dedup_key` provides TOCTOU prevention at the persistence layer and anti-automation at the application logic level.
- **RBAC enforcement via Flask-AppBuilder**: Function-level access control implemented via `@protect()` decorator and Flask-AppBuilder's permission model with session lifecycle management.
- **Session security**: Session tokens are generated using cryptographically secure PRNG with ≥128 bits of entropy; new tokens are issued on authentication and prior sessions are terminated.
- **Token verification delegation**: Token signature verification, algorithm allowlist enforcement, key material trust validation, exp/nbf verification, audience claim validation, and token type differentiation are all delegated to Flask-AppBuilder's AUTH_TYPE-specific managers.
- **Secure-by-default CSRF/auth**: MCP tool and prompt decorators default `protect=True` for secure-by-default CSRF and authentication enforcement.
- **Task system resilience**: Task infrastructure implements timeout enforcement at decorator, invocation, and storage levels with abort handler mechanisms and explicit timeout status tracking.
- **Cache and deployment security**: Cache backends are trusted infrastructure with encryption as a deployment concern; debug modes are disabled in production; documentation and monitoring endpoints are properly secured.
- **Semantic layer access control**: Semantic layer creation and configuration restricted to admin users (trusted operators) with network egress filtering delegated to deployment infrastructure.
- **Data minimization**: Enforced at the serialization boundary via `schema.dump()` / `list_columns` projection; `KeyValue` model supports bounded lifetime via `expires_on` for automatic expiration of sensitive data.
- **HTTP protocol hardening**: Intermediary header overwrite/stripping delegated to reverse proxy layer; Flask/Werkzeug returns first value for duplicate query parameters providing consistent HPP mitigation.
- **Authentication logging**: Authentication lifecycle including logging delegated to Flask-AppBuilder framework.
- **Frontend manifest validation**: `ManifestFrontend` model defines the contract for frontend JavaScript artifacts, enabling downstream validation tooling to verify frontend builds meet security requirements.
- **Rate limiting delegation**: Rate limiting and anti-automation delegated to ingress/WAF layer by documented design.
- **Dependency and availability management**: Application documentation defines risk-based remediation timeframes for third-party component vulnerabilities and identifies resource-intensive functionality with availability protection measures.
...
#### FINDING-001: TaskProperties Schema Exposes Stack Traces and Exception Types to Task Consumers
...
| **File(s)** | superset-core/src/superset_core/tasks/types.py |
| **Source Report(s)** | 16.5.1.md |
| **Related Finding(s)** | None |

**Description:**

The TaskProperties TypedDict defines stack_trace and exception_type as first-class fields in the task's stored state. If API endpoints return task properties without filtering these fields, internal file paths, library versions, and application architecture details could be disclosed. DOWNGRADED: No PoC against running system; based on type definition analysis without reviewing actual serialization/API layer; actual exposure unconfirmed.

**Remediation:**

Verify that task status API endpoints filter stack_trace and exception_type from consumer-facing responses. If no such filtering exists, implement schema-based field projection to return only consumer-safe properties.

---

### 3.5 Informational

#### FINDING-002: Abstract DAO Interface Accepts Unfiltered Attribute Dictionaries
...
| **Files** | superset-core/src/superset_core/common/daos.py:79-93 |
| **Source Reports** | 15.3.3.md |
| **Related** | None |

**Description:**

The BaseDAO abstract interface defines create() and update() methods that accept dict[str, Any] for attributes parameter without filtering. Data flow: HTTP request body → (API schema layer not shown) → DAO.create(attributes=dict) / DAO.update(attributes=dict) → ORM model. This is an informational observation, not a demonstrated vulnerability. The DAO layer is an internal interface — mass assignment protection is architecturally expected at the API/schema layer (Marshmallow schemas, Flask-AppBuilder's add_columns/edit_columns projections) which filters request JSON before it reaches the DAO. Without a concrete implementation showing the full request-to-DAO data flow, no exploitable mass assignment can be demonstrated. The interface design is standard for DAO patterns — accepting dict[str, Any] is appropriate for an internal data access layer. The security boundary for field allowlisting is at the controller/schema level.

**Remediation:**

No remediation required at this layer. Ensure that concrete host implementations: 1) Use Marshmallow schemas with explicit field declarations (fields = (...) or load_only/dump_only markers), 2) Never pass request.json directly to DAO.create(attributes=request.json) without schema validation, 3) Use Flask-AppBuilder's add_columns / edit_columns lists in ModelRestApi subclasses. Short-term: Document field-allowlisting expectations in DAO interface contract by adding docstring guidance to BaseDAO.create() and BaseDAO.update() stating that concrete implementations MUST NOT pass user-controlled dictionaries directly without schema validation. Long-term: Consider adding an allowed_attributes class variable to BaseDAO to provide an optional mechanism for concrete implementations to declare which fields are writable, shifting mass assignment protection closer to the data layer as defense-in-depth.

---

#### FINDING-003: DAO Interface Uses `Any` Type for Filter and Attribute Parameters Without Runtime Enforcement
...
| **Files** | superset-core/src/superset_core/common/daos.py:75-76, 79-93, superset-core/src/superset_core/tasks/daos.py:56 |
| **Source Reports** | 15.3.5.md |
| **Related** | None |

**Description:**

Python type annotations are present but not enforced at runtime in abstract DAO interfaces. The `find_one_or_none` method accepts `**filter_by: Any` and the `create` method accepts `attributes: dict[str, Any]`. If a string value is passed where an integer is expected (e.g., `user_id="1"` instead of `user_id=1`), SQLAlchemy may perform implicit casting or comparison with unexpected results depending on the database backend. This could lead to filter bypass in edge cases. However, SQLAlchemy's ORM layer handles type coercion consistently for most backends, and runtime type enforcement is expected at the API schema layer (Marshmallow deserialization enforces types before values reach the DAO). No concrete exploitable type confusion path is demonstrable from these abstract interfaces alone. Data flow: Caller passes arbitrary types → `**filter_by: Any` / `attributes: dict[str, Any]` → SQLAlchemy query construction. Attacker capability required: Authenticated user who can influence parameter values passed to DAO methods through API endpoints. Impact: If a string value is passed where an integer is expected, SQLAlchemy may perform implicit casting or comparison with unexpected results depending on the database backend. This could lead to filter bypass in edge cases.

**Remediation:**

Concrete implementations should add runtime type assertions. Example: `if user_id is not None and not isinstance(user_id, int): raise TypeError(f"user_id must be int, got {type(user_id)}")`. For scope parameters, validate against allowed values: `if scope not in ("private", "shared", "system"): raise ValueError(f"Invalid scope: {scope}")`. Consider using Pydantic models or similar runtime validation libraries for DAO method parameters to enforce type safety at runtime.

---

#### FINDING-004: Type definitions include security-sensitive fields without role-based projection annotations
...
| **Files** | superset-core/src/superset_core/queries/types.py:86-96, superset-core/src/superset_core/tasks/types.py:51-75 |
| **Source Reports** | 15.3.1.md |
| **Related** | None |

**Description:**

The StatementResult dataclass includes executed_sql which is explicitly documented as containing 'The SQL statement after transformations (RLS, mutations, limits).' Returning this field to non-admin users could reveal Row-Level Security filter logic, potentially enabling users to understand and probe security boundaries. Similarly, TaskProperties includes stack_trace which could reveal internal code paths, file structures, and library versions. Data flow: Query execution engine → StatementResult → (unknown serialization layer) → API response. Attacker capability required: Authenticated non-admin user with query execution permissions. Impact: Information disclosure of RLS filter logic (could aid RLS bypass attempts) or internal stack traces (could aid further exploitation). These are type definitions only. Without the API serialization layer in scope, this is an architectural observation for the serialization layer to address, not a confirmed vulnerability in the audited code.

**Remediation:**

When serializing StatementResult for API responses, apply role-based field projection using schema-based projection at API boundary. For example, use a schema class that filters executed_sql for non-admin/Alpha roles using a post_dump method. For TaskProperties, filter stack_trace and exception_type for non-admin users by creating a serialization function that conditionally includes these fields based on user role.
...
|--------|---------|-----------------|------------------|
| Extension Plugin Architecture | Extension installation is admin-only; extension developers are fully trusted principals inside the trust boundary | Extension installation gated to admin-only; extension developers operate inside application trust boundary with full privilege by design | — |
| Extension Plugin Architecture | Python type safety using Pydantic models with strict typing | The server-side extension system uses Pydantic models which are not susceptible to prototype pollution (a JavaScript-specific vulnerability class) | superset-core/src/superset_core/extensions/types.py |
| Extension Plugin Architecture | Frontend manifest schema with validation | The ManifestFrontend model defines the contract for frontend JavaScript artifacts, enabling downstream validation tooling to verify frontend builds meet security requirements | superset-core/src/superset_core/extensions/types.py |
| Rest Api Input Validation | DAO interface correctly separates data access contract from input validation concerns, following the principle that validation occurs at the API boundary (trusted service layer), not deep in the data access layer | Abstract DAO interface design pattern | superset-core/src/superset_core/common/daos.py |
| Rest Api Input Validation | base_filter mechanism on all DAO classes ensures read operations are filtered, which prevents unauthorized data access even if write operations were somehow manipulated | base_filter class variable present in BaseDAO | superset-core/src/superset_core/common/daos.py |
| Rest Api Input Validation | @protect() decorator demonstrates awareness of endpoint-level access control enforcement | Shown in api() decorator docstring example | superset-core/src/superset_core/common/daos.py |
| Rest Api Input Validation | Consistent DAO Pattern across all entity types with base_filter support provides uniform RBAC application | All entity types (Dataset, Database, Chart, Dashboard, User, Tag, KeyValue, Task, SemanticLayer, SemanticView) follow the same BaseDAO pattern | superset-core/src/superset_core/common/daos.py |
| Rest Api Input Validation | Task deduplication logic provides anti-automation at application logic level | TaskDAO.find_by_task_key() method with deduplication logic (only returning active tasks, unique index on dedup_key) | superset-core/src/superset_core/common/daos.py |
| Rest Api Input Validation | Static type annotations provide static analysis safety | All DAO files use Python type annotations consistently, enabling static analysis tools (mypy) to catch type confusion at development time | superset-core/src/superset_core/common/daos.py, superset-core/src/superset_core/tasks/daos.py |
| Rest Api Input Validation | Explicit typed parameters for security-sensitive operations | TaskDAO.find_by_task_key() uses explicit typed parameters (task_type: str, task_key: str, scope: str, user_id: int \| None) rather than **kwargs: Any | superset-core/src/superset_core/tasks/daos.py:56 |
| Rest Api Input Validation | Explicit typed parameters for uniqueness validation | AbstractSemanticLayerDAO.validate_uniqueness() and AbstractSemanticViewDAO.validate_uniqueness() use explicitly typed parameters (name: str, layer_uuid: str) rather than generic dictionaries | — |
| Rest Api Input Validation | SQLAlchemy ORM type coercion | SQLAlchemy's ORM layer handles type coercion consistently for most backends, providing implicit type safety | — |
| Rest Api Input Validation | Runtime type enforcement at API schema layer | Marshmallow deserialization enforces types before values reach the DAO (expected at API layer, not visible in provided files) | — |
| Rest Api Input Validation | Architectural separation between DAO layer and API layer | DAOs never directly access HTTP request parameters, reducing attack surface for parameter pollution at the data layer | common/daos.py, semantic_layers/daos.py, tasks/daos.py |
| Rest Api Input Validation | Consistent request parsing model via Flask-AppBuilder BaseApi | RestApi inherits from BaseApi providing consistent request parsing, avoiding ad-hoc parameter extraction | rest_api/api.py |
| Rest Api Input Validation | Flask/Werkzeug default first-value behavior for duplicate parameters | Flask/Werkzeug returns first value for duplicate query parameters via request.args.get(), providing consistent behavior against HPP | rest_api/api.py |
| Rest Api Input Validation | Rate limiting and anti-automation delegated to ingress/WAF layer by documented design | Report acknowledgement of documented delegation | — |
| Command Injection And Ssrf | Semantic layer configurations are admin-only, inside the trust boundary | Dropped finding ASVS-136-INFO-001 from ASVS 1.3.6 audit | — |
| Command Injection And Ssrf | Semantic layer configuration is admin-only (inside trust boundary); network egress filtering delegated to deployment infrastructure | Dropped finding ASVS-1324-INFO-001 from ASVS 13.2.4 audit | — |
| Authentication And Session Management | Session lifecycle including invalidation on account deactivation is delegated to Flask-AppBuilder's per-request user.active check | Promoted from dropped finding ASVS-742-LOW-001 | — |
| Authentication And Session Management | Session token verification is performed using trusted backend service (Flask-AppBuilder) | ASVS 7.2.1 - Pass status | — |
| Authentication And Session Management | Application uses dynamically generated session tokens (not static API secrets) | ASVS 7.2.2 - Pass status | — |
| Authentication And Session Management | Session tokens are generated using cryptographically secure PRNG with at least 128 bits of entropy | ASVS 7.2.3 - Pass status | — |
| Authentication And Session Management | New session tokens are generated on user authentication and current sessions are terminated | ASVS 7.2.4 - Pass status | — |
| Authorization And Rbac | RBAC enforcement delegated to Flask-AppBuilder's @protect() decorator and permission model | Function-level access control implemented via Flask-AppBuilder framework | — |
| Authorization And Rbac | Admin role documented as fully trusted operational principal equivalent to OS-level access; adaptive authentication delegated to IdP via FAB | Administrative access controls leverage identity provider integration through Flask-AppBuilder | — |
| Cryptography And Secrets | Data minimization enforced at serialization boundary via schema.dump() / list_columns projection rather than at model definition layer | Observed in serialization implementation | — |
| Cryptography And Secrets | KeyValue model supports bounded lifetime via expires_on attribute for automatic expiration of sensitive data | KeyValue model implementation | — |
| Inter Service Communication | Task system provides timeout, cleanup, and abort infrastructure (TaskContext.on_cleanup, on_abort, TaskOptions.timeout) | Promoted from dropped finding ASVS-1313-LOW-001 | — |
| Inter Service Communication | Task workers run in same trust boundary as application; TaskScope enum provides application-level access control | Promoted from dropped finding ASVS-1321-LOW-001 | — |
| Inter Service Communication | Semantic layer creation restricted to admin users (trusted operators) | Admin role is fully trusted and semantic layer creation is admin-only per Trust Boundaries §1 | — |
| Inter Service Communication | Runtime schema JSON validation constrains user-provided parameters | Noted in dropped finding ASVS-1325-INFO-001 | — |
| Inter Service Communication | Task system implements timeout enforcement at decorator, invocation, and storage levels with abort handler mechanism and explicit timeout status tracking | source: Dropped finding ASVS-1326-INFO-002 | — |
| Deployment And Configuration | Debug modes are disabled in production environments | Section 13.4.2 passed verification | — |
| Deployment And Configuration | Documentation and monitoring endpoints are properly secured | Section 13.4.5 passed verification | — |
| Deployment And Configuration | Application documentation defines risk-based remediation time frames for 3rd party component vulnerabilities | Section 15.1.1 passed verification | — |
| Deployment And Configuration | Application documentation identifies resource-intensive functionality with availability protection measures | Section 15.1.3 passed verification | — |
| Data Privacy And Retention | Cache backends are trusted infrastructure; cache encryption is a deployment concern. | Promoted from dropped finding ASVS-1422-LOW-001 | — |
| Logging And Monitoring | Authentication lifecycle including logging delegated to Flask-AppBuilder | Promoted from dropped finding ASVS-1631-LOW-001 | — |
| Browser Security Headers | MCP tool and prompt decorators default protect=True for secure-by-default CSRF/auth enforcement | Promoted from dropped finding ASVS-351-INFO-001 | — |
| Browser Security Headers | Extensions are same-trust-boundary components; path-based namespace separation is organizational, not a security boundary | source: Dropped finding ASVS-354-INFO-001 | — |
| Http Protocol Security | Intermediary header overwrite/stripping delegated to reverse proxy layer | Dropped finding ASVS-413-INFO-001 - protection implemented at infrastructure layer | — |
| Token Based Authentication | Token signature verification delegated to Flask-AppBuilder's AUTH_TYPE-specific managers | source: Report positive patterns | — |
| Token Based Authentication | Algorithm allowlist enforcement delegated to Flask-AppBuilder OAuth/OIDC managers | Report positive patterns | — |
| Token Based Authentication | Key material trust and jku/x5u/jwk header validation delegated to Flask-AppBuilder | Report positive patterns | — |
| Token Based Authentication | Token validity time span (exp/nbf) verification delegated to Flask-AppBuilder | source: Report positive patterns | — |
| Token Based Authentication | Token type validation (access vs ID token) delegated to Flask-AppBuilder OAuth/OIDC managers | Report positive patterns | — |
| Token Based Authentication | Token audience (aud) claim validation delegated to Flask-AppBuilder | source: Report positive patterns | — |
| Token Based Authentication | Audience restriction enforcement for shared-key issuers delegated to Flask-AppBuilder | Report positive patterns | — |
| Async Task Security | Database-level unique index on dedup_key provides TOCTOU prevention at the persistence layer | Dropped finding ASVS-1542-LOW-001 - atomic constraint enforcement prevents race conditions between check and use operations | — |
| Memory Safety | Pydantic schema with explicit fields | All models in extensions/types.py use explicit Field() declarations with descriptions. Pydantic's default serialization excludes undeclared attributes, providing schema-based field projection by default. | extensions/types.py |
| Memory Safety | Field-level pattern validation | Regex pattern validation for critical identifiers (publisher, name, version) in extension system prevents injection of malicious values through extension metadata. | extensions/types.py:44-68 |
| Memory Safety | __all__ export list | Controls public API surface by explicitly defining which types are exported. | queries/types.py:145, tasks/types.py:186 |
| Memory Safety | TypedDict with total=False | TaskProperties uses total=False meaning only explicitly set keys are present, reducing the risk of accidentally exposing uninitialized default values. | tasks/types.py:51 |
| Memory Safety | Frozen dataclass for options | TaskOptions uses @dataclass(frozen=True) preventing post-construction modification that could inject additional fields. | — |
| Memory Safety | Separation of concerns | Codebase separates type definitions from business logic, enabling centralized schema review and making field-level access control easier to reason about at the serialization boundary. | — |
| Memory Safety | Modern Python typing | All files use from __future__ import annotations, type \| None union syntax, and appropriate typing constructs (TypedDict, dataclass, BaseModel), reducing the risk of type confusion at runtime. | — |
| Memory Safety | Abstract task lifecycle management | TaskContext abstract class defines cleanup and abort handlers with LIFO ordering guarantee for cleanup handlers, establishing framework for proper resource lifecycle management. | — |
...
| 15.3.1 | Verify that the application only returns the required subset of fields from a data object. For example, it should not return an entire data object, as some individual fields should not be accessible to users. | **Pass** | See FINDING-004 |
...
| 15.3.3 | Verify that the application has countermeasures to protect against mass assignment attacks by limiting allowed fields per controller and action, e.g., it is not possible to insert or update a field value when it was not intended to be part of that action. | **Pass** | See FINDING-002 |
...
| 16.5.1 | Verify that a generic message is returned to the consumer when an unexpected or security-sensitive error occurs, ensuring no exposure of sensitive internal system data such as stack traces, queries, secret keys, and tokens. | **Partial** | See FINDING-001 |
...
**Summary Statistics:**
- **Pass**: 55 requirements (15.9%)
- **Partial**: 1 requirements (0.3%)
- **N/A**: 289 requirements (83.8%)
...
|------------|----------|-------------------|------------------|---------------------|
| FINDING-001 | Low | 16.5.1 | — | superset-core/src/superset_core/tasks/types.py |
| FINDING-002 | Informational | 15.3.3 | — | superset-core/src/superset_core/common/daos.py |
| FINDING-003 | Informational | 15.3.5 | — | superset-core/src/superset_core/common/daos.py, superset-core/src/superset_core/tasks/daos.py |
| FINDING-004 | Informational | 15.3.1 | — | superset-core/src/superset_core/queries/types.py, superset-core/src/superset_core/tasks/types.py |

**Total Unique Findings**: 4 (0 Critical, 0 High, 0 Medium, 1 Low, 3 Info)

*1 of 4 are actionable. Informational findings are recorded here but not opened as GitHub issues; see issues.md for the 1 actionable items.*
