...
All 41 findings are actionable. No informational-only items remain after consolidation from 345 source reports.

### ASVS Level Coverage

This audit was conducted at **ASVS Level 3** (maximum depth), encompassing all L1 and L2 requirements in addition to L3-specific controls. Findings span 21 review domains including SSL/TLS encryption, object deserialization, HTTP protocol handling, cryptographic operations, input validation, firewall filtering, and service communication.

### Top 5 Risks

1. **HTTP Request Smuggling (FINDING-001, High)** — The HTTP codec lacks Transfer-Encoding handling, enabling request smuggling attacks when Apache MINA sits behind a reverse proxy. This allows attackers to bypass security controls, poison caches, or hijack other users' requests.

2. **Unbounded Header Accumulation DoS (FINDING-002, High)** — No maximum size limit is enforced on accumulated partial HTTP request headers, allowing a remote attacker to exhaust server memory with a single slow connection sending an infinitely large header block.

3. **Certificate Validation Bypass in Production Code (FINDING-003, Medium)** — A `BogusTrustManagerFactory` ships in the production module with no runtime guard preventing its use, potentially disabling all TLS certificate validation if mistakenly or maliciously activated.

4. **Unrestricted Java Deserialization (FINDING-004, FINDING-005, Medium)** — `ObjectSerializationInputStream` performs deserialization with no class filtering, and the decoder defaults to accepting all classes when no matchers are configured, exposing applications to remote code execution via crafted payloads.

5. **Unrestricted OGNL Expression Evaluation via JMX (FINDING-012, Medium)** — `IoServiceMBean` exposes OGNL expression evaluation through JMX with no production-disable mechanism, providing a direct path to arbitrary code execution for any entity with JMX access.

### Positive Controls Observed

The audit identified several architectural decisions that demonstrate sound security design principles:

| # | Control | Domain |
|---|---------|--------|
| 1 | SslFilter is opt-in by design; TLS policy decisions are properly delegated to the consuming application rather than silently defaulting to insecure transport | SSL/TLS Encryption |
| 2 | All cryptographic operations use standard JCA/JCE APIs with no reflection-based invocation, facilitating static analysis discovery and inheriting PQC capabilities via JDK upgrades | Cryptographic Operations |
| 3 | SslFilter provides configuration-time crypto agility via `setEnabledProtocols()`, `setEnabledCipherSuites()`, and `setEndpointIdentificationAlgorithm()`, enabling consumer-driven security hardening | Cryptographic Operations |
| 4 | Framework exposes custom Executor injection via `AbstractIoService` constructor, delegating thread pool sizing and resource policy to the consuming application | Service Communication |
| 5 | KeyStoreFactory supports custom providers (including HSM) via `setProvider()` and `setType()` API, and its factory pattern enables application-driven credential rotation | Secrets Management |
| 6 | mTLS enablement APIs (`setNeedClientAuth()`/`setWantClientAuth()`) are provided without forcing authentication policy, respecting separation of mechanism and policy | SSL/TLS Encryption |
| 7 | JDK 11+ disables weak ciphers by default, and the framework inherits these protections without overriding them | Cryptographic Operations |
| 8 | Application-layer responsibility boundaries are clearly maintained — sensitive data masking and field-level access control are delegated to application handlers | Logging/Monitoring, Dependency Management |

These controls indicate that the framework generally follows a sound "mechanism, not policy" philosophy. However, several areas (particularly HTTP parsing, deserialization, and default configurations) lack adequate secure-by-default postures that L3 compliance demands.
...
#### FINDING-001: Missing Transfer-Encoding handling enables HTTP Request Smuggling
...
The HttpServerDecoder only checks Content-Length for body framing and completely ignores Transfer-Encoding headers. This enables HTTP Request Smuggling when the server is deployed behind a reverse proxy that prioritizes Transfer-Encoding per RFC 7230 §3.3.3. When both Content-Length and Transfer-Encoding headers are present, no conflict detection occurs, enabling CL.TE request smuggling attacks.
...
#### FINDING-002: No maximum size limit on accumulated partial HTTP request headers enables memory exhaustion DoS
...
#### FINDING-003: Production module ships BogusTrustManagerFactory that bypasses all certificate validation with no runtime guard
...
| **ASVS Level(s)** | L2 |
| **CWE** | CWE-295 |
| **ASVS sections** | 12.1.3, 12.3.2, 12.3.4 |
...
BogusTrustManagerFactory in mina-core's public API implements X509TrustManager with empty checkClientTrusted/checkServerTrusted methods. When used with SslContextFactory, it completely bypasses certificate validation for both mTLS client certificates (12.1.3), TLS server certificate validation (12.3.2), and internal service certificate trust restrictions (12.3.4). This enables man-in-the-middle attacks when explicitly chosen by developers. Not exploitable in default configuration (SslContextFactory uses PKIX by default) but requires explicit developer choice to use. Class is in production module with no runtime guard or deprecation warning.
...
#### FINDING-004: ObjectSerializationInputStream performs Java deserialization with no class filtering mechanism
...
| **ASVS Level(s)** | L2, L3 |
| **CWE** | CWE-502 |
| **ASVS sections** | 1.5.2, 15.1.5, 15.2.5 |
| **Files** | mina-core/src/main/java/org/apache/mina/filter/codec/serialization/ObjectSerializationInputStream.java |
| **Source Reports** | 1.5.2.md, 15.1.5.md, 15.2.5.md |
| **Related** | FINDING-005, FINDING-018 |

**Description:**

The `ObjectSerializationInputStream` performs Java deserialization with size limit and classloader controls but no mandatory class allowlist enforcement, no sandboxing, and no architectural isolation. The ASVS 15.2.5 requirement calls for additional protections around dangerous functionality. The class filtering exists in IoBuffer and the framework intentionally delegates security configuration to applications. This is a defense-in-depth gap rather than an exploitable vulnerability.
...
#### FINDING-005: ObjectSerializationDecoder defaults to accepting all classes when no matchers are configured
...
| **CWE** | CWE-502 |
| **ASVS sections** | 1.5.2 |
| **Files** | mina-core/src/main/java/org/apache/mina/filter/codec/serialization/ObjectSerializationDecoder.java |
| **Source Reports** | 1.5.2.md |
| **Related** | FINDING-004, FINDING-018 |

**Description:**

ObjectSerializationDecoder's acceptMatchers list defaults to an empty ArrayList. When no accept() calls are made by the application, setMatchers(emptyList) is passed before getObject(), meaning all classes are accepted. The filtering mechanism exists but is not enforced by default, creating a foot-gun for applications that deploy the codec without configuring an allowlist.
...
#### FINDING-006: ConnectionThrottleFilter cleanup thread executes only once, leading to unbounded memory growth and DoS vulnerability
...
The ExpiredSessionThread in ConnectionThrottleFilter runs exactly once: it sleeps for allowedInterval (default 1000ms), performs a single cleanup pass, then exits. After this single execution (~1 second post-construction), no entries are ever reclaimed from the clients ConcurrentHashMap. A sustained attack from diverse IPs exhausts JVM heap memory. The component designed to provide availability protection itself becomes an availability vulnerability, allowing unbounded memory accumulation as a DoS vector.
...
#### FINDING-007: ConnectionThrottleFilter propagates session event downstream despite throttling, partially bypassing rate limit
...
When a connection is throttled, the filter executes session.closeNow() but then unconditionally calls nextFilter.sessionCreated(session), propagating the session creation event to downstream filters and handlers. This allows downstream handlers to execute logic for throttled sessions before the asynchronous close completes, partially bypassing the rate limit protection.
...
#### FINDING-008: IPv6 address handling in Subnet class is fundamentally broken, rendering BlacklistFilter ineffective for IPv6
...
| **CWE** | CWE-190 |
| **ASVS sections** | 2.4.1 |
| **Files** | mina-core/src/main/java/org/apache/mina/filter/firewall/Subnet.java |
| **Source Reports** | 2.4.1.md |
| **Related** | FINDING-039 |

**Description:**

The toLong(InetAddress) method in the Subnet class iterates all 16 bytes of an IPv6 address while shifting into a 64-bit long, causing the upper 64 bits to overflow and be lost. This produces incorrect subnet matching results for IPv6 addresses, allowing complete bypass of BlacklistFilter for IPv6 subnets and undermining anti-automation controls for IPv6 clients.
...
#### FINDING-009: Duplicate Content-Length headers silently resolved by last-wins semantics
...
Header parsing uses HashMap.put() which silently overwrites earlier values. Multiple Content-Length headers with different values are not detected or rejected per RFC 7230 §3.3.3, enabling potential request smuggling when proxy and decoder disagree on which value to use.
...
#### FINDING-010: Content-Length value not validated before use as message boundary
...
Content-Length header value is passed directly to Integer.valueOf() without validation. Negative values, non-numeric values, or extremely large values cause incorrect message framing or unhandled exceptions, potentially enabling request smuggling or DoS. Client-supplied Content-Length is used as a body framing indicator without verification that it is a valid non-negative integer within implementation limits.
...
#### FINDING-011: Custom HMAC-MD5 implementation instead of JCA Mac class
...
| **ASVS Level(s)** | L2 |
| **CWE** | CWE-327 |
| **ASVS sections** | 11.2.1 |
| **Files** | mina-core/src/main/java/org/apache/mina/proxy/handlers/http/ntlm/NTLMResponses.java |
| **Source Reports** | 11.2.1.md |
| **Related** | FINDING-013, FINDING-014, FINDING-015, FINDING-023, FINDING-024 |

**Description:**

The `hmacMD5` method implements the HMAC algorithm manually using raw `MessageDigest` and XOR operations instead of using Java's industry-validated `javax.crypto.Mac.getInstance("HmacMD5")`. The custom implementation also has an RFC 2104 compliance defect: it does not hash keys longer than the block size (64 bytes) before use.
...
#### FINDING-012: IoServiceMBean exposes unrestricted OGNL expression evaluation with no production-disable mechanism
...
| **Files** | mina-integration-jmx/src/main/java/org/apache/mina/integration/jmx/IoServiceMBean.java |
| **Source Reports** | 13.4.2.md, 13.4.5.md |
| **Related** | - |

**Description:**

IoServiceMBean monitoring endpoint exposes unrestricted OGNL-based session inspection and manipulation with no built-in access control. findSessions, findAndRegisterSessions, and findAndProcessSessions operations are available without restriction once the MBean is registered. OGNL can traverse to IoService, its handlers, filter chains, and the entire application object graph.
...
#### FINDING-013: SslContextFactory defaults to TLSv1.2 protocol, preventing TLS 1.3 negotiation unless explicitly overridden
...
| Files | mina-core/src/main/java/org/apache/mina/filter/ssl/SslContextFactory.java |
| Source Reports | 12.1.1.md |
| Related | FINDING-011, FINDING-014, FINDING-015, FINDING-023, FINDING-024 |

**Description:**

SslContextFactory.protocol defaults to "TLSv1.2". In Java's JSSE, SSLContext.getInstance("TLSv1.2") creates a context supporting protocols up to TLS 1.2 only. TLS 1.3 requires SSLContext.getInstance("TLSv1.3") or SSLContext.getInstance("TLS"). This prevents the application from preferring the latest TLS 1.3 protocol as required by ASVS 12.1.1.
...
#### FINDING-014: SslFilter does not enforce a minimum TLS protocol version when enabledProtocols is null
...
| Related | FINDING-011, FINDING-013, FINDING-015, FINDING-023, FINDING-024 |

**Description:**

SslFilter.enabledProtocols defaults to null. When null, no call to setEnabledProtocols is made, so SSLEngine uses JDK default enabled protocols. On JDK 11+ TLS 1.0/1.1 are disabled by default, but on older JDKs legacy protocols may remain enabled. This is a defense-in-depth gap rather than an active vulnerability. Affects both external-facing services (12.1.1) and internal service-to-service communication (12.3.1).
...
#### FINDING-015: SslFilter does not enforce a cipher suite allowlist by default; relies entirely on JDK defaults
...
| Related | FINDING-011, FINDING-013, FINDING-014, FINDING-023, FINDING-024 |

**Description:**

SslFilter.enabledCipherSuites defaults to null. When null, no call to setEnabledCipherSuites is made, so SSLEngine uses all JDK-enabled cipher suites including non-forward-secrecy suites. Modern JDKs prioritize AEAD/FS suites but may not exclusively enable them. For L3 applications, this fails to guarantee that only forward-secrecy cipher suites are enabled as required by ASVS 12.1.2.
...
#### FINDING-016: No API surface for configuring OCSP stapling or certificate revocation checking in the TLS configuration classes
...
| Files | mina-core/src/main/java/org/apache/mina/filter/ssl/SslContextFactory.java, mina-core/src/main/java/org/apache/mina/filter/ssl/SslFilter.java |
| Source Reports | 12.1.4.md |
| Related | |

**Description:**

Neither SslContextFactory nor SslFilter expose any mechanism for enabling OCSP stapling, configuring PKIXRevocationChecker, or setting CRL distribution points. Java's default does not enable revocation checking unless explicitly configured. The framework does not actively disable revocation checking and it can be configured at the JVM level, but lacks application-level API support required by ASVS 12.1.4 for L3 applications.
...
#### FINDING-017: SslFilter does not enable hostname verification by default for client mode
...
SslFilter.identificationAlgorithm defaults to null. When null, createEngine() skips setEndpointIdentificationAlgorithm(), so no hostname verification is performed. An attacker with any valid certificate could impersonate the target server. The framework exposes setEndpointIdentificationAlgorithm("HTTPS") for this purpose but does not enable it by default, failing ASVS 12.3.2 requirements for TLS clients to validate certificates received before communicating with a TLS server.
...
#### FINDING-018: Two deserialization entry points for the same wire format have inconsistent security properties
...
| CWE | CWE-502 |
| ASVS sections | 1.5.3 |
| Files | mina-core/src/main/java/org/apache/mina/filter/codec/serialization/ObjectSerializationDecoder.java, mina-core/src/main/java/org/apache/mina/filter/codec/serialization/ObjectSerializationInputStream.java |
| Source Reports | 1.5.3.md |
| Related | FINDING-004, FINDING-005 |

**Description:**

ObjectSerializationDecoder applies ClassNameMatcher filtering via in.setMatchers(acceptMatchers) before deserialization, while ObjectSerializationInputStream does not apply any matchers before calling buf.getObject(classLoader). Both parse the same wire format (4-byte big-endian length prefix + Java serialized object) and use the same underlying mechanism, but have divergent security properties. An application processing the same format through both paths will get different filtering behavior.
...
#### FINDING-019: BlacklistFilter performs O(n) iteration on every filter event without size bounds or performance documentation
...
| Files | mina-core/src/main/java/org/apache/mina/filter/firewall/BlacklistFilter.java |
| Source Reports | 15.1.3.md, 15.2.2.md |
| Related | FINDING-031 |

**Description:**

The isBlocked() method performs a linear scan of the entire blacklist CopyOnWriteArrayList for every filter event (sessionCreated, messageReceived, messageSent, sessionIdle, sessionOpened, event). No size limit control exists on the blacklist, and no documentation describes the O(n) algorithmic complexity, memory growth characteristics, or performance implications. With thousands of entries and concurrent sessions, cumulative CPU cost could contribute to latency and throughput degradation.
...
#### FINDING-020: Subnet.equals() only compares IPv4 fields, breaking unblock functionality for IPv6
...
The equals() method in the Subnet class only compares subnetInt (always 0 for IPv6) and suffix, meaning all IPv6 subnets with the same prefix length are considered equal. This breaks the ability to unblock specific IPv6 subnets from the blacklist. Additionally, no hashCode() override is provided, violating the Java contract for objects used in collections.
...
#### FINDING-021: IoBuffer.free() not protected by try-finally — resource leak on exception paths
...
| CWE | CWE-401 |
| ASVS sections | 1.4.3 |
| Files | mina-core/src/main/java/org/apache/mina/filter/codec/ProtocolCodecFilter.java |
| Source Reports | 1.4.3.md |
| Related | FINDING-006 |

**Description:**

In `ProtocolCodecFilter.messageReceived()`, `in.free()` is called after the decode loop but is not protected by a try-finally block. If an exception escapes the catch block (e.g., during `decoderOut.flush()` within the catch handler), the IoBuffer's direct memory is not released, potentially leading to resource exhaustion under sustained malformed input.
...
#### FINDING-022: No key lifecycle management mechanisms in framework cryptographic components
...
| Files | mina-core/src/main/java/org/apache/mina/filter/ssl/KeyStoreFactory.java |
| Source Reports | 11.1.1.md |
| Related | FINDING-026, FINDING-027, FINDING-032 |

**Description:**

The `KeyStoreFactory` class provides KeyStore creation and loading but implements no key lifecycle management features (rotation, expiration tracking, revocation checking, or usage constraints). The class stores the KeyStore password as a `char[]` field that is never zeroed after use, and passwords accepted as `String` remain in the JVM string pool indefinitely.
...
#### FINDING-023: DigestUtilities provides no algorithm agility beyond RFC 2617 mandated MD5
...
| ASVS Level(s) | L1, L2 |
| CWE | CWE-327 |
| ASVS sections | 11.2.2, 11.4.1, 11.4.3 |
...
| Related | FINDING-011, FINDING-013, FINDING-014, FINDING-015, FINDING-024 |

**Description:**

The `DigestUtilities` class initializes a static shared `MessageDigest` for MD5 with no mechanism to configure or swap the algorithm. RFC 7616 (2015) supersedes RFC 2617 and defines SHA-256 as a supported digest algorithm for HTTP Digest authentication. The current implementation cannot be reconfigured to use SHA-256 without code changes, preventing seamless upgrade to stronger algorithms.
...
#### FINDING-024: DES/ECB mode used in NTLM protocol implementation
...
| ASVS Level(s) | L1 |
| CWE | CWE-327 |
| ASVS sections | 11.3.1 |
| Files | mina-core/src/main/java/org/apache/mina/proxy/handlers/http/ntlm/NTLMResponses.java |
| Source Reports | 11.3.1.md |
| Related | FINDING-011, FINDING-013, FINDING-014, FINDING-015, FINDING-023 |

**Description:**

The `lmHash()` and `lmResponse()` methods use DES/ECB/NoPadding which is an insecure block mode. However, this is mandated by the NTLM protocol specification and cannot be changed without breaking compatibility. Applications should prefer NTLMv2 or modern authentication methods.
...
#### FINDING-025: Hardcoded Nonce Count Prevents Proper Nonce Reuse Detection in HTTP Digest Authentication
...
The DigestUtilities class hardcodes nonce count as '00000001' regardless of actual reuse count, preventing servers from detecting replayed authentication responses via nonce count tracking. If a proxy server reuses nonces across multiple requests, the client always claims nc=00000001.
...
#### FINDING-026: KeyStoreFactory retains password and keystore data after use without zeroing
...
| ASVS Level(s) | L3, L2 |
| CWE | CWE-316 |
| ASVS sections | 11.7.2, 13.3.1 |
| Files | mina-core/src/main/java/org/apache/mina/filter/ssl/KeyStoreFactory.java |
| Source Reports | 11.7.2.md, 13.3.1.md |
| Related | FINDING-022, FINDING-027, FINDING-032 |

**Description:**

KeyStoreFactory retains sensitive secret material (password and keystore data) in memory indefinitely without clearing. password and data fields are NEVER zeroed after use in newInstance(). Requires local/privileged access (heap dump, debugging attachment, memory forensics).
...
#### FINDING-027: NTLMResponses does not zero intermediate key material after cryptographic operations
...
| Related | FINDING-022, FINDING-026, FINDING-032 |

**Description:**

NTLMResponses methods (lmHash, lmResponse, hmacMD5) retain intermediate key-derived byte arrays (oemPassword, keyBytes, ipad, opad) in memory without zeroing after use. Requires local/physical JVM access. Effectiveness limited by Java String immutability of source password.
...
#### FINDING-028: MdcInjectionFilter.setProperty() does not encode values before MDC injection, enabling log injection when applications pass user-controlled input
...
| Files | mina-core/src/main/java/org/apache/mina/filter/logging/MdcInjectionFilter.java |
...
#### FINDING-029: ThreadLocal state leak when exception occurs before try-finally block
...
#### FINDING-030: Default Unbounded Thread Pool Without Documented Limits
...
| Files | mina-core/src/main/java/org/apache/mina/core/service/AbstractIoService.java |
| Source Reports | 13.1.2.md |
| Related | FINDING-002 |

**Description:**

When no Executor is provided, AbstractIoService creates an unbounded CachedThreadPool. There is no documented maximum number of concurrent connections or threads, no fallback mechanism when resources are exhausted, and no recovery strategy. Remote unauthenticated attacker can open many connections simultaneously causing DoS through thread/memory exhaustion.
...
#### FINDING-031: No Documented Resource-Release Procedures or Timeout Strategies for Default Executor
...
#### FINDING-032: KeyStoreFactory accepts password as immutable Java String, preventing secret scrubbing of the original value
...
| Related | FINDING-022, FINDING-026, FINDING-027 |
...
#### FINDING-033: IoServiceMBean uses blacklist-based method filtering rather than whitelist for JMX operation exposure
...
#### FINDING-034: IoService exposes TransportMetadata and IoServiceStatistics through JMX without version information filtering
...
| Files | mina-core/src/main/java/org/apache/mina/core/service/IoService.java, mina-integration-jmx/src/main/java/org/apache/mina/integration/jmx/IoServiceMBean.java |
...
#### FINDING-035: IoBuffer free() does not guarantee content clearing before pool return
...
| Files | mina-core/src/main/java/org/apache/mina/core/buffer/IoBuffer.java |
| Source Reports | 14.2.2.md |
| Related | |

**Description:**

When using CachedBufferAllocator (non-default), IoBuffer.free() returns buffers to the pool without clearing content, potentially allowing sensitive data from one session to be visible to another session that receives the recycled buffer. Requires non-default configuration and co-tenant sessions in the same JVM.
...
#### FINDING-036: No formal SBOM generation mechanism observed in the project
...
| Files | Project build configuration |
| Source Reports | 15.1.2.md |
| Related | |

**Description:**

The project relies on Maven `pom.xml` for dependency declarations but does not appear to generate a formal Software Bill of Materials (SBOM) in a standard format (CycloneDX, SPDX). While Maven provides dependency visibility via `mvn dependency:tree`, this is not a maintained, versioned SBOM artifact that can be audited, shared with consumers, or monitored for newly disclosed vulnerabilities.
...
#### FINDING-037: Deprecated API usage (Class.newInstance()) indicates component not updated for modern Java
...
| Files | mina-integration-beans/src/main/java/org/apache/mina/integration/beans/PropertyEditorFactory.java:121 |
| Source Reports | 15.2.1.md |
| Related | |

**Description:**

PropertyEditorFactory uses Class.newInstance() (line 121), which was deprecated in Java 9 (2017) in favor of Constructor.newInstance(). While not a security vulnerability per se, the continued use of deprecated APIs indicates that this component has not been updated to follow modern Java practices.
...
#### FINDING-038: Dynamic Class Loading in PropertyEditorFactory Uses Pattern Susceptible to Classpath Manipulation
...
| ASVS Level(s) | L3 |
| CWE | |
| ASVS sections | 15.2.4, 15.1.5 |
| Files | mina-integration-beans/src/main/java/org/apache/mina/integration/beans/PropertyEditorFactory.java |
| Source Reports | 15.2.4.md, 15.1.5.md |
| Related | |

**Description:**

`PropertyEditorFactory.getInstance(Class<?>)` uses dynamic class loading that constructs class names via string concatenation within the `org.apache.mina.integration.beans` package namespace. If a malicious transitive dependency were to place a class in this exact package (possible via split packages in multi-JAR environments before Java modules), the `newInstance()` call would execute the no-arg constructor of the malicious class.
...
#### FINDING-039: Integer Overflow Due to Missing Upper Bound Validation on maxObjectSize
...
When `maxObjectSize` is set to a value > `Integer.MAX_VALUE - 4`, an attacker-controlled stream providing `objectSize = Integer.MAX_VALUE` passes validation, then `objectSize + 4` overflows to a negative value passed to `IoBuffer.allocate()`. Requires unusual application configuration; default 1MB is safe.
...
#### FINDING-040: Mutable Field `maxObjectSize` Not Declared Volatile
...
The `maxObjectSize` field is mutable via `setMaxObjectSize()` but not declared `volatile`. In multi-threaded access scenarios, a reading thread may see a stale value. Exploitability is very low since `InputStream` objects are inherently sequential and per-session in typical Mina usage.
...
#### FINDING-041: Blocking I/O in readObject() May Contribute to Thread Starvation in Shared Worker Pools
...
The `readObject()` method performs a blocking read operation via `DataInputStream.readFully()` that can hold a thread pool worker thread until all `objectSize` bytes are received from the network. In shared worker pool scenarios, slow or malicious clients could contribute to thread pool exhaustion by holding threads for extended periods.

**Remediation:**

Consider adding an optional read timeout mechanism or documenting that applications using `ObjectSerializationDecoder` should configure socket-level timeouts and appropriately size their thread pools.
...
|--------|---------|-----------------|------------------|
| Ssl Tls Encryption | SslFilter is opt-in by design; whether to require TLS is an application policy decision delegated to the consumer | 12.2.1 assessment - framework provides TLS capabilities without enforcing usage policy | — |
| Ssl Tls Encryption | Certificate provisioning delegated to deployer; framework provides KeyStore configuration API without enforcing certificate trust chain policy | 12.2.2 assessment - framework provides certificate configuration without mandating public CA trust | — |
| Ssl Tls Encryption | mTLS enablement is an application policy decision; framework provides setNeedClientAuth()/setWantClientAuth() APIs for consumers to configure | 12.3.5 assessment - framework provides mTLS capabilities without enforcing authentication policy | — |
| Ssl Tls Encryption | SslFilter provides TLS encryption capabilities for internal HTTP-based service communication | 12.3.3 assessment - Pass status indicates adequate support for encrypted internal communications | — |
| Cryptographic Operations | Cryptographic operations use standard JCA APIs organized by logical function, facilitating external inventory tooling | source: Dropped finding ASVS-1112-INFO-001 | — |
| Cryptographic Operations | All crypto operations use standard JCA APIs with no reflection-based invocation, making them trivially discoverable by static analysis | Promoted from dropped finding ASVS-1113-INFO-001 | — |
| Cryptographic Operations | Framework delegates to JCA/JCE and provides configurable TLS protocol/cipher setters, inheriting PQC capabilities via JDK upgrades | source: Dropped finding ASVS-1114-INFO-001 | — |
| Cryptographic Operations | SslFilter provides configuration-time crypto agility via setEnabledProtocols/setEnabledCipherSuites/setEndpointIdentificationAlgorithm | source: Dropped finding ASVS-1122-LOW-002 | — |
| Cryptographic Operations | SslFilter exposes setEnabledCipherSuites for consumer-driven cipher restriction | Promoted from dropped finding ASVS-1123-LOW-001 | — |
| Cryptographic Operations | JDK 11+ disables weak ciphers by default | Promoted from dropped finding ASVS-1123-LOW-001 | — |
| Cryptographic Operations | SslFilter provides setEnabledCipherSuites allowing consumer to restrict to AEAD-only; JDK prefers GCM by default | Promoted from dropped finding ASVS-1132-LOW-001 | — |
| Cryptographic Operations | TLS provides inherent MAC authentication even with non-AEAD suites; GCM preferred by modern JDKs | source: Dropped finding ASVS-1133-LOW-001 | — |
| Cryptographic Operations | TLS cipher suite and protocol selection delegated to consuming application and JDK defaults; framework exposes configuration surface without enforcing policy | Promoted from dropped finding ASVS-1161-LOW-001 | — |
| Logging Monitoring | Application is responsible for masking/classifying sensitive data before passing to framework APIs | Dropped finding ASVS-1625-LOW-001 | — |
| Service Communication | Framework exposes custom Executor injection via AbstractIoService constructor, delegating thread pool sizing policy to the consuming application. | Promoted from dropped finding ASVS-1326-LOW-001 | — |
| Secrets Management | KeyStoreFactory supports custom providers (including HSM) via setProvider() and setType() API | Dropped finding ASVS-1333-LOW-001 | — |
| Secrets Management | KeyStoreFactory factory pattern allows re-creation with updated credentials, enabling application-driven rotation | Promoted from dropped finding ASVS-1334-LOW-001 | — |
| Dependency Management | Framework delegates field-level access control to application handlers after message delivery | Promoted from dropped finding ASVS-1533-LOW-001 | — |
...
| 1.4.3 | Verify that dynamically allocated memory and resources are released, and that references or pointers to freed memory are removed or set to null to prevent dangling pointers and use-after-free vulnerabilities. | **Partial** | See FINDING-021 |
...
| 2.4.1 | Verify that anti-automation controls are in place to protect against excessive calls to application functions that could lead to data exfiltration, garbage-data creation, quota exhaustion, rate-limit breaches, denial-of-service, or overuse of costly resources. | **Fail** | See FINDING-006, FINDING-007, FINDING-008, FINDING-020 |
...
| 4.1.4 | Verify that only HTTP methods that are explicitly supported by the application or its API (including OPTIONS during preflight requests) can be used and that unused methods are blocked. | **N/A** |  |
...
| 4.2.1 | Verify that all application components (including load balancers, firewalls, and application servers) determine boundaries of incoming HTTP messages using the appropriate mechanism for the HTTP version to prevent HTTP request smuggling. In HTTP/1.x, if a Transfer-Encoding header field is present, the Content-Length header must be ignored per RFC 2616. When using HTTP/2 or HTTP/3, if a Content-Length header field is present, the receiver must ensure that it is consistent with the length of the DATA frames. | **Fail** | See FINDING-001, FINDING-009, FINDING-010 |
| 4.2.2 | Verify that when generating HTTP messages, the Content-Length header field does not conflict with the length of the content as determined by the framing of the HTTP protocol, in order to prevent request smuggling attacks. | **Fail** | See FINDING-001, FINDING-010 |
...
| **V11: Cryptography** | | | |
| 11.1.1 | Verify that there is a documented policy for management of cryptographic keys and a cryptographic key lifecycle that follows a key management standard such as NIST SP 800-57. This should include ensuring that keys are not overshared (for example, with more than two entities for shared secrets and more than one entity for private keys). | **Partial** | See FINDING-022 |
...
| 11.2.1 | Verify that industry-validated implementations (including libraries and hardware-accelerated implementations) are used for cryptographic operations. | **Fail** | See FINDING-011 |
| 11.2.2 | Verify that the application is designed with crypto agility such that random number, authenticated encryption, MAC, or hashing algorithms, key lengths, rounds, ciphers and modes can be reconfigured, upgraded, or swapped at any time, to protect against cryptographic breaks. Similarly, it must also be possible to replace keys and passwords and re-encrypt data. This will allow for seamless upgrades to post-quantum cryptography (PQC), once high-assurance implementations of approved PQC schemes or standards are widely available. | **Partial** | See FINDING-023 |
...
| 11.3.1 | Verify that insecure block modes (e.g., ECB) and weak padding schemes (e.g., PKCS#1 v1.5) are not used. | **Partial** | See FINDING-024 |
...
| 11.3.4 | Verify that nonces, initialization vectors, and other single-use numbers are not used for more than one encryption key and data-element pair. The method of generation must be appropriate for the algorithm being used. | **Partial** | See FINDING-025 |
...
| 11.4.1 | Verify that only approved hash functions are used for general cryptographic use cases, including digital signatures, HMAC, KDF, and random bit generation. Disallowed hash functions, such as MD5, must not be used for any cryptographic purpose. | **Partial** | See FINDING-023 |
...
| 11.4.3 | Verify that hash functions used in digital signatures, as part of data authentication or data integrity are collision resistant and have appropriate bit-lengths. If collision resistance is required, the output length must be at least 256 bits. If only resistance to second pre-image attacks is required, the output length must be at least 128 bits. | **Partial** | See FINDING-023 |
...
| 11.7.2 | Verify that data minimization ensures the minimal amount of data is exposed during processing, and ensure that data is encrypted immediately after use or as soon as feasible. | **Partial** | See FINDING-026, FINDING-027 |
| **V12: Secure Communication** | | | |
| 12.1.1 | Verify that only the latest recommended versions of the TLS protocol are enabled, such as TLS 1.2 and TLS 1.3. The latest version of the TLS protocol must be the preferred option. | **Partial** | See FINDING-013, FINDING-014 |
| 12.1.2 | Verify that only recommended cipher suites are enabled, with the strongest cipher suites set as preferred. L3 applications must only support cipher suites which provide forward secrecy. | **Partial** | See FINDING-015 |
| 12.1.3 | Verify that the application validates that mTLS client certificates are trusted before using the certificate identity for authentication or authorization. | **Partial** | See FINDING-003 |
| 12.1.4 | Verify that proper certification revocation, such as Online Certificate Status Protocol (OCSP) Stapling, is enabled and configured. | **Partial** | See FINDING-016 |
...
| 12.3.4 | Verify that TLS connections between internal services use trusted certificates. Where internally generated or self-signed certificates are used, the consuming service must be configured to only trust specific internal CAs and specific self-signed certificates. | **Partial** | See FINDING-003 |
...
| 13.3.1 | Verify that a secrets management solution, such as a key vault, is used to securely create, store, control access to, and destroy backend secrets. These could include passwords, key material, integrations with databases and third-party systems, keys and seeds for time-based tokens, other internal secrets, and API keys. Secrets must not be included in application source code or included in build artifacts. For an L3 application, this must involve a hardware-backed solution such as an HSM. | **Fail** | See FINDING-026, FINDING-032 |
| 13.3.2 | Verify that access to secret assets adheres to the principle of least privilege. | **Pass** |  |
| 13.3.3 | Verify that all cryptographic operations are performed using an isolated security module (such as a vault or hardware security module) to securely manage and protect key material from exposure outside of the security module. | **Partial** |  |
...
| 13.4.2 | Verify that debug modes are disabled for all components in production environments to prevent exposure of debugging features and information leakage. | **Fail** | See FINDING-012 |
...
| 13.4.5 | Verify that documentation (such as for internal APIs) and monitoring endpoints are not exposed unless explicitly intended. | **Fail** | See FINDING-012, FINDING-033 |
| 13.4.6 | Verify that the application does not expose detailed version information of backend components. | **Fail** | See FINDING-034 |
...
| 14.2.2 | Verify that the application prevents sensitive data from being cached in server components, such as load balancers and application caches, or ensures that the data is securely purged after use. | **Partial** | See FINDING-035 |
...
| 15.1.2 | Verify that an inventory catalog, such as software bill of materials (SBOM), is maintained of all third-party libraries in use, including verifying that components come from pre-defined, trusted, and continually maintained repositories. | **Partial** | See FINDING-036 |
| 15.1.3 | Verify that the application documentation identifies functionality which is time-consuming or resource-demanding. This must include how to prevent a loss of availability due to overusing this functionality and how to avoid a situation where building a response takes longer than the consumer's timeout. Potential defenses may include asynchronous processing, using queues, and limiting parallel processes per user and per application. | **Fail** | See FINDING-019 |
...
| 15.1.5 | Verify that application documentation highlights parts of the application where "dangerous functionality" is being used. | **Partial** | See FINDING-004, FINDING-038 |
| 15.2.1 | Verify that the application only contains components which have not breached the documented update and remediation time frames. | **Partial** | See FINDING-037 |
| 15.2.2 | Verify that the application has implemented defenses against loss of availability due to functionality which is time-consuming or resource-demanding, based on the documented security decisions and strategies for this. | **Fail** | See FINDING-006, FINDING-019 |
...
| 15.2.4 | Verify that third-party components and all of their transitive dependencies are included from the expected repository, whether internally owned or an external source, and that there is no risk of a dependency confusion attack. | **Partial** | See FINDING-038 |
| 15.2.5 | Verify that the application implements additional protections around parts of the application which are documented as containing "dangerous functionality" or using third-party libraries considered to be "risky components". This could include techniques such as sandboxing, encapsulation, containerization or network level isolation to delay and deter attackers who compromise one part of an application from pivoting elsewhere in the application. | **Partial** | See FINDING-004 |
...
| 15.4.1 | Verify that shared objects in multi-threaded code (such as caches, files, or in-memory objects accessed by multiple threads) are accessed safely by using thread-safe types and synchronization mechanisms like locks or semaphores to avoid race conditions and data corruption. | **Partial** | See FINDING-040 |
...
| 15.4.4 | Verify that resource allocation policies prevent thread starvation by ensuring fair access to resources, such as by leveraging thread pools, allowing lower-priority threads to proceed within a reasonable timeframe. | **Partial** | See FINDING-041 |
...
| 16.2.1 | Verify that each log entry includes necessary metadata (such as when, where, who, what) that would allow for a detailed investigation of the timeline when an event happens. | **N/A** |  |
...
| 16.4.1 | Verify that all logging components appropriately encode data to prevent log injection. | **Fail** | See FINDING-028 |
...
| 16.5.1 | Verify that a generic message is returned to the consumer when an unexpected or security-sensitive error occurs, ensuring no exposure of sensitive internal system data such as stack traces, queries, secret keys, and tokens. | **N/A** |  |
...
| 16.5.3 | Verify that the application fails gracefully and securely, including when an exception occurs, preventing fail-open conditions such as processing a transaction despite errors resulting from validation logic. | **Fail** | See FINDING-029 |
...
**Summary Statistics:**
- **Pass**: 15 requirements (4.3%)
- **Partial**: 28 requirements (8.1%)
- **N/A**: 288 requirements (83.5%)
- **Fail**: 14 requirements (4.1%)
...
|------------|----------|-------------------|------------------|---------------------|
| FINDING-001 | High | 4.2.1, 4.2.2 | FINDING-009 | mina-http/src/main/java/org/apache/mina/http/HttpServerDecoder.java |
| FINDING-002 | High | 4.2.5 | FINDING-030 | mina-http/src/main/java/org/apache/mina/http/HttpServerDecoder.java |
| FINDING-003 | Medium | 12.1.3, 12.3.2, 12.3.4 | — | mina-core/src/main/java/org/apache/mina/filter/ssl/BogusTrustManagerFactory.java |
| FINDING-004 | Medium | 1.5.2, 15.1.5, 15.2.5 | FINDING-005, FINDING-018 | mina-core/src/main/java/org/apache/mina/filter/codec/serialization/ObjectSerializationInputStream.java |
| FINDING-005 | Medium | 1.5.2 | FINDING-004, FINDING-018 | mina-core/src/main/java/org/apache/mina/filter/codec/serialization/ObjectSerializationDecoder.java |
| FINDING-006 | Medium | 15.2.2, 2.4.1 | FINDING-021 | mina-core/src/main/java/org/apache/mina/filter/firewall/ConnectionThrottleFilter.java |
| FINDING-007 | Medium | 2.4.1 | — | mina-core/src/main/java/org/apache/mina/filter/firewall/ConnectionThrottleFilter.java |
| FINDING-008 | Medium | 2.4.1 | FINDING-039 | mina-core/src/main/java/org/apache/mina/filter/firewall/Subnet.java |
| FINDING-009 | Medium | 4.2.1 | FINDING-001 | mina-http/src/main/java/org/apache/mina/http/HttpServerDecoder.java |
| FINDING-010 | Medium | 4.2.1, 4.2.2 | — | mina-http/src/main/java/org/apache/mina/http/HttpServerDecoder.java |
| FINDING-011 | Medium | 11.2.1 | FINDING-013, FINDING-014, FINDING-015, FINDING-023, FINDING-024 | mina-core/src/main/java/org/apache/mina/proxy/handlers/http/ntlm/NTLMResponses.java |
| FINDING-012 | Medium | 13.4.2, 13.4.5 | — | mina-integration-jmx/src/main/java/org/apache/mina/integration/jmx/IoServiceMBean.java |
| FINDING-013 | Low | 12.1.1 | FINDING-011, FINDING-014, FINDING-015, FINDING-023, FINDING-024 | mina-core/src/main/java/org/apache/mina/filter/ssl/SslContextFactory.java |
| FINDING-014 | Low | 12.1.1, 12.3.1 | FINDING-011, FINDING-013, FINDING-015, FINDING-023, FINDING-024 | mina-core/src/main/java/org/apache/mina/filter/ssl/SslFilter.java |
| FINDING-015 | Low | 12.1.2 | FINDING-011, FINDING-013, FINDING-014, FINDING-023, FINDING-024 | mina-core/src/main/java/org/apache/mina/filter/ssl/SslFilter.java |
| FINDING-016 | Low | 12.1.4 | — | mina-core/src/main/java/org/apache/mina/filter/ssl/SslContextFactory.java, mina-core/src/main/java/org/apache/mina/filter/ssl/SslFilter.java |
| FINDING-017 | Low | 12.3.2 | — | mina-core/src/main/java/org/apache/mina/filter/ssl/SslFilter.java |
| FINDING-018 | Low | 1.5.3 | FINDING-004, FINDING-005 | mina-core/src/main/java/org/apache/mina/filter/codec/serialization/ObjectSerializationDecoder.java, mina-core/src/main/java/org/apache/mina/filter/codec/serialization/ObjectSerializationInputStream.java |
| FINDING-019 | Low | 15.1.3, 15.2.2 | FINDING-031 | mina-core/src/main/java/org/apache/mina/filter/firewall/BlacklistFilter.java |
| FINDING-020 | Low | 2.4.1 | — | mina-core/src/main/java/org/apache/mina/filter/firewall/Subnet.java |
| FINDING-021 | Low | 1.4.3 | FINDING-006 | mina-core/src/main/java/org/apache/mina/filter/codec/ProtocolCodecFilter.java |
| FINDING-022 | Low | 11.1.1 | FINDING-026, FINDING-027, FINDING-032 | mina-core/src/main/java/org/apache/mina/filter/ssl/KeyStoreFactory.java |
| FINDING-023 | Low | 11.2.2, 11.4.1, 11.4.3 | FINDING-011, FINDING-013, FINDING-014, FINDING-015, FINDING-024 | mina-core/src/main/java/org/apache/mina/proxy/handlers/http/digest/DigestUtilities.java |
| FINDING-024 | Low | 11.3.1 | FINDING-011, FINDING-013, FINDING-014, FINDING-015, FINDING-023 | mina-core/src/main/java/org/apache/mina/proxy/handlers/http/ntlm/NTLMResponses.java |
| FINDING-025 | Low | 11.3.4 | — | mina-core/src/main/java/org/apache/mina/proxy/handlers/http/digest/DigestUtilities.java |
| FINDING-026 | Low | 11.7.2, 13.3.1 | FINDING-022, FINDING-027, FINDING-032 | mina-core/src/main/java/org/apache/mina/filter/ssl/KeyStoreFactory.java |
| FINDING-027 | Low | 11.7.2 | FINDING-022, FINDING-026, FINDING-032 | mina-core/src/main/java/org/apache/mina/proxy/handlers/http/ntlm/NTLMResponses.java |
| FINDING-028 | Low | 16.4.1 | — | mina-core/src/main/java/org/apache/mina/filter/logging/MdcInjectionFilter.java |
| FINDING-029 | Low | 16.5.3 | — | mina-core/src/main/java/org/apache/mina/filter/logging/MdcInjectionFilter.java |
| FINDING-030 | Low | 13.1.2 | FINDING-002 | mina-core/src/main/java/org/apache/mina/core/service/AbstractIoService.java |
| FINDING-031 | Low | 13.1.3 | FINDING-019 | mina-core/src/main/java/org/apache/mina/core/service/AbstractIoService.java |
| FINDING-032 | Low | 13.3.1 | FINDING-022, FINDING-026, FINDING-027 | mina-core/src/main/java/org/apache/mina/filter/ssl/KeyStoreFactory.java |
| FINDING-033 | Low | 13.4.5 | — | mina-integration-jmx/src/main/java/org/apache/mina/integration/jmx/IoServiceMBean.java |
| FINDING-034 | Low | 13.4.6 | — | mina-core/src/main/java/org/apache/mina/core/service/IoService.java, mina-integration-jmx/src/main/java/org/apache/mina/integration/jmx/IoServiceMBean.java |
| FINDING-035 | Low | 14.2.2 | — | mina-core/src/main/java/org/apache/mina/core/buffer/IoBuffer.java |
| FINDING-036 | Low | 15.1.2 | — | Project build configuration |
| FINDING-037 | Low | 15.2.1 | — | mina-integration-beans/src/main/java/org/apache/mina/integration/beans/PropertyEditorFactory.java |
| FINDING-038 | Low | 15.2.4, 15.1.5 | — | mina-integration-beans/src/main/java/org/apache/mina/integration/beans/PropertyEditorFactory.java |
| FINDING-039 | Low | 15.3.5 | FINDING-008 | mina-core/src/main/java/org/apache/mina/filter/codec/serialization/ObjectSerializationInputStream.java |
| FINDING-040 | Low | 15.4.1 | — | mina-core/src/main/java/org/apache/mina/filter/codec/serialization/ObjectSerializationInputStream.java |
| FINDING-041 | Low | 15.4.4 | — | mina-core/src/main/java/org/apache/mina/filter/codec/serialization/ObjectSerializationInputStream.java |

**Total Unique Findings**: 41 (0 Critical, 2 High, 10 Medium, 29 Low, 0 Info)
...
**Total consolidated findings: 41**
