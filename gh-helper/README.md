...
1. **Sentinel, label, and PR-link checks** — skip issues already triaged by this agent (label-based fast-path first, comment-sentinel scan as fallback), or skip with classification `pr_in_progress` if a PR is linked. The label fast-path filters out issues that already carry the configured label (default `gh-helper`) before fetching their comments, since labels are applied on every processing path — saves one paginated comments fetch per already-seen issue, which matters on long-thread issues with many pages of discussion. The comment-sentinel scan runs as backup on the remaining un-labeled issues, catching the case where someone stripped the label manually but a v2 sentinel-bearing comment is still present. For `pr_in_progress`, a brief explanatory comment is posted (idempotent via its own sentinel) noting which PR(s) the agent identified as in-flight, so the user isn't left wondering why the issue got a label without an explanation. Sentinel-skipped issues already have a full triage comment from the prior run, so no new comment is posted there.
